...
class AutomationController:
            ...
            if performance_config.get("enabled", True):
                self.task_executors[
                    ...
                    ai_orchestra_dir=str(
                        self.ai_orchestra_dir.relative_to(self.base_dir)
                    ),
                    automation_level=AutomationLevel(
                        performance_config.get("automation_level", 2)
                    ),
    ...
    async def run_task(
        self, task: AutomationTask, mode: AutomationMode
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if mode == AutomationMode.ANALYZE:
                if task == AutomationTask.PERFORMANCE:
                    ...
                    result = {
                        "status": "success",
                        "profile": optimization_profile.to_dict(),
                    }
                ...
                if task == AutomationTask.PERFORMANCE:
                    ...
                    frequency = self.config.get("performance", {}).get(
                        "analysis_frequency_hours", 24
                    )
    ...
    async def run_multiple_tasks(
        self, tasks: List[AutomationTask], mode: AutomationMode
    ...
    async def run_all_tasks(self, mode: AutomationMode) -> Dict[str, Dict[str, Any]]:
        ...
        tasks = [
            ...
            if task != AutomationTask.ALL
            and self.config.get(task.value, {}).get("enabled", True)
        ]

        return await self.run_multiple_tasks(tasks, mode)

    async def generate_report(
        self, results: Dict[str, Dict[str, Any]], format: str = "text"
    ) -> str:
        """
    ...
    async def notify_results(self, report: str, format: str = "text") -> bool:
        """
        ...
        if notification_config.get("email_enabled", False):
            ...
            if recipients:
                try:
                    ...
                    msg[
                        "Subject"
                    ] = f"AI Orchestra Automation Report - {self.environment.value.title()}"
                        ...
                        "attachments": [
                            {
                                ...
                                "text": report
                                if format == "text"
                                else "Please see attached report",
                                "color": "#36a64f",
                            }
                        ],
                    }
