...
if [[ "$DEPLOY_MODE" == "docker" ]]; then
  ...
  if [[ -z "$GOOGLE_CLOUD_PROJECT" ]]; then
    echo -e "${RED}Error: GOOGLE_CLOUD_PROJECT is required for GCP deployment.${NC}"
    echo -e "${YELLOW}Set it in $ENV_FILE or export it manually.${NC}"
    exit 1
  fi
  
  # Build and push the container to GCR
  IMAGE_NAME="gcr.io/$GOOGLE_CLOUD_PROJECT/orchestra-llm:${ENVIRONMENT}"
  
  echo -e "${YELLOW}Building and pushing Docker image to GCR...${NC}"
  if [[ "$BUILDKIT_ENABLED" == "true" ]]; then
  ...
  if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Failed to push Docker image to GCR.${NC}"
    exit 1
  fi
  ...
  if [[ "$START_SERVICE" == "true" ]]; then
    ...
    # Convert env file to Cloud Run format
    ENV_VARS=$(grep -v '^#' "$ENV_FILE" | grep -v '^$' | sed 's/^/--set-env-vars=/g' | tr '\n' ' ')
    
    # Deploy to Cloud Run
    gcloud run deploy orchestra-llm-service \
    ...
    echo -e "${GREEN}✓ Service URL: $(gcloud run services describe orchestra-llm-service --platform=managed --region=us-central1 --format='value(status.url)')${NC}"
  fi
