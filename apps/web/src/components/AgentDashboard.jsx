...
const AgentDashboard = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      if (agents.length > 1) {
        const newSelectedAgent = agents.find(agent => agent.id !== selectedAgent.id);
      ...
      if (formMode === 'create') {
        ...
        const updatedAgents = agents.map(agent => 
          agent.id === selectedAgent.id 
            ? { ...agent, name: response.name, description: response.description } 
            : agent
        );
        
        setAgents(updatedAgents);
        setSelectedAgent({ ...selectedAgent, name: response.name, description: response.description });
      }
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        <Grid item xs={12} md={4}>
          <Paper elevation={2} sx={{ p: 2, height: '100%' }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
              ...
              <Box>
                <IconButton onClick={fetchAgents} size="small" title="Refresh agents">
                  <RefreshIcon />
                </IconButton>
                <Button
                  variant="contained"
          ...
          {selectedAgent ? (
            <Paper elevation={2} sx={{ p: 2 }}>
              ...
              <Box sx={{ py: 2 }}>
                ...
                {tabValue === 2 && (
                  <Box>
                    <Typography variant="subtitle1">Agent Configuration</Typography>
      ...
      {/* Test Dialog */}
      <Dialog open={testDialogOpen} onClose={handleTestClose} maxWidth="md" fullWidth>
