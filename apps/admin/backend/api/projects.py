...
PROJECT_STATUS_VALUES = [
    'initiated', 'planning', 'execution', 'review', 
    'consolidation', 'completed', 'failed'
]
...
def filter_projects(
    ...
    if persona_id:
        filtered_projects = [p for p in filtered_projects if p.get("active_persona") == persona_id]
    
    if priority:
        filtered_projects = [p for p in filtered_projects if p.get("priority") == priority]
