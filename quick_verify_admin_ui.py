...
class AdminUIVerifier:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if content_size < 500:
                    ...
                    self.results["accessibility"]["details"].append("Content size is suspiciously small. Possible blank page.")
    ...
    def _check_resources(self) -> None:
        ...
        try:
            ...
            for link in soup.find_all('link'):
                if link.has_attr('rel') and 'stylesheet' in link['rel'] and link.has_attr('href') and link['href'].endswith('.css'):
                    ...
                    try:
                        ...
                        if js_response.status_code == 200:
                            ...
                            if js_size < self.js_threshold:
                                self.results["resources"]["details"].append(f"JavaScript file {js_file} is too small: {js_size} bytes")
                                print(f"{STATUS_WARNING} JavaScript file {js_file} is too small: {js_size} bytes")
                                js_issues += 1
                            else:
                                self.results["resources"]["details"].append(f"JavaScript file {js_file} size: {js_size} bytes")
                                if self.verbose:
                                    print(f"{STATUS_OK} JavaScript file {js_file} size: {js_size} bytes")
                        else:
                            self.results["resources"]["details"].append(f"JavaScript file {js_file} returned HTTP {js_response.status_code}")
                            print(f"{STATUS_ERROR} JavaScript file {js_file} returned HTTP {js_response.status_code}")
                            js_issues += 1
                    
                    except requests.exceptions.RequestException as e:
                        self.results["resources"]["details"].append(f"Error accessing JavaScript file {js_file}: {str(e)}")
                        print(f"{STATUS_ERROR} Error accessing JavaScript file {js_file}: {str(e)}")
                        js_issues += 1
                
                if js_issues > 0:
                    self.results["resources"]["status"] = "Warning" if self.results["resources"]["status"] != "Error" else "Error"
                    ...
                    try:
                        ...
                        if css_response.status_code == 200:
                            ...
                            if css_size < self.css_threshold:
                                self.results["resources"]["details"].append(f"CSS file {css_file} is too small: {css_size} bytes")
                                print(f"{STATUS_WARNING} CSS file {css_file} is too small: {css_size} bytes")
                                css_issues += 1
                            else:
                                self.results["resources"]["details"].append(f"CSS file {css_file} size: {css_size} bytes")
                                if self.verbose:
                                    print(f"{STATUS_OK} CSS file {css_file} size: {css_size} bytes")
                        else:
                            self.results["resources"]["details"].append(f"CSS file {css_file} returned HTTP {css_response.status_code}")
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                if response.status_code == expected_status:
                    ...
                    print(f"{STATUS_WARNING} {method} {endpoint['path']}: Unexpected status {response.status_code} (expected {expected_status})")
                    api_issues += 1
    ...
    def _check_performance(self) -> None:
        ...
        try:
            ...
            self.results["performance"]["details"].append("Selenium not installed. Run: pip install selenium webdriver-manager")
    ...
    def _generate_text_report(self) -> str:
        ...
        lines.append("-" * 80)
        status_icon = STATUS_OK if self.results["summary"]["status"] == "OK" else \
                     STATUS_WARNING if self.results["summary"]["status"] == "Warning" else \
                     STATUS_ERROR if self.results["summary"]["status"] == "Error" else \
                     STATUS_INFO
        ...
        for key, result in self.results.items():
            ...
            lines.append("-" * 80)
            status_icon = STATUS_OK if result["status"] == "OK" else \
                         STATUS_WARNING if result["status"] == "Warning" else \
                         STATUS_ERROR if result["status"] == "Error" else \
                         STATUS_INFO
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--url", default=DEFAULT_URL,
                        help=f"Site URL to check (default: {DEFAULT_URL})")
    parser.add_argument("--api-url", default=DEFAULT_API_URL,
                        help=f"API URL to check (default: {DEFAULT_API_URL})")
    ...
    parser.add_argument("--mock-api", action="store_true",
                        help="Use mock API responses instead of real requests")
    parser.add_argument("--verbose", action="store_true",
                        help="Show detailed output")
    parser.add_argument("--ci", action="store_true",
                        help="CI mode (exit code based on critical checks only)")
    parser.add_argument("--threshold-js", type=int, default=DEFAULT_JS_THRESHOLD,
                        help=f"Minimum expected JS file size in bytes (default: {DEFAULT_JS_THRESHOLD})")
    parser.add_argument("--threshold-css", type=int, default=DEFAULT_CSS_THRESHOLD,
                        help=f"Minimum expected CSS file size in bytes (default: {DEFAULT_CSS_THRESHOLD})")
    parser.add_argument("--performance", action="store_true",
                        help="Run performance checks (requires Chrome)")
