  ...
  public async initialSync(): Promise<void> {
    try {
      ...
      vscode.window.showInformationMessage(
        "Starting initial file synchronization...",
      );
      ...
      vscode.window.withProgress(
        ...
        async (progress, token) => {
          ...
          vscode.window.showInformationMessage(
            `Initial sync complete. Synchronized ${syncedFiles} files.`,
          );
        },
      );
  ...
  private async syncFileToRemote(uri: vscode.Uri): Promise<void> {
    try {
      ...
      const remoteFilePath = path
        .join(this.remoteBasePath, relativePath)
        .replace(/\\/g, "/");
      ...
      await client.post(
        `https://workstations.googleapis.com/v1/projects/${this.authManager.getProjectId()}/locations/${this.authManager.getRegion()}/workstations/execute`,
        {
