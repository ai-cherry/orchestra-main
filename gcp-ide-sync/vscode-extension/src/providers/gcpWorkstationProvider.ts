...
class WorkstationTreeItem extends vscode.TreeItem {
  constructor(
    ...
    this.contextValue = state === 'RUNNING' ? 'workstation-running' : 'workstation-stopped';
    
    // Set icon based on state
    this.iconPath = state === 'RUNNING' 
 ...
 */
export class GcpWorkstationProvider implements vscode.TreeDataProvider<WorkstationTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<WorkstationTreeItem | undefined | null | void> = new vscode.EventEmitter<WorkstationTreeItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<WorkstationTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
  ...
  constructor(context: vscode.ExtensionContext, authManager: GcpAuthManager) {
    ...
    context.subscriptions.push(
      vscode.commands.registerCommand('gcp-ide-sync.refreshWorkstations', () => this.refresh())
    );
  }
   ...
   */
  async getChildren(element?: WorkstationTreeItem): Promise<WorkstationTreeItem[]> {
    if (element) {
