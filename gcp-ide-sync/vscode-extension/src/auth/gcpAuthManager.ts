    ...
    try {
      ...
      if (!keyPath) {
        ...
        if (!keyFile || keyFile.length === 0) {
          vscode.window.showErrorMessage('No service account key file selected');
          return false;
        }
        
        // Save the selected key path to settings
        await config.update('serviceAccountKeyPath', keyFile[0].fsPath, vscode.ConfigurationTarget.Global);
      ...
      vscode.window.showInformationMessage('Successfully authenticated with GCP');
      return true;
    } catch (error) {
