...
export async function activate(context: vscode.ExtensionContext) {
  ...
  vscode.window.registerTreeDataProvider('gcp-ide-sync-explorer', workstationProvider);
  ...
  const commands = [
    ...
    vscode.commands.registerCommand('gcp-ide-sync.disconnect', async () => {
      try {
        ...
        vscode.window.showInformationMessage('Disconnected from GCP Workstation');
      } catch (error) {
    ...
    vscode.commands.registerCommand('gcp-ide-sync.startWorkstation', async (workstationId: string) => {
      try {
        await workstationProvider.startWorkstation(workstationId);
        vscode.window.showInformationMessage(`Starting workstation ${workstationId}`);
      } catch (error) {
        vscode.window.showErrorMessage(`Failed to start workstation: ${error}`);
      }
    }),
    
    vscode.commands.registerCommand('gcp-ide-sync.stopWorkstation', async (workstationId: string) => {
      try {
        await workstationProvider.stopWorkstation(workstationId);
        vscode.window.showInformationMessage(`Stopping workstation ${workstationId}`);
      } catch (error) {
