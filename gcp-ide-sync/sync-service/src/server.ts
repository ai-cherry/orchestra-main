...
app.post("/sync", authenticate, async (req, res) => {
  try {
    ...
    if (operation === "delete") {
      ...
      res
        .status(200)
        .json({ success: true, message: "File synchronized successfully" });
    }
  } catch (error) {
...
app.post("/sync/initial", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Initial sync completed successfully" });
  } catch (error) {
    console.error("Initial sync error:", error);
    res
      .status(500)
      .json({ error: "Initial sync failed", details: error.message });
  }
});
      ...
      if (data.type === "auth") {
        ...
        if (data.action === "create") {
          ...
          terminal.on("close", (code: number) => {
            ws.send(
              JSON.stringify({ type: "terminal", action: "closed", code }),
            );
          });
          ...
          terminal = null;
          ws.send(
            JSON.stringify({ type: "terminal", action: "closed", code: 0 }),
          );
        }
      }
    } catch (error) {
