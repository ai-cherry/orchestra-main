...
class PhidataAgentWrapper(OrchestraAgentBase, AgentProtocol):
    ...
    def __init__(
        ...
        logger.info(
            f"PhidataAgentWrapper initialized: {self.name} (using {'Team' if self._is_team else 'Agent'} with Cloud SQL/PGVector storage)"
        )
    ...
    def _init_knowledge_base(self) -> None:
        ...
        try:
            ...
            knowledge_base_class_path = knowledge_config.get(
                "knowledge_base_class_path", "phi.knowledge.pdf.PDFUrlKnowledgeBase"
            )
    ...
    def _init_phidata_agent(self) -> None:
        ...
        try:
            ...
            if self._is_team:
                ...
                if hasattr(self, "response_model") and self.response_model is not None:
                    ...
                    if "structured" in self.phidata_agent_class.lower():
                        logger.info(
                            f"Initializing structured output agent with response model: {self.response_model.__name__}"
                        )
            ...
            logger.info(
                f"Successfully initialized Phidata {'Team' if self._is_team else 'Agent'} with CloudSQL storage"
            )
