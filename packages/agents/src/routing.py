...
class AgentRouting:
        ...
        try:
            ...
            if self.matching_engine_endpoint:
                ...
                for agent_id, capabilities in self.agent_capabilities.items():
                    ...
                    query_vector = await self._generate_agent_performance_vector(agent_id)
                    ...
                    if response and response.matches:
                        ...
                        self.agent_scores[agent_id] = (0.7 * current_score) + (0.3 * updated_score)
                ...
                for agent_id, capabilities in self.agent_capabilities.items():
                    ...
                    capability_weight = len(capabilities) / 10.0  # Scale based on number of capabilities
                    success_rate = self.agent_success_rates.get(agent_id, 0.8)  # Default to 80% if unknown
    ...
    async def _generate_agent_performance_vector(self, agent_id: str) -> List[float]:
        ...
        for i in range(128):
            ...
            if i % 10 == 0:
                value = success_rate + (random.random() * 0.2 - 0.1)  # Add some randomness
            else:
                value = random.random()
            vector.append(value)
        ...
        try:
            if production_agent_id in self.shadow_agents:
                ...
                if self.use_async_db and self.async_spanner_client:
                    ...
                    await loop.run_in_executor(
                        None, 
                        self._sync_remove_shadow_agent, 
                        production_agent_id
                    )
                else:
    ...
    async def register_agent(
        ...
        try:
            ...
            logger.info(f"Registered new agent {agent_id} in routing system with {len(capabilities or [])} capabilities")
            return True
