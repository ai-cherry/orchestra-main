...
def get_deal_details(deal_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        deal = client.crm.deals.basic_api.get_by_id(
            deal_id=deal_id, properties=["*"], archived=False
        )
        ...
        associated_companies = client.crm.deals.associations_api.get_all(
            deal_id=deal_id, to_object_type="companies"
        )
        ...
        return {
            ...
            "associated_contacts": [r.to_dict() for r in associated_contacts.results]
            if associated_contacts.results
            else [],
            "associated_companies": [r.to_dict() for r in associated_companies.results]
...
def update_deal_stage(deal_id: str, stage: str) -> Dict[str, Any]:
    ...
    try:
        ...
        current_deal = client.crm.deals.basic_api.get_by_id(
            deal_id=deal_id, properties=["dealstage"], archived=False
        )
        ...
        client.crm.deals.basic_api.update(
            deal_id=deal_id, simple_public_object_input={"properties": properties}
        )
...
def enrich_contact(contact_id: str, enrichment_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        contact = client.crm.contacts.basic_api.get_by_id(
            contact_id=contact_id, properties=["*"], archived=False
        )
...
class HubSpotTool(OrchestraTool):
    ...
    def enrich_contact(
        self, contact_id: str, enrichment_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Enrich a HubSpot contact with additional data.
