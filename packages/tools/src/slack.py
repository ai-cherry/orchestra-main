...
def send_slack_message(
    ...
    try:
        ...
        msg_params = {"channel": channel, "text": text}
...
def format_slack_message(
    title: str, sections: List[Dict[str, Any]], include_divider: bool = True
) -> List[Dict[str, Any]]:
    """
    ...
    for section in sections:
        ...
        if "fields" in section and section["fields"]:
            section_block["fields"] = [
                ...
                if "title" in field
                else {"type": "mrkdwn", "text": field["text"]}
...
def send_data_visualization(
    ...
    try:
        response = client.chat_postMessage(
            channel=channel, text=f"{title}: {report_text}", blocks=blocks
        )
...
class SlackMessenger(OrchestraTool):
    ...
    def format_message(
        self, title: str, sections: List[Dict[str, Any]], include_divider: bool = True
    ) -> List[Dict[str, Any]]:
        """
        Format a structured message using Slack Block Kit components.
