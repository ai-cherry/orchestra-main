...
class ToolRegistry:
    ...
    def register_tool_class(self, tool_type: str, tool_class: Type[OrchestraTool]) -> None:
        """
    ...
    def create_tool(self, tool_id: str, tool_config: Dict[str, Any]) -> Optional[OrchestraTool]:
        """
        ...
        if tool_type in self._tool_classes:
            ...
            try:
                ...
                if not issubclass(tool_class, OrchestraTool):
                    logger.error(f"Class {tool_type} is not a subclass of OrchestraTool")
                    return None
        ...
        try:
            ...
            logger.error(f"Failed to create tool: {tool_id} of type: {tool_type}: {e}", exc_info=True)
            return None
