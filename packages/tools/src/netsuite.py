...
def _check_netsuite_auth() -> None:
    ...
    if missing_vars:
        raise ValueError(f"Missing required NetSuite credentials: {', '.join(missing_vars)}")
...
def _generate_netsuite_signature(url: str, http_method: str) -> Dict[str, str]:
    ...
    signature_base = f"{http_method.upper()}&{quote_plus(url)}&{quote_plus(param_string)}"
...
def get_inventory_levels(sku: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not inventory_data or 'items' not in inventory_data or not inventory_data['items']:
...
@tool
def update_inventory_record(item_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
    """
...
class NetSuiteTool(OrchestraTool):
    ...
    def update_inventory_record(self, item_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Update inventory record fields in NetSuite.
