...
def _check_looker_auth() -> None:
    ...
    required_env_vars = ["LOOKER_API_URL", "LOOKER_CLIENT_ID", "LOOKER_CLIENT_SECRET"]

    missing_vars = [var for var in required_env_vars if not os.environ.get(var)]
    if missing_vars:
        raise ValueError(
            f"Missing required Looker credentials: {', '.join(missing_vars)}"
        )
...
def run_look(look_id: str, result_format: str = "json") -> Dict[str, Any]:
    ...
    try:
        ...
        if result_format.lower() == "json":
            data = response.json()
            return {"look_id": look_id, "data": data}
...
@tool
def run_query(
    query_body: Dict[str, Any], result_format: str = "json"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if result_format.lower() == "json":
            data = response.json()
            return {"query": query_body, "data": data}
        # For other formats, return metadata and raw content
        else:
            return {
                "query": query_body,
...
def list_dashboards(folder_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"dashboards": formatted_dashboards, "count": len(formatted_dashboards)}
...
class LookerTool(OrchestraTool):
    ...
    def run_query(
        self, query_body: Dict[str, Any], result_format: str = "json"
    ) -> Dict[str, Any]:
        """
        Run a Looker query and retrieve the results.
