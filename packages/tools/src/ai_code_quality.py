...
class HallucinationDetector:
    ...
    def __init__(self):
        self.patterns = [
            (r"#.*(AI-generated|generated by AI|auto-generated by AI)", "AI-generated comment detected"),
            (r"def\s+\w+\s*\(\s*\)\s*:\s*\n\s*pass", "Empty function definition, possible placeholder"),
            (r"for\s+\w+\s+in\s+range\(\d+\)\s*:\s*\n\s*pass", "Empty loop, possible placeholder"),
        ]
...
class PromptInjectionChecker:
    ...
    def __init__(self):
        self.vulnerable_patterns = [
            (r"format\s*\(\s*[^)]*user_input[^)]*\)", "Direct string formatting with user input"),
            (r"\+\s*user_input\s*\+", "String concatenation with user input"),
            (r"prompt\s*=.*\{\}.*format\s*\(", "Unescaped user input in prompt formatting"),
        ]
...
class RAGContextValidator:
    ...
    def __init__(self):
        self.context_patterns = [
            (r"context\s*=.*list|dict|str", "Context variable assignment without token limit check"),
            (r"messages\s*=.*append\s*\(\s*context", "Context added to messages without truncation"),
        ]
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for i, line in enumerate(lines, 1):
                    for model, suggestion in self.outdated_models.items():
                        if model in line:
                            issues.append((i, line.strip(), f"Outdated model '{model}' detected. {suggestion}"))
        except Exception as e:
            print(f"Error reading file {file_path}: {e}")
        return issues
