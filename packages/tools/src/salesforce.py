...
def _get_salesforce_client() -> Salesforce:
    ...
    if missing_vars:
        raise ValueError(
            f"Missing required Salesforce credentials: {', '.join(missing_vars)}"
        )
...
@tool
def get_salesforce_record(object_name: str, record_id: str) -> Dict[str, Any]:
    """
...
@tool
def create_salesforce_record(object_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
    """
