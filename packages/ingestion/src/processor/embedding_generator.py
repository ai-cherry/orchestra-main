...
class EmbeddingGenerator:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._client = aiplatform.PredictionServiceClient(
                client_options=client_options
            )
        ...
        try:
            ...
            for batch in batches:
                ...
                response = self._client.predict(
                    endpoint=model_name, instances=instances
                )

                # Extract the embedding vectors from the response
                batch_embeddings = [
                    pred["embeddings"]["values"] for pred in response.predictions
                ]
            ...
            logger.debug(
                f"Generated {len(all_embeddings)} embeddings in {len(batches)} batches"
            )
            ...
            raise EmbeddingGenerationError(
                f"Failed to generate embeddings in batch: {e}"
            )
    ...
    async def get_embedding_dimension(self) -> int:
        ...
        try:
            ...
            sample_embedding = await self.generate_embedding(
                "Sample text for dimension check"
            )
