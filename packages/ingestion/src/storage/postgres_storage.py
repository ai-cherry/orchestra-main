...
class PostgresStorage:
        ...
        try:
            if not self.dsn:
                raise PostgresStorageError(
                    "No DSN or connection details provided for PostgreSQL"
                )
    ...
    async def store_embedding(self, chunk: TextChunk) -> int:
        ...
        try:
            ...
            logger.debug(
                f"Stored embedding for chunk {chunk.file_id}/{chunk.chunk_number} with ID {embedding_id}"
            )
            return embedding_id
        except Exception as e:
