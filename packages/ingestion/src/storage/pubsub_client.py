...
class PubSubClient:
    ...
    async def publish_task(self, message: PubSubMessage) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Published message to {self.topic_path} with ID: {message_id}"
            )
            return message_id
        except Exception as e:
    ...
    async def pull_messages(self, max_messages: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(
                f"Pulled {len(messages)} messages from {self.subscription_path}"
            )
            return messages
        except Exception as e:
            ...
            for message in messages:
                try:
                    ...
                    if success:
                        ...
                        logger.warning(
                            f"Message {message['message_id']} not processed successfully, will be retried"
                        )
                except Exception as e:
                    logger.error(
