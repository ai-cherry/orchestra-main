...
class GCSStorage:
        ...
        try:
            ...
            try:
                ...
                self._raw_bucket = self._client.create_bucket(
                    self.raw_bucket_name, location="us-central1"
                )

            # Ensure processed text bucket exists
            try:
                self._processed_text_bucket = self._client.get_bucket(
                    self.processed_text_bucket_name
                )
            except NotFound:
                logger.info(
                    f"Creating processed text bucket: {self.processed_text_bucket_name}"
                )
                self._processed_text_bucket = self._client.create_bucket(
                    self.processed_text_bucket_name, location="us-central1"
                )
    ...
    async def download_from_url(
        self, url: str, task_id: str, max_size_mb: int = 500
    ) -> Tuple[str, str]:
        """
        ...
        try:
            ...
            logger.info(
                f"Downloaded file from {url} to {temp_path} ({total_size / 1024 / 1024:.1f} MB)"
            )
    ...
    async def upload_raw_file(
        self, file_path: str, task_id: str, filename: Optional[str] = None
    ) -> str:
        """
    ...
    async def upload_text_content(
        self, text_content: str, task_id: str, filename: str
    ) -> str:
        """
    ...
    async def get_download_url(self, gcs_uri: str, expiration_minutes: int = 60) -> str:
        """
        ...
        try:
            ...
            logger.debug(
                f"Generated signed URL for {gcs_uri}, expires in {expiration_minutes} minutes"
            )
            return url
        except Exception as e:
    ...
    async def list_task_files(
        self, task_id: str, bucket_name: Optional[str] = None
    ) -> List[str]:
        """
