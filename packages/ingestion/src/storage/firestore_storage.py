...
class FirestoreStorage:
    ...
    async def create_task(self, task: IngestionTask) -> str:
        ...
        try:
            ...
            # Store in Firestore
            await self._client.collection(self.tasks_collection).document(task.id).set(task_data)
    ...
    async def update_task_status(
        ...
        try:
            ...
            # Update the document
            await self._client.collection(self.tasks_collection).document(task_id).update(update_data)
    ...
    async def get_task(self, task_id: str) -> Optional[IngestionTask]:
        ...
        try:
            ...
            doc = await self._client.collection(self.tasks_collection).document(task_id).get()
    ...
    async def create_file(self, file: ProcessedFile) -> str:
        ...
        try:
            ...
            # Store in Firestore
            await self._client.collection(self.files_collection).document(file.id).set(file_data)
            
            logger.debug(f"Created processed file record {file.id} for task {file.task_id}")
            return file.id
        except Exception as e:
    ...
    async def update_file(
        ...
        try:
            ...
            # Update the document
            await self._client.collection(self.files_collection).document(file_id).update(updates)
    ...
    async def get_file(self, file_id: str) -> Optional[ProcessedFile]:
        ...
        try:
            # Get document from Firestore
            doc = await self._client.collection(self.files_collection).document(file_id).get()
    ...
    async def list_files_for_task(self, task_id: str) -> List[ProcessedFile]:
        ...
        try:
            ...
            query = self._client.collection(self.files_collection).where("task_id", "==", task_id)
