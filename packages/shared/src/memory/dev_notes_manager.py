...
from packages.shared.src.storage.config import AGENT_DATA_COLLECTION, DEV_NOTES_COLLECTION, StorageConfig
...
class DevNotesManager:
    ...
    async def add_note(
        ...
        meta["note_type"] = note_type.value if isinstance(note_type, DevNoteType) else note_type
    ...
    async def get_component_notes(
        ...
        if note_type:
            type_value = note_type.value if isinstance(note_type, DevNoteType) else note_type
