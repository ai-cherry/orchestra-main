...
from packages.shared.src.memory.integrations.agno_memory_adapter import (
    AgnoMemoryAdapter,
)
...
class TieredMemoryManager(MemoryManager):
    ...
    async def search_memories(
        ...
        if not self._initialized or not self.long_term_provider:
            logger.warning(
                "Semantic search unavailable: tiered memory not initialized or long-term provider missing"
            )
            return []
    ...
    def _merge_memories(
        self, existing: List[MemoryItem], additional: List[MemoryItem], limit: int
    ) -> List[MemoryItem]:
        """
