...
class FirestoreMemoryManager(MemoryManager):
    ...
    def __init__(self, settings: Settings = Depends(get_settings), namespace: Optional[str] = None):
        """
    ...
    async def add_memory(self, memory: MemoryRecord) -> str:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: self._save_record_sync(memory, collection_name)
            )
    ...
    def _save_record_sync(self, record: MemoryRecord, collection: str) -> None:
        ...
        if hasattr(self.settings, "FIRESTORE_TTL_DAYS") and self.settings.FIRESTORE_TTL_DAYS > 0:
            expiry_date = datetime.now() + timedelta(days=self.settings.FIRESTORE_TTL_DAYS)
    ...
    async def get_memory(self, id: str) -> Optional[MemoryRecord]:
        ...
        try:
            ...
            record = await loop.run_in_executor(
                None,
                lambda: self._get_record_sync(id, collection_name)
            )
            return record
        except ValueError:
            # Record not found
    ...
    async def search_memories(self, query: Dict[str, Any], limit: int = 10) -> List[MemoryRecord]:
        """
        ...
        try:
            ...
            records = await loop.run_in_executor(
                None,
                lambda: self._query_records_sync(query, collection_name, limit)
            )
            return records
        except Exception as e:
    ...
    def _query_records_sync(self, filters: dict, collection: str, limit: int = 10) -> List[MemoryRecord]:
    ...
    async def get_conversation_history(self, user_id: str, limit: int = 20) -> List[MemoryRecord]:
        """
    ...
    async def clear_user_memories(self, user_id: str) -> int:
        ...
        try:
            ...
            count = await loop.run_in_executor(
                None,
                lambda: self._clear_user_memories_sync(user_id, collection_name)
            )
            return count
        except Exception as e:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await loop.run_in_executor(
                None,
                lambda: self._client.collection("__healthcheck__").document("test").get()
            )
