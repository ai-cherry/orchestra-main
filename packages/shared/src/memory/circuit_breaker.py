...
class CircuitBreaker:
    ...
    async def _check_state_transition(self) -> None:
        ...
        if self._state == CircuitState.OPEN:
            ...
            # Check if we should reset the failure count
                ...
                and self._last_success_time > self._last_failure_time
                and now - self._last_success_time >= self.reset_timeout
            ):
    ...
    async def _record_failure(self, error: Exception) -> None:
        ...
        async with self._lock:
            ...
            if any(
                isinstance(error, exc_type) for exc_type in self.excluded_exceptions
            ):
                logger.debug(
                    f"Excluded exception {type(error).__name__} for circuit {self.name}"
                )
                return
