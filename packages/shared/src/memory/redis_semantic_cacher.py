...
class RedisSemanticCacheProvider(MemoryProvider):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.cacher = SemanticCacher(
                threshold=self.config.get("threshold", 0.82),  # Similarity threshold adjusted for increased recall tolerance
                ttl=self.config.get("ttl", 3600),              # Time-to-live in seconds
                index_schema=self.config.get("index_schema", "agent_memory.yaml"),
                eviction_policy=self.config.get("eviction_policy", "LRU+TTL")  # Implement LRU alongside TTL
            )
    ...
    async def add_memory(self, item: MemoryItem) -> str:
        ...
        try:
            ...
            latency = (datetime.utcnow().timestamp() - start_time) * 1000  # Convert to milliseconds
            logger.debug(f"Added memory item to Redis cache with ID: {cache_id}, Latency: {latency:.2f}ms")
    ...
    async def get_memories(
        ...
        try:
            ...
            if query:
                ...
                start_time = datetime.utcnow().timestamp()
                results = await asyncio.to_thread(
                    self.cacher.get_recent,
                    filters=filters,
                    limit=limit
                )
                latency = (datetime.utcnow().timestamp() - start_time) * 1000  # Convert to milliseconds
                logger.debug(f"Recent items retrieval completed, Latency: {latency:.2f}ms")
            ...
            logger.error(f"Error retrieving memories from Redis cache: {e}", exc_info=True)
            return []
