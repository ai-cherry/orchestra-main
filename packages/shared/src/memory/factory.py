...
class MemoryManagerFactory:
    ...
    async def create_memory_manager(
        ...
        if config.backend == MemoryBackendType.FIRESTORE:
            return await MemoryManagerFactory._create_firestore_memory_manager(config, log_level)
        elif config.backend == MemoryBackendType.REDIS:
            return await MemoryManagerFactory._create_redis_memory_manager(config, log_level)
        elif config.backend == MemoryBackendType.IN_MEMORY:
            return await MemoryManagerFactory._create_in_memory_manager(config, log_level)
        else:
        ...
        if vector_search_config:
            ...
            if vector_search_provider == VectorSearchType.VERTEX.value:
                if not vector_search_config.vertex:
                    raise ValueError("Vertex AI Vector Search configuration is required")
    ...
    async def _create_in_memory_manager(
        ...
        try:
            ...
            raise ImportError(
                "Memory manager dependencies not available."
            )
    ...
    def detect_capabilities() -> Dict[str, bool]:
        ...
        capabilities = {
            ...
            "redis": False,
            "vector_search": {
                "in_memory": True,  # Always available
                "vertex": False
            }
        }
    ...
    def get_available_vector_search_providers() -> List[VectorSearchType]:
        ...
        """
        capabilities = MemoryManagerFactory.detect_capabilities()
        available_providers = [VectorSearchType.IN_MEMORY, VectorSearchType.NONE]  # Always available
