...
class ContextPruner:
    ...
    async def prune_conversation_history(
        ...
        summary_item = MemoryItem(
            ...
            metadata={
                ...
                "timestamp_range": [
                    summarizable_items[0].timestamp.isoformat()
                    if summarizable_items[0].timestamp
                    else "",
                    summarizable_items[-1].timestamp.isoformat()
                    if summarizable_items[-1].timestamp
                    else "",
                ],
            },
    ...
    async def prune_by_token_budget(
        ...
        if important_tokens + recent_tokens > token_budget:
            ...
            for item, tokens in recent_bucket:
                if (
                    current_tokens + tokens <= token_budget * 0.8
            ...
            if to_summarize:
                ...
                summary_item = MemoryItem(
                    ...
                    metadata={
                        ...
                        "timestamp_range": [
                            to_summarize[0].timestamp.isoformat()
                            if to_summarize[0].timestamp
                            else "",
                            to_summarize[-1].timestamp.isoformat()
                            if to_summarize[-1].timestamp
                            else "",
                        ],
                    },
                    timestamp=datetime.now(),
            ...
            for item, tokens in older_bucket:
                if (
                    current_tokens + tokens <= remaining_budget * 0.8
                ):  # Reserve 20% for summary
            ...
            result = (
                [item for item, _ in important_bucket]
                + [item for item, _ in recent_bucket]
                + kept_older
            )

            # Generate summary for remaining older items if needed
            if (
                older_to_summarize and remaining_budget - current_tokens >= 100
            ):  # Only if we have at least 100 tokens left
                ...
                summary_item = MemoryItem(
                    ...
                    metadata={
                        ...
                        "token_budget": token_budget,
                        "timestamp_range": [
                            older_to_summarize[0].timestamp.isoformat()
                            if older_to_summarize[0].timestamp
                            else "",
                            older_to_summarize[-1].timestamp.isoformat()
                            if older_to_summarize[-1].timestamp
                            else "",
                        ],
                    },
                    timestamp=datetime.now(),
    ...
    async def _generate_summary(
        ...
        try:
            ...
            logger.info(
                f"Generated summary for {len(items)} items, {len(summary)} chars"
            )
