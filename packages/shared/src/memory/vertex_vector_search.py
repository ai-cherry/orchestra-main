...
class VertexVectorSearch:
    ...
    async def initialize(self) -> None:
        ...
        if not self.index_endpoint_id or not self.index_id:
            logger.warning(
                "Vertex Vector Search index_endpoint_id or index_id not provided"
            )
            self._fallback_to_custom = True
            return
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Vertex Vector Search initialized for project {self.project_id}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to validate Vertex Vector Search index endpoint: {e}"
                )
                self._fallback_to_custom = True
    ...
    def _check_initialized(self) -> None:
        ...
        if not self._initialized and not self._fallback_to_custom:
            raise RuntimeError(
                "Vertex Vector Search not initialized. Call initialize() first."
            )
    ...
    async def index_memory_item(self, item: MemoryItem) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to index memory item {item.id} in Vertex Vector Search: {e}"
            )
            if self.monitoring:
    ...
    async def search(
        ...
        if not self.enabled or self._fallback_to_custom:
            logger.warning(
                "Vertex Vector Search not available, falling back to custom implementation"
            )
            return []
        ...
        try:
            ...
            logger.error(
                f"Failed to perform semantic search in Vertex Vector Search: {e}"
            )
            if self.monitoring:
    ...
    async def delete_memory_item(self, item: MemoryItem) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to delete memory item {item.id} from Vertex Vector Search: {e}"
            )
            if self.monitoring:
