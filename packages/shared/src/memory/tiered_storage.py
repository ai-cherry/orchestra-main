...
class TieredStorageManager(MemoryInterface):
    ...
    def __init__(
        ...
        self._hot_cache_manager = get_cache_manager(
            ...
            default_ttl_seconds=int(
                os.environ.get("HOT_TIER_TTL", str(60 * 60 * 24))
            ),  # 24 hours default
    ...
    def _initialize_clients(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully initialized tiered storage clients for project {self._project_id}"
            )
    ...
    async def get_memory_item(self, item_id: str) -> Optional[MemoryItem]:
        ...
        try:
            ...
            if not item:
                warm_doc_ref = self._firestore_client.collection("warm_tier").document(
                    item_id
                )
            ...
            if not item:
                cold_doc_ref = self._firestore_client.collection("cold_tier").document(
                    item_id
                )
    ...
    async def add_raw_agent_data(self, data: AgentData) -> str:
        ...
        try:
            ...
            data_dict = data.dict()
            warm_doc_ref = self._firestore_client.collection(
                "warm_tier_agent_data"
            ).document(data_id)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        tier_health = {
            ...
            "details": {
                ...
                "last_migration": datetime.fromtimestamp(
                    self._last_tier_migration
                ).isoformat(),
        ...
        try:
            ...
            test_doc = self._firestore_client.collection("health_check").document(
                "test"
            )
    ...
    async def _store_in_tier(
        self, item_id: str, item: MemoryItem, tier: StorageTier
    ) -> None:
        """
            ...
            if self._redis_client:
                try:
                    ...
                    self._redis_client.setex(
                        redis_key, 60 * 60 * 24, json.dumps(item_dict)  # 24 hour TTL
                    )
                except Exception as e:
            ...
            try:
                if self._enable_compression and len(item.content) > 500:
                    try:
                        ...
                        cold_doc_ref = self._firestore_client.collection(
                            "cold_tier"
                        ).document(item_id)
    ...
    async def _remove_from_tier(self, item_id: str, tier: StorageTier) -> None:
        """
        ...
        try:
            ...
            raise MemoryOperationError(
                f"Failed to remove item from {tier.value} tier: {e}", e
            )

    async def _migrate_item(
        self, item_id: str, from_tier: StorageTier, to_tier: StorageTier
    ) -> None:
        """
        ...
        """
        logger.debug(
            f"Migrating item {item_id} from {from_tier.value} to {to_tier.value}"
        )
        ...
        if from_tier == StorageTier.HOT:
            ...
            if cold_doc.exists:
                ...
                if (
                    not item
                ):  # Only create if not already created in decompression error handling
                    item = MemoryItem(**item_dict)
        ...
        if not item:
            ...
            if not item:
                logger.warning(
                    f"Item {item_id} not found in {from_tier.value} tier or base storage"
                )
                return
    ...
    async def _ensure_item_in_tier(self, item_id: str, item: MemoryItem) -> None:
        ...
        if (
            ...
            and (current_time - last_access) <= self._hot_tier_max_age
        ):
            # Item should be in hot tier
            if not self._hot_cache_manager.contains(item_id) and (
                self._redis_client is None
                or not self._redis_client.exists(f"hot_tier:{item_id}")
            ):
                await self._store_in_tier(item_id, item, StorageTier.HOT)

        elif (
            access_count >= self._min_access_count_warm
            and (current_time - last_access) <= self._warm_tier_max_age
        ):
    ...
    async def _run_tier_migration(self, force: bool = False) -> None:
        ...
        for item_id in hot_tier_items:
            ...
            if (
                current_time - last_access > self._hot_tier_max_age
                or access_count < self._min_access_count_hot
            ):
                try:
                    # Get the item data
                    item_dict = self._hot_cache_manager.get(item_id)
                    if item_dict:
                        item = MemoryItem(**item_dict)
                        await self._migrate_item(
                            item_id, StorageTier.HOT, StorageTier.WARM
                        )
                    else:
        ...
        try:
            ...
            for doc in warm_docs:
                ...
                last_access = self._last_access_time.get(
                    item_id, datetime.fromtimestamp(0)
                )
                access_count = self._access_counts.get(item_id, 0)
                ...
                if access_count >= self._min_access_count_hot:
                    ...
                    try:
                        ...
                        tasks.append(
                            self._migrate_item(
                                item_id, StorageTier.WARM, StorageTier.HOT
                            )
                        )
                    except Exception as e:
                        logger.warning(
                            f"Error promoting item {item_id} to hot tier: {e}"
                        )
                elif (
                    current_time - last_access > self._warm_tier_max_age
                    or access_count < self._min_access_count_warm
                ):
                    ...
                    try:
                        ...
                        tasks.append(
                            self._migrate_item(
                                item_id, StorageTier.WARM, StorageTier.COLD
                            )
                        )
                    except Exception as e:
                        logger.warning(
                            f"Error demoting item {item_id} to cold tier: {e}"
                        )
    ...
    async def _load_access_tracking(self) -> None:
        ...
        try:
            # Load access counts
            access_counts_doc = self._firestore_client.collection(
                "tiered_storage_meta"
            ).document("access_counts")
            if access_counts_doc.get().exists:
                self._access_counts = access_counts_doc.get().to_dict() or {}

            # Load last access times
            access_times_doc = self._firestore_client.collection(
                "tiered_storage_meta"
            ).document("access_times")
            if access_times_doc.get().exists:
                ...
                for item_id, timestamp_str in access_times_dict.items():
                    try:
                        self._last_access_time[item_id] = datetime.fromisoformat(
                            timestamp_str
                        )
            ...
            logger.info(
                f"Loaded access tracking data for {len(self._access_counts)} items"
            )
    ...
    async def _save_access_tracking(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Saved access tracking data for {len(self._access_counts)} items"
            )
