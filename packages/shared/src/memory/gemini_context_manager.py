...
class GeminiContextManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(f"GeminiContextManager initialized with {self.max_tokens} max tokens")
    ...
    async def add_to_context(self, memory_item: MemoryItem, priority: float = 0.5) -> bool:
        """
        ...
        try:
            ...
            if self.current_tokens + token_count > self.max_tokens:
                ...
                if freed_tokens < token_count:
                    logger.warning(f"Could not free enough tokens for new context item: needed {token_count}, freed {freed_tokens}")
            ...
            logger.debug(f"Added item to context: {token_count} tokens, {self.current_tokens}/{self.max_tokens} total")
            return True
        except Exception as e:
            logger.error(f"Error adding item to context: {e}")
            return False
    
    async def get_relevant_context(self, query: str, user_id: Optional[str] = None, 
                                 session_id: Optional[str] = None, limit: int = 10) -> List[MemoryItem]:
        """
        ...
        try:
            ...
            if user_id:
                filtered_items = [entry for entry in filtered_items 
    ...
    async def clear_context(self, user_id: Optional[str] = None, 
        ...
        try:
            if user_id or session_id:
                ...
                logger.info(f"Cleared {cleared_tokens} tokens from context for user_id={user_id}, session_id={session_id}")
            else:
                # Clear all context
                self.context_items = []
                self.current_tokens = 0
    ...
    async def _prune_context(self, required_tokens: int) -> int:
        ...
        for item in self.context_items:
            ...
            if tokens_freed < required_tokens and item["priority"] >= self.priority_threshold:
                continue
