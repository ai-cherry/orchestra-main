...
class MemoryProfiler:
    ...
    def _create_metric_descriptor(
        ...
        descriptor.metric_kind = getattr(monitoring_v3.types.MetricDescriptor.MetricKind, metric_kind)
        descriptor.value_type = getattr(monitoring_v3.types.MetricDescriptor.ValueType, value_type)
    ...
    async def collect_metrics(self) -> Dict[str, Any]:
        ...
        if self._redis_cache:
            try:
                ...
                metrics.update({
                    ...
                    "redis_memory_usage_pct": cache_metrics.get("memory_usage_pct", 0) * 100,
        ...
        try:
            if os.path.exists("/proc/meminfo"):
                ...
                mem_usage_pct = (mem_used_mb / mem_total_mb) * 100 if mem_total_mb > 0 else 0
    ...
    def _check_memory_pressure(self) -> bool:
        ...
        if "redis_memory_usage_pct" in self._last_metrics:
            ...
            if usage_pct > threshold:
                ...
                alert = f"Redis memory usage at {usage_pct:.1f}% (threshold: {threshold}%)"
                alerts.append(alert)
        ...
        if "system_mem_usage_pct" in self._last_metrics:
            ...
            if usage_pct > threshold:
                pressure_detected = True
                alert = f"System memory usage at {usage_pct:.1f}% (threshold: {threshold}%)"
                alerts.append(alert)
                logger.warning(alert)
