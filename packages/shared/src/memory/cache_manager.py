...
class CacheManager(Generic[T]):
    ...
    def _cleanup(self) -> int:
        ...
        if to_remove:
            logger.debug(f"Cleaned up {len(to_remove)} expired entries from cache '{self.name}'")
    ...
    def _evict_lru(self, count: int) -> None:
        ...
        if count > 0:
            logger.debug(f"Evicted {min(count, len(entries))} entries using LRU policy from cache '{self.name}'")
    ...
    def _evict_lfu(self, count: int) -> None:
        ...
        if count > 0:
            logger.debug(f"Evicted {min(count, len(entries))} entries using LFU policy from cache '{self.name}'")
    ...
    def _evict_ttl(self, count: int) -> None:
        ...
        if count > 0:
            logger.debug(f"Evicted {min(count, len(entries))} entries using TTL policy from cache '{self.name}'")
...
class ExternalServiceIntegration:
    ...
    def use_service(self, service_name: str, payload: Dict[str, Any]) -> Any:
        ...
        if not self.is_service_available(service_name):
            raise ValueError(f"Service {service_name} is not available or API key is missing.")
