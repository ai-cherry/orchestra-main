...
class RedisCache:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Redis client initialized successfully: {self.settings.REDIS_HOST}"
                )
            else:
    ...
    def _get_redis_password(self) -> Optional[str]:
        ...
        try:
            ...
            if not project_id:
                logger.warning(
                    "No GCP project ID configured, cannot access Redis auth secret"
                )
                return None
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            success = await loop.run_in_executor(
                None, lambda: self.redis_client.set(key, value, ex=ttl)
            )
            return bool(success)
        except Exception as e:
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            count = await loop.run_in_executor(
                None, lambda: self.redis_client.delete(key)
            )
            return count > 0
        except Exception as e:
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            ...
            exists = await loop.run_in_executor(
                None, lambda: self.redis_client.exists(key)
            )
            return bool(exists)
        except Exception as e:
