...
class MemoryItem(BaseModel):
    ...
    priority: float = Field(default=0.5,
                          description="Priority of the memory (0.0-1.0)")
...
class PersonaTraits(BaseModel):
    ...
    resilience: int = Field(default=50, ge=0, le=100, 
                           description="Resilience to challenges (0-100)")
    detail_orientation: int = Field(default=50, ge=0, le=100, 
                                   description="Attention to detail (0-100)")
    social_awareness: int = Field(default=50, ge=0, le=100, 
                                 description="Awareness of social dynamics (0-100)")
    technical_depth: int = Field(default=50, ge=0, le=100, 
                               description="Depth of technical knowledge (0-100)")
    leadership: int = Field(default=50, ge=0, le=100, 
                           description="Leadership capabilities (0-100)")
    analytical_thinking: int = Field(default=50, ge=0, le=100, 
                                    description="Analytical thinking ability (0-100)")
...
class PersonaConfig(BaseModel):
    ...
    traits: PersonaTraits = Field(default_factory=PersonaTraits,
                                description="Traits that define this persona")
    system_prompt: str = Field(..., description="System prompt to use for this persona")
    metadata: Dict[str, Any] = Field(default_factory=dict,
                                   description="Additional metadata for this persona")
...
class AgentConfig(BaseModel):
    ...
    persona_id: Optional[str] = Field(default=None, 
                                    description="ID of the persona to use (if any)")
