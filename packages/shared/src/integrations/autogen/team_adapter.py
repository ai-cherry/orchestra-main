...
class AutoGenTeamAdapter:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                import autogen
                from autogen import Agent, ConversableAgent, AssistantAgent, UserProxyAgent, GroupChat, GroupChatManager
                ...
                logger.warning("AutoGen library not available. Install with: pip install pyautogen")
                return False
    ...
    async def _create_autogen_agents(self) -> None:
        ...
        for idx, agent_config in enumerate(agent_configs):
            ...
            if agent_type == "assistant":
                ...
                logger.warning(f"Unknown agent type: {agent_type}, creating default assistant agent")
                agent = await self._create_assistant_agent(agent_config)
    ...
    async def _create_tool_function(self, tool_config: Dict[str, Any]) -> Optional[Callable]:
    ...
    def _setup_group_chat(self) -> None:
        ...
        if self.team_config.get("use_manager", True):
            llm_config = self._get_llm_config(self.team_config.get("manager_config", {}))
    ...
    async def _run_group_chat(self, prompt: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "content": str(e),
                "messages": self.group_chat.messages if hasattr(self.group_chat, "messages") else [],
                "elapsed_seconds": elapsed,
                "success": False,
                "error": str(e)
            }
