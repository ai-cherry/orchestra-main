...
class LangChainMemoryAdapter(MemoryProvider):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                from langchain.memory import ConversationBufferMemory, EntityMemory, SummaryMemory
                ...
                logger.warning("LangChain library not available. Install with: pip install langchain langchain-community")
                return False
            ...
            self.embeddings = self.VertexAIEmbeddings(
                model_name=self.config.get("embedding_model", "textembedding-gecko@latest")
            )
    ...
    async def _add_to_entity_memory(self, item: MemoryItem, input_text: str, output_text: str) -> None:
    ...
    async def _add_to_summary_memory(self, item: MemoryItem, input_text: str, output_text: str) -> None:
    ...
    async def get_memories(self, 
        ...
        try:
            if query and self.vectorstore:
                ...
                conversation_memories = await self._get_conversation_memories(user_id, session_id, limit)
    ...
    async def _semantic_search(self, query: str, user_id: str, 
                             session_id: Optional[str] = None, 
                             limit: int = 10) -> List[MemoryItem]:
                    ...
                    memory_item = MemoryItem(
                        ...
                        metadata={
                            **metadata,
                            ...
                            "source": "system" if metadata.get('is_ai', False) else "user"
                        }
                    )
    ...
    async def _get_conversation_memories(self, user_id: str, 
                                       session_id: Optional[str] = None, 
                                       limit: int = 10) -> List[MemoryItem]:
    ...
    async def _get_entity_information(self, query: Optional[str] = None) -> Dict[str, Any]:
        """Get entity information from entity memory."""
        try:
            # If query is provided, look up specific entities
            if query:
                entities = await asyncio.to_thread(
                    self.entity_memory.get_entities_from_text,
                    text=query
                )
                ...
                for entity in entities:
                    info = await asyncio.to_thread(
                        self.entity_memory.load_entity_from_memory,
                        entity=entity
                    )
                    if info:
