...
class FirestoreClient:
    ...
    @eh.with_retry(max_retries=3, retryable_exceptions=(eh.TemporaryError, eh.TimeoutError))
    ...
    async def get_document(self, collection: str, doc_id: str) -> Dict[str, Any]:
        ...
        if not doc.exists:
            raise eh.NotFoundError(f"Document {doc_id} not found in collection {collection}")
    ...
    @eh.with_circuit_breaker(firestore_circuit)
    @eh.handle_gcp_error
    async def save_document(self, collection: str, doc_id: str, data: Dict[str, Any]) -> str:
        """
...
class LLMClient:
    ...
    @eh.with_retry(max_retries=2, retryable_exceptions=(eh.TemporaryError, eh.TimeoutError, eh.RateLimitError))
