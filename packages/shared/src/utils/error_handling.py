            ...
            if gcp_exceptions_available:
                ...
                if isinstance(e, NotFound):
                    ...
                    error_id = error_logger.log_error(
                        func.__name__, e, level="warning", context={"status_code": 404}
                    )
                    ...
                    error_id = error_logger.log_error(
                        func.__name__, e, level="error", context={"status_code": 400}
                    )
                    raise PreconditionError(
                        f"Operation precondition failed: {e}", e
                    ) from e

                elif isinstance(e, ResourceExhausted):
                    # Handle rate limits (429)
                    error_id = error_logger.log_error(
                        func.__name__, e, level="warning", context={"status_code": 429}
                    )
                    ...
                    error_id = error_logger.log_error(
                        func.__name__, e, level="warning", context={"status_code": 503}
                    )
                    raise TemporaryError(
                        f"Service temporarily unavailable: {e}", e
                    ) from e

                elif isinstance(e, DeadlineExceeded):
                    # Handle timeout (504)
                    error_id = error_logger.log_error(
                        func.__name__, e, level="warning", context={"status_code": 504}
                    )
                    ...
                    error_id = error_logger.log_error(
                        func.__name__, e, level="error", context={"status_code": 401}
                    )
                    ...
                    error_id = error_logger.log_error(
                        func.__name__, e, level="error", context={"status_code": 403}
                    )
        ...
        try:
            ...
            if openai_exceptions_available:
                ...
                if isinstance(e, APITimeoutError):
                    ...
                    raise AuthenticationError(
                        f"OpenAI authentication failed: {e}", e
                    ) from e
            ...
            if anthropic_exceptions_available:
                ...
                if isinstance(e, AnthropicTimeoutError):
                    ...
                    raise RateLimitError(
                        f"Anthropic rate limit exceeded: {e}", e
                    ) from e
                    ...
                    raise ValidationError(
                        f"Invalid request to Anthropic: {e}", e
                    ) from e
            ...
            if portkey_exceptions_available:
                ...
                if isinstance(e, PortkeyAPIError):
                    error_id = error_logger.log_error(
                        func.__name__, e, level="error", context={"provider": "portkey"}
                    )
            ...
            if any(term in error_str for term in ["timeout", "timed out", "deadline"]):
                ...
                term in error_str for term in ["connect", "network", "unreachable"]
            ):
                error_id = error_logger.log_error(func.__name__, e, level="warning")
                ...
                term in error_str for term in ["invalid", "bad request", "validation"]
            ):
                error_id = error_logger.log_error(func.__name__, e, level="error")
...
class ResourceManager:
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> bool:
        ...
        try:
            ...
            if hasattr(self.resource, "close") and callable(
                getattr(self.resource, "close")
            ):
                self.resource.close()
            elif hasattr(self.resource, "shutdown") and callable(
                getattr(self.resource, "shutdown")
            ):
                self.resource.shutdown()
            elif hasattr(self.resource, "cleanup") and callable(
                getattr(self.resource, "cleanup")
            ):
    ...
    async def __aexit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> bool:
        ...
        try:
            ...
            if hasattr(self.resource, "aclose") and callable(
                getattr(self.resource, "aclose")
            ):
                await self.resource.aclose()
            elif hasattr(self.resource, "async_close") and callable(
                getattr(self.resource, "async_close")
            ):
            ...
            self.logger.debug(
                f"Successfully cleaned up async resource: {self.resource_name}"
            )
        except Exception as e:
            self.logger.warning(
                f"Error cleaning up async resource {self.resource_name}: {e}"
            )
            # Don't suppress the original exception
            return False
...
class CircuitBreaker:
    ...
    def record_success(self) -> None:
        ...
        if self.state == self.HALF_OPEN:
            self.logger.info(
                f"Circuit '{self.name}': Successful request in HALF_OPEN state, closing circuit"
            )
            self.state = self.CLOSED
            self.failure_count = 0
    ...
    def record_failure(self) -> None:
        ...
        if self.state == self.HALF_OPEN:
            self.logger.warning(
                f"Circuit '{self.name}': Request failed in HALF_OPEN state, reopening circuit"
            )
    ...
    def decorator(func: AsyncFunc) -> AsyncFunc:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if not circuit.allow_request():
                raise TemporaryError(
                    f"Circuit '{circuit.name}' is open, request rejected", None
                )
...
async def example_usage():
    ...
    firestore_circuit = CircuitBreaker(
        name="firestore", failure_threshold=3, recovery_timeout=10
    )
