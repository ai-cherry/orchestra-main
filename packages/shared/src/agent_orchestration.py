...
class DomainExpertAgent(BaseAgent):
    ...
    async def _initialize_workers(self) -> None:
        ...
        if self.specialization == "sales":
            ...
            self.workers["analyst"] = WorkerAgent(
                f"{self.agent_id}_analyst", 
                AgentRole.ANALYST
            )
            self.workers["crm"] = WorkerAgent(
                f"{self.agent_id}_crm", 
    ...
    async def _determine_worker_assignments(
        ...
        messages = [
            {"role": "system", "content": f"You are a {self.specialization} domain expert. Assign workers to subtasks."},
    ...
    async def _integrate_results(
        ...
        messages = [
            {"role": "system", "content": f"You are a {self.specialization} domain expert. Integrate these worker results into a coherent response."},
            {"role": "user", "content": f"ORIGINAL TASK: {original_message.content}\n\nWORKER RESULTS:\n{formatted_results}\n\nPlease integrate these results into a comprehensive, coherent response."}
        ]
...
class WorkerAgent(BaseAgent):
    ...
    def _get_capabilities_for_role(self, role: AgentRole) -> Set[str]:
        ...
        capabilities_map = {
            ...
            AgentRole.DATA_PROCESSOR: {"data_cleaning", "data_transformation", "data_validation"},
            AgentRole.WEB_RESEARCHER: {"web_search", "data_extraction", "source_validation"},
            AgentRole.CRM_INTEGRATOR: {"crm_query", "customer_analysis", "sales_forecasting"},
        }
    ...
    async def _process_coder_task(self, content: str, subtasks: List[str]) -> AgentMessage:
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Coding Task: {content}\nPlease implement this."}
        ]
    ...
    async def _process_data_processor_task(self, content: str, subtasks: List[str]) -> AgentMessage:
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Data Processing Task: {content}\nPlease process this data."}
        ]
    ...
    async def _process_web_researcher_task(self, content: str, subtasks: List[str]) -> AgentMessage:
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"Research Task: {content}\nPlease research this topic."}
        ]
    ...
    async def _process_crm_task(self, content: str, subtasks: List[str]) -> AgentMessage:
        ...
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"CRM Task: {content}\nPlease analyze this customer data."}
        ]
...
class AgentOrchestrator:
    ...
    async def _determine_domain(
        ...
        messages = [
            {"role": "system", "content": "You are a message router. Determine which domain expert should handle this request. Choose from: sales, service, tech, research. Respond with just the domain name in lowercase."},
