...
class FirestoreClient:
    ...
    def __init__(
        ...
        if firestore is None:
            logger.error(
                "Google Cloud Firestore library not available. Install with: pip install google-cloud-firestore"
            )
                ...
                if self._service_account_json:
                    ...
                    if not self._project_id:
                        try:
                            ...
                            logger.warning(
                                "Could not extract project_id from service account JSON"
                            )
                    ...
                    if not self._project_id:
                        self._project_id = os.environ.get(
    ...
    async def query_collection(
        ...
        try:
            ...
            if order_by:
                for field, direction in order_by:
                    if direction.lower() == "desc":
                        ...
                        query = query.order_by(
                            field, direction=firestore.Query.ASCENDING
                        )
            ...
            logger.debug(
                f"Query returned {len(results)} documents from collection {collection}"
            )
            return results
        except Exception as e:
    ...
    def _process_data_for_firestore(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            # Handle lists with possible nested objects
                ...
                processed[key] = [
                    self._process_data_for_firestore(item)
