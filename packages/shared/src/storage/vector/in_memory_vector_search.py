...
class InMemoryVectorSearch(AbstractVectorSearch):
    ...
    async def store_embedding(
        ...
        if len(embedding) != self._dimensions:
            raise ValueError(f"Expected embedding dimension {self._dimensions}, got {len(embedding)}")
    ...
    async def find_similar(
        ...
        if len(query_embedding) != self._dimensions:
            raise ValueError(f"Expected query dimension {self._dimensions}, got {len(query_embedding)}")
    ...
    async def _find_similar_numpy(
        ...
        for item_id, embedding in self._embeddings.items():
            ...
            if embedding_norm > 0 and query_norm > 0:
                similarity = np.dot(query_np, embedding_np) / (query_norm * embedding_norm)
            else:
                similarity = 0.0
