...
class FirestoreMemoryManagerV2(MemoryManager):
    ...
    async def add_memory_item(self, item: MemoryItem) -> str:
        ...
        try:
            ...
            # Store in Firestore
            collection = self.firestore.config.get_collection_name(
                MEMORY_ITEMS_COLLECTION
            )
    ...
    async def get_conversation_history(
        ...
        try:
            # Set up filters
            query_filters = {USER_ID_FIELD: user_id, ITEM_TYPE_FIELD: "conversation"}
        ...
        try:
            ...
            while True:
                ...
                batch_results = await asyncio.gather(
                    *[process_doc(doc) for doc in docs_with_embeddings]
                )
    ...
    async def check_duplicate(self, item: MemoryItem) -> bool:
        ...
        try:
            ...
            docs = await self.firestore.query_documents(
                collection=collection, filters=query_filters, limit=1
            )
    ...
    async def cleanup_expired_items(self) -> int:
        ...
        try:
            ...
            query_filters = {"expiration": {"end": now}}
            ...
            expired_docs = await self.firestore.query_documents(
                collection=collection, filters=query_filters
            )
    ...
    async def health_check(self) -> MemoryHealth:
        ...
        # Add recent errors
        if self._errors:
            health["details"]["recent_errors"] = list(self._errors)[
                -5:
            ]  # Last 5 errors
