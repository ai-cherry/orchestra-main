...
class VertexEmbeddingService(EmbeddingService if HAS_BASE_CLASS else object):
    ...
    async def initialize(self) -> bool:
        ...
        if not self.project_id:
            logger.info(
                "No GCP project ID configured, skipping Vertex AI initialization"
            )
            return False
    ...
    def _initialize_sync(self) -> bool:
        ...
        try:
            ...
            self.embedding_endpoint = aiplatform.TextEmbeddingModel.from_pretrained(
    ...
    async def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            embeddings = await loop.run_in_executor(
                None, lambda: self._get_embeddings_sync(texts)
            )
    ...
    async def vector_search(
        ...
        try:
            ...
            results = await loop.run_in_executor(
                None,
                lambda: self._vector_search_sync(
                    query_vector, namespace, num_neighbors
                ),
            )
    ...
    def _vector_search_sync(
        ...
        for match in response[0]:
            ...
            # Add to results
            results.append(
                {"id": match.id, "distance": match.distance, "metadata": metadata}
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await loop.run_in_executor(
                None, lambda: self._get_embeddings_sync(["Test health check"])
            )
