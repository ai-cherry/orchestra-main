...
def get_vertex_endpoint(
    endpoint_id: str, settings: Settings = Depends(get_settings)
) -> str:
    """
...
def get_secret(
    ...
    settings: Settings = Depends(get_settings),
    client: secretmanager.SecretManagerServiceClient = Depends(
    ...
    try:
        name = (
            f"projects/{settings.project_id}/secrets/{secret_id}/versions/{version_id}"
        )
