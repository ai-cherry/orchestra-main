...
# 6. Enabling GitHub security features
...
section "6. Enabling GitHub Security Features"

# Function to enable GitHub security features
enable_github_security_features() {
    echo "Enabling GitHub security features..."
    ...
    if [ "$response" -ne 200 ]; then
        echo "Error enabling Secret Scanning. HTTP status: $response"
        exit 1
    fi
    ...
    response=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH \
        ...
        -H "Accept: application/vnd.github.v3+json" \
        https://api.github.com/repos/$GITHUB_ORG/$GITHUB_REPO \
        -d '{"security_and_analysis": {"push_protection": {"status": "enabled"}}}')

    if [ "$response" -ne 200 ]; then
        echo "Error enabling Push Protection. HTTP status: $response"
        exit 1
    fi

    echo "GitHub security features enabled successfully."
}
...
add_github_secrets() {
    ...
    # Example: Add GCP_PROJECT_MANAGEMENT_KEY
    response=$(curl -s -o /dev/null -w "%{http_code}" -X PUT \
        -H "Authorization: token $GITHUB_PAT" \
        -H "Accept: application/vnd.github.v3+json" \
        https://api.github.com/repos/$GITHUB_ORG/$GITHUB_REPO/actions/secrets/GCP_PROJECT_MANAGEMENT_KEY \
        -d '{"encrypted_value": "$GCP_PROJECT_MANAGEMENT_KEY"}')

    if [ "$response" -ne 201 ]; then
        echo "Error adding secret GCP_PROJECT_MANAGEMENT_KEY. HTTP status: $response"
        exit 1
    fi

    echo "Secrets added to GitHub repository successfully."
}

# Main script execution for enabling security features
...
echo
read -s -p "Enter your GCP Project Management Key: " GCP_PROJECT_MANAGEMENT_KEY
echo
...
success "GitHub security features enabled and secrets added."
...
echo "  5. Enable GitHub security features and add repository secrets"
echo ""
