...
try:
    ...
    logger.error(
        "Google Cloud libraries not found. Install with: pip install google-cloud-secretmanager"
    )
    sys.exit(1)
    ...
    if env == "prod":
        ...
        for secret in secrets:
            if any(non_prod in secret for non_prod in NON_PROD_SECRETS):
                logger.error(
                    f"Development secret {secret} found in production environment"
                )
                success = False
...
def check_required_secrets(
    project_id: str, available_secrets: List[str], env: str
) -> bool:
...
def validate_cloud_run_service_access(
    project_id: str, service_name: str, env: str
) -> bool:
    ...
    try:
        ...
        if not service_account:
            logger.error(
                f"Could not determine service account for {service_name}-{env}"
            )
            return False

        logger.info(
            f"Service {service_name}-{env} uses service account: {service_account}"
        )
        ...
        if not has_access:
            logger.error(
                f"Service account {service_account} does not have secretAccessor role"
            )
            return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Secret Manager configuration"
    )
    parser.add_argument(
        "--env",
        default="dev",
    ...
    parser.add_argument(
        "--project-id", help="GCP Project ID (defaults to environment variable)"
    )
    parser.add_argument(
        "--service",
        default="orchestra-api",
    ...
    env = args.env
    logger.info(
        f"Validating Secret Manager in project {project_id} for {env} environment"
    )
    ...
    if not service_ok:
        logger.warning(
            f"Cloud Run service {args.service}-{env} may not have proper secret access"
        )
