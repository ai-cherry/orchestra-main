...
class ThreadPoolManager:
    ...
    async def run_in_thread(cls, func, *args, **kwargs):
        ...
        return await asyncio.wrap_future(
            cls.get_pool().submit(func, *args, **kwargs)
        )
...
def with_retry(max_retries=3, base_delay=0.1, retryable_exceptions=(GoogleAPIError, ConnectionError)):
    """
...
async def cleanup_expired_items_with_progress(self) -> int:
    ...
    try:
        ...
        logger.info(f"Cleanup completed: deleted {total_deleted} expired items in {total_batches} batches ({duration:.2f} seconds)")
