...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content=ErrorResponse(
            success=False, error=exc.detail, details=getattr(exc, "details", None)
        ).dict(),
    )
...
async def store_memory(
    ...
    try:
        success = await memory_manager.store(
            key, content_data.content, content_data.tool_name, content_data.ttl_seconds
        )
...
async def get_memory(
    key: str, memory_manager: PerformanceMemoryManager = Depends(get_memory_manager)
):
