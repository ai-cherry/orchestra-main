...
echo -e "${GREEN}claude \"analyze if project cherry-ai-project is ready for migration to organization 873291114285\"${NC}"
echo -e "${GREEN}claude \"identify potential blockers for GCP project migration\"${NC}"
echo -e "${GREEN}claude \"check if service account vertex-agent@cherry-ai-project.iam.gserviceaccount.com has sufficient permissions\"${NC}"

echo -e "\n${YELLOW}Migration Planning${NC}"
echo -e "${BLUE}Run these commands to get migration planning assistance:${NC}"
echo -e "${GREEN}claude \"create a detailed migration plan for moving cherry-ai-project to organization 873291114285\"${NC}"
