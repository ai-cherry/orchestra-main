...
class GcpResourceCompareRequest(BaseModel):
    ...
    snapshot_id: str = Field(..., description="Snapshot ID (directory name)")
    resource_types: List[str] = Field(default=["compute", "storage", "databases", "ai"], 
...
async def compare_gcp_with_code(request: GcpResourceCompareRequest) -> Dict[str, Any]:
    ...
    metadata_file = os.path.join(snapshot_dir, "metadata.json")
    if not os.path.exists(metadata_file):
        raise HTTPException(status_code=404, detail=f"Metadata for snapshot {request.snapshot_id} not found")
...
async def list_gcp_resources(request: GcpResourceListRequest) -> Dict[str, Any]:
    ...
    if not os.path.isdir(resource_dir):
        raise HTTPException(status_code=404, detail=f"Resource type {request.resource_type} not found in snapshot")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--snapshot-dir", default=DEFAULT_SNAPSHOT_DIR, help="Directory to store snapshots")
    parser.add_argument("--snapshot-script", default=DEFAULT_SNAPSHOT_SCRIPT, help="Path to snapshot script")
