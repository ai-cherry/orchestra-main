...
async def test_enhanced_methods():
    ...
    session_id = await postgres.session_create(
        user_id="test_user", agent_id="test_agent", data={"test": "session"}, ttl=3600
    )
    ...
    snapshot_id = await postgres.memory_snapshot_create(
        agent_id="test_agent", user_id="test_user", snapshot_data={"memory": "test snapshot"}, metadata={"test": True}
    )
