...
async function routeModelRequest(mode, prompt, systemPrompt) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request(PORTKEY_API_URL, {
      method: 'POST',
      headers: headers
    }, (res) => {
      let data = '';
    ...
    req.on('end', async () => {
      try {
        ...
        if (!mode || !prompt) {
          ...
          res.end(JSON.stringify({ error: 'Missing required parameters: mode and prompt' }));
          return;
        }
...
server.listen(PORT, () => {
  ...
  console.log(JSON.stringify({
    schema: {
      route_model_request: {
        description: "Route a model request to the appropriate provider via Portkey",
        parameters: {
          mode: {
            type: "string",
