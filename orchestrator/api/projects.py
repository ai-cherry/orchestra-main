...
PROJECT_STATUSES = [
    "initiated", "planning", "execution", 
    "review", "consolidation", "completed", "failed"
]
...
def create_project():
    ...
    if data['status'] not in PROJECT_STATUSES:
        return jsonify({"error": f"Invalid status: {data['status']}. Must be one of {PROJECT_STATUSES}"}), 400
...
def update_project_status(project_id, status):
    ...
    if status not in PROJECT_STATUSES:
        return jsonify({"error": f"Invalid status: {status}. Must be one of {PROJECT_STATUSES}"}), 400
    ...
    if status in status_progress_map and project['progress'] < status_progress_map[status]:
