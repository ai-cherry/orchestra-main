...
class SecureGCPProjectMigrator:
    ...
    def create_service_account_key(self) -> str:
        ...
        cmd = [
            "gcloud", "iam", "service-accounts", "keys", "create", self.temp_key_file.name,
            "--iam-account", self.service_account_email, "--format", "json"
        ]
    ...
    def move_project(self) -> bool:
        ...
        cmd = [
            "gcloud", "beta", "projects", "move", self.project_id,
            "--organization", self.target_org_id,
            "--billing-project", self.project_id
        ]
    ...
    def _handle_permission_denied_error(self) -> None:
        ...
        try:
            ...
            cmd = [
                "gcloud", "organizations", "get-iam-policy", self.target_org_id,
                "--filter", f"bindings.members:serviceAccount:{self.service_account_email}",
                "--format", "json"
            ]
    ...
    def verify_migration(self) -> bool:
        ...
        try:
            ...
            if current_org == self.target_org_id:
                ...
                logger.error(f"❌ Verification failed: Project is in organization {current_org}, not {self.target_org_id}")
                return False
        except subprocess.CalledProcessError as e:
...
def main():
    ...
    parser.add_argument("--iam-wait", type=int, default=300, help="IAM propagation wait time in seconds (default: 300)")
