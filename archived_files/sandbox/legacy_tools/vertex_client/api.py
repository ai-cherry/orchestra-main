...
async def execute_task_in_background(task: str, params: Optional[Dict[str, Any]] = None):
    ...
    """
    logger.info(f"Executing task in background: {task}")
    result = trigger_vertex_task(task, **(params or {}))
    logger.info(f"Task completed: {task} - Status: {result.get('status')}")
...
async def trigger_task(request: VertexTaskRequest, background_tasks: BackgroundTasks):
    ...
    """
    logger.info(f"Received request to trigger task: {request.task}")
...
async def monitor_resources(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {
            ...
            "task": task,
            "result": {
                "message": "Started monitoring GCP resources"
            }
        }
    except Exception as e:
