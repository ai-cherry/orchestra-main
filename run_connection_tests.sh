...
# Check Lambda auth environment
if [ -z "$  echo "⚠️ Warning:   echo "Firestore tests may fail without proper Lambda authentication"
  echo ""
fi

if [ -z "$Lambda_SA_KEY_PATH" ] && [ -z "$Lambda_SA_KEY_JSON" ]; then
  echo "⚠️ Warning: Neither Lambda_SA_KEY_PATH nor Lambda_SA_KEY_JSON is set"
  echo "Firestore tests may fail without proper Lambda authentication"
  echo ""
fi
