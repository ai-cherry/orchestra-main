...
# Mode-specific MCP assessment prompts
MODE_MCP_PROMPTS = {
    "reviewer": """
When reviewing code or architecture designs, assess MCP (Model Context Protocol) compatibility:

1. Evaluate if the solution's memory management approach is appropriate for the use case
2. Check if MCP is properly implemented for cross-component communication
3. Verify that appropriate memory persistence strategies are used
4. Assess if data flow between models/components follows MCP best practices
5. Check for security considerations in the MCP implementation
6. Validate that the solution meets the performance requirements with the chosen MCP approach

Your review should explicitly state whether MCP is appropriate for the scenario and why.
""",
    "architect": """
When designing solutions, carefully consider MCP (Model Context Protocol) requirements:

1. Evaluate data persistence needs across model interactions
2. Consider memory size requirements and token constraints
3. Determine if cross-session or cross-agent memory is required
4. Factor in performance implications of MCP approaches for the specific use case
5. Design appropriate interfaces for MCP components

Always specify which MCP approach is most suitable and provide a clear rationale.
""",
    "strategy": """
When breaking down tasks, consider MCP (Model Context Protocol) implications:

1. Identify subtasks that would benefit from MCP integration
2. Specify which components need MCP-compatibility
3. Consider memory persistence requirements across task execution
4. Factor in cross-agent communication needs
5. Include specific steps for MCP testing and validation

Recommend the appropriate MCP implementation strategy for the overall task.
"""
}
...
# MCP workflow templates for common scenarios
MCP_WORKFLOW_TEMPLATES = {
    "memory_system_review": [
        {
            "type": "mode",
            "mode": "architect",
            "task": "Analyze the current memory system architecture and identify any limitations or bottlenecks. Focus on MCP compatibility and integration points."
        },
        {
            "type": "mode",
            "mode": "reviewer",
        ...
        },
        {
            "type": "mode",
            "mode": "strategy",
            "task": "Develop a strategy for enhancing the memory system with improved MCP integration. Consider different usage scenarios and reliability requirements."
        }
    ],
    "mcp_integration": [
        {
            "type": "mode",
            "mode": "architect",
            "task": "Design an integration approach for adding MCP support to the existing system. Consider data structures, API interfaces, and performance requirements."
        },
        {
            "type": "mode",
            "mode": "code",
            "task": "Implement the core MCP integration components based on the architecture design. Focus on clean abstractions and maintainable code."
        },
        {
            "type": "mode",
            "mode": "debug",
            "task": "Test the MCP integration and identify any issues with data flow, persistence, or error handling."
        },
        {
            "type": "mode",
            "mode": "reviewer",
            "task": "Review the complete MCP implementation for correctness, performance, and security."
        }
    ],
    "mcp_performance_optimization": [
        {
            "type": "mode",
            "mode": "debug",
            "task": "Profile the current MCP implementation to identify performance bottlenecks and memory usage patterns."
        },
        {
            "type": "mode",
            "mode": "architect",
            "task": "Design performance optimizations for the MCP system based on the profiling results. Consider caching strategies, data structures, and query patterns."
        },
        {
            "type": "mode",
            "mode": "code",
            "task": "Implement the performance optimizations while maintaining backward compatibility."
        },
        {
            "type": "mode",
            "mode": "reviewer",
            "task": "Review the optimizations for correctness and measure the performance improvements."
        }
    ]
}
...
class MemoryBank:
    ...
    def save(key: str, content: str) -> bool:
        ...
        try:
            ...
            cmd = ["roo-cli", "mcp", "memory_bank_write", "--key", key, "--content", content]
    ...
    def update(key: str, content: str) -> bool:
        ...
        try:
            ...
            cmd = ["roo-cli", "mcp", "memory_bank_update", "--key", key, "--content", content]
            result = subprocess.run(cmd, check=True, capture_output=True, text=True)
...
class RooModeManager:
    ...
    def execute_in_mode(mode: str, prompt: str, context: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            if mode in MODE_MCP_PROMPTS and "mcp" in prompt.lower():
                # Add the mode-specific MCP guidance to the context
                mcp_guidance = MODE_MCP_PROMPTS[mode]
                if context:
                    context = f"{context}\n\n{mcp_guidance}"
                else:
            ...
            cmd = ["roo-cli", "mcp", "portkey-router", "route_model_request", 
    ...
    def get_mcp_workflow_template(template_name: str) -> Optional[List[Dict[str, Any]]]:
        ...
        return MCP_WORKFLOW_TEMPLATES.get(template_name)
...
class GithubIntegration:
    ...
    def create_pr(title: str, body: str, base_branch: str = "main") -> Optional[str]:
        ...
        try:
            ...
            # Create PR
            pr_cmd = ["gh", "pr", "create", "--title", title, "--body", body, "--base", base_branch]
...
class TerraformManager:
    ...
    @staticmethod
    def get_outputs(working_dir: str = "terraform", output_name: Optional[str] = None) -> Union[Dict[str, str], Optional[str]]:
...
def main():
    ...
    task_parser.add_argument("--mode", required=True, choices=MODE_MAP.keys(), 
    ...
    vscode_task_parser.add_argument("--task_id", required=True, choices=VSCODE_TASKS.keys(),
    ...
    terraform_parser.add_argument("--operation", required=True, 
                                help="Terraform operation to run (e.g., plan, apply, init)")
    terraform_parser.add_argument("--working_dir", default="terraform", 
    ...
    gcp_parser.add_argument("--operation", required=True,
    ...
    # New command for MCP template listing
    ...
    if args.command == "task":
        ...
        for name, template in MCP_WORKFLOW_TEMPLATES.items():
