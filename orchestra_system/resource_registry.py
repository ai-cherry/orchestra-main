...
class ResourceRegistry:
    ...
    async def _discover_cli_tools(self, environment: Environment, new_resources: List[Resource]) -> None:
        ...
        cli_tools = [
            "gcloud", "gsutil", "terraform", "docker", "kubectl", "python", "pip",
            "node", "npm", "yarn", "git", "curl", "wget"
        ]
    ...
    async def _discover_gcp_apis(self, environment: Environment, new_resources: List[Resource], project_id: str) -> None:
    ...
    async def _discover_github_actions(self, environment: Environment, new_resources: List[Resource], owner: str, repo: str) -> None:
        ...
        for port, service_name in common_ports:
            if await self._check_port_open("localhost", port):
                ...
                service_resource = Resource(
                    ...
                    priority=ResourcePriority.MEDIUM,
                    metadata={"port": port, "host": "localhost", "service": service_name}
                )
