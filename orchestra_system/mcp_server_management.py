        ...
        try:
            ...
            try:
                ...
                self.process = subprocess.Popen(
                    [sys.executable, "-c", "from mcp_server import run_server; run_server()"],
                    env=env,
                    stdout=log_file,
...
async def main():
    ...
    parser.add_argument("--format", choices=["json", "binary", "auto"], default="auto", help="Import/export format")
    ...
    if not any([args.start, args.stop, args.restart, args.check, args.sync, args.info,
                args.export, args.import_path, getattr(args, "sync_from", None)]):
        args.info = True
