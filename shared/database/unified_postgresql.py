...
class UnifiedPostgreSQL:
    ...
    async def cache_delete(self, key: str) -> bool:
        """Delete cache entry."""
        result = await self._manager.execute(
            "DELETE FROM cache.entries WHERE key = $1", key
        )
    ...
    async def session_create(
        ...
        """, session_id, user_id, agent_id, json.dumps(data or {}), 
            expires_at, ip_address, user_agent, json.dumps(metadata or {}))
    ...
    async def session_update(
        ...
        result = await self._manager.execute(f"""
            UPDATE sessions.sessions
    ...
    async def agent_update(
        ...
        row = await self._manager.fetchrow(f"""
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        table_stats = {}
        for schema_table in ['cache.entries', 'sessions.sessions', 'orchestra.agents', 
        ...
        return {
            **manager_health,
            ...
            'cleanup_tasks': {
                name: not task.done() 
                for name, task in self._cleanup_tasks.items()
            }
        }
