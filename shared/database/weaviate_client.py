...
class WeaviateClient:
    ...
    def get_recent_memories(self, agent_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of memories
        """
        agent_memories = [m for m in self._memories.values() if m["agent_id"] == agent_id]
    ...
    def get_conversation_history(self, session_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of messages
        """
        messages = [m for m in self._conversations.values() if m["session_id"] == session_id]
    ...
    def search_knowledge(
        ...
        for item in knowledge_items:
            score = self._calculate_relevance(query_lower, f"{item['title']} {item['content']}".lower())
            if score > 0:
