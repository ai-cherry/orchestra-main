    ...
    async def cache_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'health': {
                    'expired_ratio': float(stats['expired_entries']) / float(stats['total_entries']) 
                        if stats and stats['total_entries'] > 0 else 0,
                    'avg_entry_size_kb': float(stats['avg_entry_size']) / 1024 
                        if stats and stats['avg_entry_size'] else 0,
                    'cache_efficiency': float(stats['avg_access_count']) 
                        if stats and stats['avg_access_count'] else 0
                }
            }
