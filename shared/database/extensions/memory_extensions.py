...
class MemoryExtensionsMixin:
    ...
    async def memory_snapshot_delete(
        self,
        snapshot_id: Union[str, UUID]
    ) -> bool:
        """
    ...
    async def memory_snapshot_cleanup(
        self,
        days_old: int = 90,
        keep_minimum: int = 5
    ) -> int:
    ...
    async def memory_snapshot_restore(
        self,
        snapshot_id: Union[str, UUID],
        target_agent_id: Optional[Union[str, UUID]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def memory_snapshot_search(
        ...
        try:
            ...
            rows = await self._manager.fetch(f"""
                SELECT 
                    id,
                    agent_id,
                    user_id,
    ...
    async def memory_snapshot_stats(
        self,
        agent_id: Optional[Union[str, UUID]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            stats = await self._manager.fetchrow(f"""
                SELECT 
            ...
            size_dist = await self._manager.fetch(f"""
                SELECT 
                    CASE 
            ...
            return {
                ...
                'health': {
                    'avg_size_kb': float(stats['avg_snapshot_size']) / 1024 
                        if stats and stats['avg_snapshot_size'] else 0,
                    'total_size_mb': float(stats['total_snapshots'] * stats['avg_snapshot_size']) / 1048576
                        if stats and stats['total_snapshots'] and stats['avg_snapshot_size'] else 0
                }
            }
