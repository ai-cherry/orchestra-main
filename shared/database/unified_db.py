...
class UnifiedDatabase:
    ...
    def search_agent_context(
        ...
        if include_memories:
            results["memories"] = self.weaviate.search_memories(
                agent_id=agent_id,
                query=query,
                limit=limit // 2 if include_conversations else limit
            )
        ...
        if include_conversations:
            results["conversations"] = self.weaviate.search_conversations(
                query=query,
                agent_id=agent_id,
                limit=limit // 2 if include_memories else limit
            )
    ...
    def create_workflow_with_context(
        ...
        workflow = self.postgres.create_workflow({
            "name": name,
    ...
    def search_knowledge(
        ...
        return self.weaviate.search_knowledge(
            query=query,
            category=category,
            tags=tags,
            limit=limit
    ...
    def update_session_activity(self, session_id: str) -> bool:
        ...
        updated = self.postgres.create_session(
            session_id,
            session_data,
            session["expires_at"]
        )
    ...
    def get_system_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "postgresql": {
                ...
                "active_sessions": len([s for s in self.postgres.execute_query(
    ...
    def get_agent_activity_report(
        ...
        # Get audit logs
        audit_logs = self.postgres.get_audit_logs(
            resource_type="agent",
            limit=100
        )
