...
class Issue:
    ...
    def __init__(self, file_path: str, line: int, category: str, message: str, 
...
class CodeVisitor(ast.NodeVisitor):
        ...
        for name in node.names:
            ...
            if '.' not in name.name and self._is_in_package():
                self.issues.append(Issue(
                    self.file_path, node.lineno, 'import_style',
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        ...
        if not ast.get_docstring(node):
            self.issues.append(Issue(
                self.file_path, node.lineno, 'missing_docstring',
        ...
        if node.returns is None and not self._is_test_function(node.name):
            self.issues.append(Issue(
                self.file_path, node.lineno, 'missing_type_hint',
        ...
        if complexity > 10:
            self.issues.append(Issue(
                self.file_path, node.lineno, 'complex_function',
    ...
    def analyze(self) -> List[Issue]:
        ...
        for name in self.imported_names:
            if name not in self.used_names and name != '__future__':
                self.issues.append(Issue(
                    self.file_path, 1, 'unused_import',
        ...
        for name in self.defined_names:
            if name not in self.used_names and not name.startswith('_'):
                self.issues.append(Issue(
                    self.file_path, 1, 'unused_variable',
