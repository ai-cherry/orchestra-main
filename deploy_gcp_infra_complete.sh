...
# Additional GCP infrastructure deployment
echo -e "\n${BLUE}${BOLD}========================================================================${NC}"
echo -e "${BLUE}${BOLD}   ADDITIONAL GCP INFRASTRUCTURE DEPLOYMENT   ${NC}"
echo -e "${BLUE}${BOLD}========================================================================${NC}"

echo "Deploying complete GCP infrastructure..."
...
gcloud storage buckets create gs://cherry-ai-me-model-artifacts \
    --project=${PROJECT_ID} \
    --location=${REGION}
gcloud storage buckets create gs://cherry-ai-me-cloudbuild-artifacts \
    --project=${PROJECT_ID} \
    --location=${REGION}
...
gcloud run deploy orchestra-prod \
    --image=us-central1-docker.pkg.dev/${PROJECT_ID}/orchestra/api:latest \
    --platform=managed \
    --region=${REGION} \
