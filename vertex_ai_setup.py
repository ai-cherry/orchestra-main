...
try:
    ...
    print("Please install them using: pip install google-cloud-aiplatform google-cloud-storage google-cloud-secret-manager")
    sys.exit(1)
...
class VertexAISetup:
    ...
    def list_notebook_instances(self) -> List[Dict[str, Any]]:
        ...
        """
        print(f"Listing notebook instances in project {self.project_id}, region {self.region}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Vertex AI Setup and Integration Utility")
    ...
    deploy_parser = subparsers.add_parser("deploy-model", help="Deploy a model to Vertex AI")
    deploy_parser.add_argument("--model-path", required=True, help="Path to model artifacts in GCS")
    deploy_parser.add_argument("--endpoint-name", required=True, help="Name of the endpoint")
    deploy_parser.add_argument("--machine-type", default="n1-standard-4", help="VM machine type")
    deploy_parser.add_argument("--min-replicas", type=int, default=1, help="Minimum number of replicas")
    deploy_parser.add_argument("--max-replicas", type=int, default=1, help="Maximum number of replicas")
    
    # Create notebook command
    notebook_parser = subparsers.add_parser("create-notebook", help="Create a Vertex AI workbench notebook")
    notebook_parser.add_argument("--name", required=True, help="Name of the notebook instance")
    notebook_parser.add_argument("--machine-type", default="n1-standard-4", help="VM machine type")
    notebook_parser.add_argument("--accelerator-type", help="GPU accelerator type")
    notebook_parser.add_argument("--accelerator-count", type=int, default=0, help="Number of accelerators")
    
    # List resources command
    list_parser = subparsers.add_parser("list-resources", help="List Vertex AI resources")
    list_parser.add_argument("--resource-type", choices=["notebooks", "endpoints", "all"], 
