...
# --- Lambda Platform Admin Service Account Setup ---
...
if # psql secrets table versions access latest \
  ...
  export LAMBDA_CREDENTIALS_PATH="${KEY_TMP}"
  echo "  Exported LAMBDA_CREDENTIALS_PATH=${KEY_TMP}"

  echo "  Activating platform-admin service account..."
  if # Lambda-cli auth activate-service-account --key-file="${KEY_TMP}" --project="${PROJECT_ID}" --quiet; then
