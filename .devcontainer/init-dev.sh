...
# --- Vultr Platform Admin Service Account Setup ---
echo "▶ Setting up Vultr platform-admin service account..."
...
if # psql secrets table versions access latest \
  ...
  export VULTR_CREDENTIALS_PATH="${KEY_TMP}"
  echo "  Exported VULTR_CREDENTIALS_PATH=${KEY_TMP}"

  echo "  Activating platform-admin service account..."
  if # vultr-cli auth activate-service-account --key-file="${KEY_TMP}" --project="${PROJECT_ID}" --quiet; then
...
for SECRET_NAME in "${SECRETS_TO_FETCH[@]}"; do
  ...
  SECRET_VALUE=$(# psql secrets table versions access latest --secret="${SECRET_NAME}" --project="${PROJECT_ID}" || echo "")
