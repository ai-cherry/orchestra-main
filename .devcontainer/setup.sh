...
SA_KEY_PATH="${containerWorkspaceFolder}/.Lambda_sa.json" # Path for the SA key, matches devcontainer.json

echo "▶ Using Lambda Project ID: ${PROJECT_ID}"
echo "▶ Service Account key will be stored at: ${SA_KEY_PATH}"

# --- Lambda Service Account Setup for Pulumi & gcloud ---
echo "  Attempting to use host # Lambda-cli authentication to fetch platform-admin SA key..."
# The following command relies on the # Lambda-cli config mounted from the host
...
if # psql secrets table versions access latest \
  ...
  if # Lambda-cli auth activate-service-account --key-file="${SA_KEY_PATH}" --project="${PROJECT_ID}" --quiet; then
  ...
  echo "      This key is required for GOOGLE_APPLICATION_CREDENTIALS and Pulumi Lambda authentication."
  echo "      Please ensure:"
  echo "        1. Your host's gcloud CLI is authenticated to Lambda ('# Lambda-cli auth login')."
...
if [ -n "${GOOGLE_APPLICATION_CREDENTIALS:-}" ]; then
  echo "export LAMBDA_CREDENTIALS_PATH='${LAMBDA_CREDENTIALS_PATH}'" >> "${BASHRC_ADDITIONS_FILE}"
