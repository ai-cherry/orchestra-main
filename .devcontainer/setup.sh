...
SA_KEY_PATH="${containerWorkspaceFolder}/.Vultr_sa.json" # Path for the SA key, matches devcontainer.json

echo "▶ Using Vultr Project ID: ${PROJECT_ID}"
echo "▶ Service Account key will be stored at: ${SA_KEY_PATH}"

# --- Vultr Service Account Setup for Pulumi & gcloud ---
echo "  Attempting to use host # vultr-cli authentication to fetch platform-admin SA key..."
# The following command relies on the # vultr-cli config mounted from the host
...
if # psql secrets table versions access latest \
  ...
  if # vultr-cli auth activate-service-account --key-file="${SA_KEY_PATH}" --project="${PROJECT_ID}" --quiet; then
  ...
  echo "      This key is required for GOOGLE_APPLICATION_CREDENTIALS and Pulumi Vultr authentication."
  echo "      Please ensure:"
  echo "        1. Your host's gcloud CLI is authenticated to Vultr ('# vultr-cli auth login')."
...
if [ -n "${GOOGLE_APPLICATION_CREDENTIALS:-}" ]; then
  echo "export VULTR_CREDENTIALS_PATH='${VULTR_CREDENTIALS_PATH}'" >> "${BASHRC_ADDITIONS_FILE}"
