#!/bin/bash
# run_terraform.sh - Unified Terraform execution in Docker
#
# This script replaces both run_terraform.sh and run_terraform_dev.sh
# with a unified approach that works for all environments.
#
# Usage:
#   ./run_terraform.sh [env] [action] [args]
#
# Where:
#   env = dev|stage|prod (defaults to dev if not specified)
#   action = init|plan|apply|destroy|output|... (any Terraform command)
...
shift 2 2>/dev/null || shift $(($#)) # Shift off the env and action, or all args if fewer than 2

# Validate environment argument
if [[ ! "$ENV" =~ ^(dev|stage|prod)$ ]]; then
    echo -e "${RED}Error: Environment must be one of: dev, stage, prod${NC}"
    echo -e "${YELLOW}Usage: ./run_terraform.sh [env] [action] [args]${NC}"
    exit 1
fi
