...
class DirectorySyncItem(SyncItem):
    ...
    def _should_exclude(self, path: Path) -> bool:
        ...
        path_str = str(path)
        return any(
            pattern.search(path_str) for pattern in self._compiled_exclude_patterns
        )
    ...
    async def _do_sync(self) -> None:
        ...
        if not (self.source_path / ".git").exists():
            raise FileNotFoundError(
                f"Source git repository not found: {self.source_path}"
            )
...
class EnvironmentVariableSyncItem(SyncItem):
    ...
    async def _do_sync(self) -> None:
        ...
        target_env = (
            self._parse_env_file(self.target_path) if self.target_path.exists() else {}
        )
        ...
        if self.mode in [SyncMode.BIDIRECTIONAL, SyncMode.GCP_TO_CODESPACES]:
            self.add_sync_item(
                EnvironmentVariableSyncItem(
                    ...
                    direction=SyncDirection.TO_CODESPACES,
                    exclude_vars=[
                        "CLOUD_WORKSTATIONS_ENVIRONMENT",
            ...
            "items": [
                {
                    ...
                    "direction": item.direction.value,
                    "status": item.status,
                    "last_sync_time": item.last_sync_time.isoformat()
                    if item.last_sync_time
                    else None,
                    "error": item.error,
                }
                for item in self.sync_items
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--status-file", type=str, help="Path to save synchronization status"
    )
