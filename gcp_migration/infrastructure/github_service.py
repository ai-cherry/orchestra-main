...
class GitHubService(IGitHubService):
    ...
    def __init__(
        ...
        if not self.github_token:
            logger.warning("GitHub token not provided. Some operations may fail due to rate limiting or authorization.")
    ...
    @async_wrap_exceptions(GitHubError, (httpx.HTTPError, ValueError, json.JSONDecodeError), 
    ...
    async def _request(
        ...
        **kwargs: Any
        ...
        if response.status_code == 401:
            raise AuthenticationError(message="Invalid GitHub token or insufficient permissions")
        elif response.status_code == 403:
            if "rate limit exceeded" in response.text.lower():
                raise GitHubError(message="GitHub API rate limit exceeded")
            else:
                raise AuthenticationError(message="Forbidden: Check GitHub token permissions")
        elif response.status_code == 404:
            raise ResourceNotFoundError(message=f"Resource not found: {url}")
        elif response.status_code >= 400:
            raise GitHubError(message=f"GitHub API error: {response.status_code} - {response.text}")
            
        # Parse JSON
        return response.json()
    
    @async_wrap_exceptions(GitHubError, (httpx.HTTPError, ValueError, KeyError), 
    ...
    async def get_repository_info(self, repository_name: str) -> RepositoryInfo:
        ...
        try:
            devcontainer_content = await self._request(
                "GET", 
                f"/repos/{formatted_name}/contents/.devcontainer"
            )
            has_devcontainer = isinstance(devcontainer_content, list) and len(devcontainer_content) > 0
        ...
        try:
            actions_content = await self._request(
                "GET", 
                f"/repos/{formatted_name}/contents/.github/workflows"
            )
            has_github_actions = isinstance(actions_content, list) and len(actions_content) > 0
        except ResourceNotFoundError:
        ...
        try:
            secrets_data = await self._request(
                "GET",
                f"/repos/{formatted_name}/actions/secrets"
            )
        ...
        logger.info(f"Successfully fetched repository information for: {repository_name}")
        return result
    ...
    async def get_codespace_config(self, repository_name: str) -> Dict[str, Any]:
        ...
        try:
            content_data = await self._request(
                "GET",
                f"/repos/{formatted_name}/contents/.devcontainer.json"
            )
            if "content" in content_data and content_data.get("encoding") == "base64":
                import base64
                content = base64.b64decode(content_data["content"]).decode("utf-8")
            ...
            raise ResourceNotFoundError(message=f"No Codespaces configuration found for {repository_name}")
    ...
    async def get_extensions(self, repository_name: str) -> List[str]:
        ...
        try:
            ...
            if "customizations" in config and isinstance(config["customizations"], dict):
    ...
    def _format_repository_name(self, repository_name: str) -> str:
        ...
        if repository_name.startswith(("http://", "https://")):
            ...
            if path.count("/") >= 1:
                ...
                raise ValidationError(message=f"Invalid repository URL: {repository_name}")
