...
class ServiceContainer:
    ...
    async def resolve_async(
        self, service_type: Type[T], scope_id: Optional[int] = None
    ) -> T:
        """
    ...
    async def _create_instance_async(self, registration: ServiceRegistration) -> Any:
        ...
        try:
            ...
            ) and inspect.iscoroutinefunction(implementation_type.create_async):
    ...
    def _create_instance_from_type(self, implementation_type: Type) -> Any:
        ...
        try:
            ...
            for param in arg_params:
                ...
                if param_type is inspect.Parameter.empty or param_type in (
                    str,
                    int,
                    float,
                    bool,
...
class GCPServiceContainer(ServiceContainer):
    ...
    async def _initialize_connection_pools(self) -> None:
        ...
        logger.info(
            f"Initialized connection pools for {len(self.connection_pools)} services"
        )

    def _register_core_services(self) -> None:
        """Register core GCP services in the container."""
        # Register GCP service
        self.register_factory(IGCPServiceCore, self._create_gcp_service)

        # Register extended GCP service
        self.register_factory(IExtendedGCPService, self._create_gcp_service)

        # Register individual service adapters
        self.register_factory(IGCPSecretManager, self._create_secret_manager_adapter)

        self.register_factory(IGCPStorage, self._create_storage_adapter)

        self.register_factory(IGCPFirestore, self._create_firestore_adapter)

        self.register_factory(IGCPVertexAI, self._create_vertex_ai_adapter)
    ...
    def _create_secret_manager_adapter(
        ...
        client = GCPServiceFactory.create_secret_manager_client(
            project_id=self.project_id, credentials_path=self.credentials_path
        )
