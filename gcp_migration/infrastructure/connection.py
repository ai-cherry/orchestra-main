...
class ConnectionPool:
    ...
    def __init__(
        ...
        self._pool: List[Tuple[T, float, float]] = []  # (connection, create_time, last_used_time)
    ...
    async def initialize(self) -> None:
        ...
        logger.debug(f"Connection pool '{self.name}' initialized with {self.min_size} connections")
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    await asyncio.wait_for(waiter.wait(), timeout=self.connection_timeout)
    ...
    async def release(self, conn: T) -> None:
        ...
        async with self._lock:
            ...
            if conn not in self._in_use:
                logger.warning(f"Attempted to release foreign connection to pool '{self.name}'")
                return
    ...
    async def _create_connection(self) -> T:
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to create connection for pool '{self.name}'", 
                cause=e
            )
    ...
    async def _maintain_pool(self) -> None:
        ...
        while not self._closed:
            try:
                ...
                logger.error(f"Error in connection pool maintenance for '{self.name}': {e}")
