...
from gcp_migration.domain.exceptions_fixed import (
    APIError, AuthenticationError, AuthorizationError, BatchProcessingError,
    ConfigurationError, ConnectionError, CredentialError, FirestoreError,
    GCPError, ResourceAlreadyExistsError, ResourceExhaustedError, 
    ResourceNotFoundError, SecretError, StorageError, TimeoutError, 
    VertexAIError, map_to_migration_error
)
from gcp_migration.domain.models import Secret, StorageItem
from gcp_migration.domain.protocols import (
    IGCPServiceCore, IGCPSecretManager, IGCPStorage, IGCPFirestore, IGCPVertexAI
)
from gcp_migration.infrastructure.connection import ConnectionPool
from gcp_migration.infrastructure.resilience import (
    CircuitBreaker, CircuitBreakerRegistry, with_circuit_breaker
)
...
try:
    ...
    class secretmanager_v1:
        class SecretManagerServiceClient: pass
    ...
    class firestore_v1:
        ...
        class CollectionReference: pass
    
    class aiplatform_v1:
        class ModelServiceClient: pass
        class PredictionServiceClient: pass
...
class SecretManagerAdapter(IGCPSecretManager):
    ...
    async def access_secret(
        ...
        try:
            ...
            if self._connection_pool:
                ...
                return await self._access_secret_impl(self._client, secret_id, version_id)
    ...
    async def store_secret(
        ...
        try:
            ...
            if self._connection_pool:
                client = await self._connection_pool.acquire()
                try:
                    return await self._store_secret_impl(client, secret_id, secret_value)
                finally:
                    await self._connection_pool.release(client)
            else:
                # Use the instance client directly
                return await self._store_secret_impl(self._client, secret_id, secret_value)
    ...
    async def list_secrets(
        ...
        try:
            ...
            raise map_to_migration_error(
                e, SecretError, "Failed to list secrets"
            )
...
class FirestoreAdapter(IGCPFirestore):
    ...
    async def store_document(
        ...
        try:
            ...
            raise map_to_migration_error(
                e, FirestoreError, "Failed to store document"
            )
    ...
    async def _store_document_impl(
        ...
        if document_id:
            ...
            doc_ref = await asyncio.to_thread(
                lambda: collection_ref.add(data)[1]
            )
            return doc_ref.id
    ...
    async def get_model_list(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            models = await asyncio.to_thread(
                lambda: client.list_models(parent=parent)
            )
            ...
            for model in models:
                result.append({
                    ...
                    "create_time": model.create_time.ToDatetime().isoformat() 
                                  if model.create_time else None,
                    "update_time": model.update_time.ToDatetime().isoformat() 
                                  if model.update_time else None,
                })
            ...
            raise map_to_migration_error(
                e, VertexAIError, "Failed to get model list"
            )
