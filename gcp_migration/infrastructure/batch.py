...
from gcp_migration.domain.exceptions_fixed import (
    BatchProcessingError, MigrationError, TimeoutError, 
...
from gcp_migration.infrastructure.resilience import CircuitBreakerRegistry, with_circuit_breaker
    ...
    async def _processor_loop(self) -> None:
        ...
        while self._state != BatchState.STOPPING:
            try:
                ...
                try:
                    flush_wait = max(0, self.flush_interval - (time.monotonic() - last_flush_time))
                ...
                should_process = (
                    current_size >= self.min_batch_size or
    ...
    async def _process_batch(self) -> None:
        ...
        try:
            ...
            if self._circuit:
                ...
                results = await self._circuit.execute(
                    self.batch_processor, batch_items
                )
            else:
                # Direct execution
            ...
            for item in batch:
                if item.retry_count < self.max_retry_count:
                    ...
                    failed_count += 1
                    item.fail(BatchProcessingError(
...
class BatchProcessorRegistry:
    ...
    def get_all_metrics(cls) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {name: processor.metrics.get_stats() 
