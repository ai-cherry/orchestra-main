...
from gcp_migration.domain.exceptions_fixed import (
    CacheError, DataConsistencyError, MigrationError, safe_async_operation
)
        ...
        try:
            async with self._lock:
                ...
                self._metrics.record_write(
                    (time.monotonic() - start_time) / len(items)
                )
            ...
            for i, cache in enumerate(self.caches):
                ...
                if value is not None:
                    ...
                    if i > 0:
                        fill_tasks = []
                        for j in range(i):
                            fill_tasks.append(self.caches[j].set(
        ...
        try:
            ...
            for i, cache in enumerate(self.caches):
                ...
                if i > 0 and level_values:
                    fill_tasks = []
                    for j in range(i):
                        fill_tasks.append(self.caches[j].set_many(
                            level_values, self.default_ttl
                        ))
...
class CacheRegistry:
    ...
    def get_all_metrics(cls) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {name: cache.metrics.get_stats() 
