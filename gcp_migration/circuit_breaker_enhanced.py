...
class MetricsCollector:
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_rejection": self.last_rejection_time.isoformat()
...
class CircuitBreakerStorage:
    ...
    @staticmethod
    def save_state(
        name: str, state: Dict[str, Any], storage_dir: Optional[Path] = None
    ) -> bool:
    ...
    @staticmethod
    def load_state(
        name: str, storage_dir: Optional[Path] = None
    ...
    def _load_state(self) -> None:
        ...
        if state:
            try:
                ...
                logger.info(
                    f"Loaded circuit breaker state for '{self.name}': {self.state.value}"
                )
...
class CircuitContext:
    ...
    def __enter__(self) -> "CircuitContext":
        ...
        if not self.breaker.allow_request():
            until = (
                self.breaker.last_failure_time
                + timedelta(seconds=self.breaker.recovery_timeout)
    ...
    def decorator(func: F) -> F:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            context = CircuitContext(
                circuit, details=f"Async function: {func.__name__}"
            )
            try:
                with context:
...
def generate_report(output_file: Optional[str] = None) -> str:
    ...
    if not metrics:
        ...
        report_lines.append("")
        report_lines.append(
            "| Circuit | State | Success Rate | Failures | Avg Response Time |"
        )
        report_lines.append(
            "|---------|-------|--------------|----------|-------------------|"
        )
        ...
        for name, metric in metrics.items():
            ...
            report_lines.append(
                f"| {name} | {state_emoji} {state} | {success_rate:.1f}% | {failures} | {avg_time_str} |"
            )
        ...
        for name, metric in metrics.items():
            ...
            report_lines.append(
                f"**Failure Count:** {metric['failure_count']}/{metric['failure_threshold']}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--failure-rate", type=float, default=0.7, help="Failure rate for test"
    )
    parser.add_argument(
        "--iterations", type=int, default=20, help="Iterations for test"
    )
    parser.add_argument(
        "--delay", type=float, default=0.5, help="Delay between iterations"
    )
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate report after test"
    )
