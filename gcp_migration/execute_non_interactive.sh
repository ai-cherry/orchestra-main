#!/bin/bash
#
# AI Orchestra GCP Migration - Non-Interactive Mode
#
# This version uses pre-existing credentials and non-interactive auth
#
# Author: Roo
...
# Print banner
echo -e "${BOLD}${BLUE}AI Orchestra GCP Migration - NON-INTERACTIVE EXECUTION${NC}"
echo -e "${BLUE}===============================================${NC}"
...
# Step 2: Configure GCP - Non-Interactive method
step_configure_gcp() {
  log "STEP" "2. Configuring GCP (non-interactive mode)"
  
  # Check if already authenticated
  log "INFO" "Checking GCP authentication status..."
  if ACTIVE_ACCOUNT=$(gcloud auth list --filter=status:ACTIVE --format="value(account)" 2>/dev/null | grep -q '@'); then
    log "INFO" "Already authenticated with GCP."
  else
    log "WARNING" "No active GCP account found. Using non-interactive auth."
    # Try to use environment authentication if available
    run_command "gcloud auth activate-service-account --key-file=${GOOGLE_APPLICATION_CREDENTIALS}" "Activate service account from environment" || {
      log "WARNING" "Could not activate service account from environment. Using default credentials."
    }
  fi
  ...
  log "INFO" "GCP configuration completed."
}
...
execute_migration() {
  log "INFO" "Starting AI Orchestra migration to GCP (non-interactive version)..."
  ...
  step_configure_gcp || log "ERROR" "Failed at GCP configuration step"
