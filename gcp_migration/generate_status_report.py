...
class StatusReportGenerator:
    ...
    def generate_report(self, include_metrics: bool = True) -> str:
        ...
        in_progress_phases = sum(
            1 for p in phases.values() if p.get("status") == "IN_PROGRESS"
        )
        ...
        if include_metrics and metrics_data:
            ...
            if vector_metrics:
                ...
                report.append("")
                report.append(
                    f"* **Average Latency**: {vector_metrics.get('avg_latency_ms', 'N/A')} ms"
                )
                report.append(
                    f"* **P95 Latency**: {vector_metrics.get('p95_latency_ms', 'N/A')} ms"
                )
                report.append(
                    f"* **Throughput**: {vector_metrics.get('throughput', 'N/A')} queries/sec"
                )
                report.append("")
            ...
            if api_metrics:
                ...
                report.append("")
                report.append(
                    f"* **Average Response Time**: {api_metrics.get('avg_response_time_ms', 'N/A')} ms"
                )
                report.append(
                    f"* **Cold Start Latency**: {api_metrics.get('cold_start_latency_ms', 'N/A')} ms"
                )
                report.append("")
            ...
            if circuit_metrics:
                ...
                report.append(
                    f"* **Failures**: {circuit_metrics.get('failure_count', 'N/A')}"
                )
                report.append(
                    f"* **Success Rate**: {circuit_metrics.get('success_rate', 'N/A')}%"
                )
                report.append("")
    ...
    def send_report_by_email(
        ...
        try:
            ...
            if subject is None:
                subject = (
                    f"AI Orchestra GCP Migration Status Report - {current_phase} Phase"
                )
...
def setup_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--path", type=str, default="reports", help="Output directory for file output"
    )

    parser.add_argument("--bucket", type=str, help="GCS bucket name for GCS output")
    ...
    parser.add_argument(
        "--smtp-port", type=int, default=25, help="SMTP port for email output"
    )

    parser.add_argument("--smtp-user", type=str, help="SMTP username for email output")

    parser.add_argument(
        "--smtp-password", type=str, help="SMTP password for email output"
    )
    ...
    parser.add_argument("--subject", type=str, help="Email subject for email output")
