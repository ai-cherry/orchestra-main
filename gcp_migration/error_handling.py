...
class MigrationError(Exception):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "traceback": self.traceback_info if self.traceback_info != "None\n" else None,
        }
    ...
    def __init__(
        ...
        **kwargs
        ...
        if status_code:
            ...
            is_transient = status_code in (408, 429) or (status_code >= 500 and status_code < 600)
...
def classify_exception(exception: Exception) -> MigrationError:
    ...
    elif any(net_err in str(type(exception).__name__.lower()) for net_err in ["connection", "timeout", "network"]):
        return NetworkError(
            message=str(exception),
...
if __name__ == "__main__":
    ...
    def flaky_function(succeed_on_attempt: int, current_attempts: list):
        ...
        if current_attempts[0] < succeed_on_attempt:
            raise NetworkError("Temporary network issue", error_code="TEST_NETWORK_ERROR")
        return "Success!"
