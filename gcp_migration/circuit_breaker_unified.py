...
class CircuitOpenError(CircuitBreakerError):
    ...
    def __init__(
        self, breaker_name: str, until: datetime, original_exceptions: List[Exception]
    ):
    ...
    def reset(self) -> None:
        ...
        with self._lock:
            ...
            if old_state != CircuitState.CLOSED:
                ...
                logger.info(
                    f"Circuit breaker '{self.name}' reset from {old_state.value} to CLOSED"
                )
    ...
    def _get_recovery_time(self) -> Optional[datetime]:
        ...
        with self._lock:
            if self._state == CircuitState.OPEN and self._last_failure_time is not None:
                return self._last_failure_time + timedelta(
        ...
        if not self._allow_request():
            ...
            if self.fallback_function:
                try:
                    ...
                    logger.error(
                        f"Fallback function for circuit '{self.name}' failed: {fallback_error}"
                    )
