...
def map_to_migration_error(
    exception: Exception, target_type: Type[T], message: Optional[str] = None, **kwargs
) -> T:
    """
...
def map_gcp_error(
    ...
    if "not found" in error_message.lower() or error_type.endswith("NotFoundError"):
    ...
    elif "already exists" in error_message.lower() or error_type.endswith(
        "AlreadyExistsError"
    ):
        ...
        return AuthorizationError(
            message=f"Access denied: {error_message}", cause=exception, details=details
        )
        ...
        "unauthenticated" in error_message.lower()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if isinstance(exc_val, MigrationError):
            ...
            mapped_error = map_gcp_error(
                exc_val, service_name=self.service_name, operation=self.operation_name
            )
        else:
        ...
        if self.logger:
            self.logger.error(
                f"Error in {self.operation_name}: {mapped_error}", exc_info=True
            )
