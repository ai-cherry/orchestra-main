...
class RollbackTool:
    ...
    def _rollback_to_checkpoint(self) -> int:
        ...
        logger.info(
            f"Rolling back to checkpoint: {self.checkpoint_id} (Phase: {checkpoint['phase']})"
        )
    ...
    def _rollback_phase(self, phase: MigrationPhase) -> int:
        ...
        rollback_checkpoint_id = (
            f"rollback_{phase.value}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        )
    ...
    def _rollback_core_infrastructure(self) -> bool:
        ...
        if tf_dir.exists():
            ...
            if not self._run_command(
                ["terraform", "destroy", "-auto-approve"], cwd=tf_dir
            ):
                return False
    ...
    def _rollback_memory_system(self) -> bool:
        ...
        vector_indices_backup = (
            self.base_dir / "checkpoints" / "vector_indices_backup.sql"
        )
    ...
    def _rollback_ai_coding(self) -> bool:
        ...
        gemini_config_backup = (
            self.base_dir / "checkpoints" / "gemini_config_backup.yaml"
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="AI Orchestra GCP Migration Rollback Tool"
    )
    ...
    if args.list_checkpoints:
        ...
        for checkpoint in rollback_tool.state["checkpoints"]:
            print(
                f"  {checkpoint['id']} - {checkpoint['phase']} ({checkpoint['timestamp']})"
            )
        return 0
    ...
    if not args.force:
        if args.phase:
            confirm = input(
                f"Are you sure you want to rollback the {args.phase} phase? (y/n): "
            )
        elif args.to_checkpoint:
            confirm = input(
                f"Are you sure you want to rollback to checkpoint {args.to_checkpoint}? (y/n): "
            )
        else:
