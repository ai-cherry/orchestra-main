...
class GitHubSecretManager:
    ...
    def list_secrets(
        ...
        try:
            ...
            if self.sync_level == SyncLevel.REPOSITORY or repo_name:
                ...
                if not repo_name:
                    logger.error("Repository name is required for repository-level secrets")
                    return secrets
            ...
            if self.sync_level == SyncLevel.ENVIRONMENT or environment:
                ...
                if not repo_name:
                    logger.error("Repository name is required for environment-level secrets")
                    return secrets
                
                if not environment:
                    logger.error("Environment name is required for environment-level secrets")
                    return secrets
    ...
    def get_public_key(
        ...
        """
        try:
            if self.sync_level == SyncLevel.ORGANIZATION and not repo_name and not environment:
    ...
    def create_or_update_secret(
        ...
        try:
            ...
            if self.sync_level == SyncLevel.ORGANIZATION and not secret.repo_name and not secret.environment:
    ...
    def delete_secret(
        ...
        try:
            ...
            if response.status_code == 204 or response.status_code == 404:
                logger.info(f"Successfully deleted GitHub secret (or it didn't exist): {secret.name}")
    ...
    def list_secrets(
        ...
        try:
            ...
            for secret in self.client.list_secrets(request={"parent": parent}):
                ...
                if filter_labels:
                    if not all(secret.labels.get(k) == v for k, v in filter_labels.items()):
                        continue
...
class SecretSynchronizer:
    ...
    def __init__(
        ...
        self.gcp_manager = GCPSecretManager(
            gcp_auth=gcp_auth,
            auditor=self.auditor
        )
    ...
    def _filter_secrets(self, secret_name: str) -> bool:
        ...
        if self.config.include_patterns:
            return any(pattern in secret_name for pattern in self.config.include_patterns)
        
        # If exclude patterns are specified, exclude secrets matching those patterns
        if self.config.exclude_patterns:
            return not any(pattern in secret_name for pattern in self.config.exclude_patterns)
    ...
    def sync_github_to_gcp(
        ...
        # Filter secrets
        github_secrets = [
            secret for secret in github_secrets
            if self._filter_secrets(secret.name)
        ]
        ...
        for github_secret in github_secrets:
            ...
            logger.info(f"Processing GitHub secret: {github_secret.name} -> GCP: {gcp_secret_name}")
            ...
            if self.config.dry_run:
                logger.info(f"[DRY RUN] Would create/update GCP secret: {gcp_secret_name}")
            ...
            try:
                ...
                if gcp_secret_name in gcp_secret_map:
                    logger.info(f"GCP secret already exists: {gcp_secret_name}, updating labels only")
                    
                    # Get the existing secret value
                    existing_value = self.gcp_manager.get_secret_value(gcp_secret_map[gcp_secret_name])
                    
                    # Use the existing value if available, otherwise use placeholder
                    secret_value = existing_value if existing_value else placeholder_value
                else:
                ...
                logger.error(f"Error syncing GitHub secret {github_secret.name} to GCP: {e}")
                results.append(SyncResult(
                    success=False,
                    source=f"GitHub:{github_secret.name}",
                    target=f"GCP:{gcp_secret_name}",
    ...
    def sync_gcp_to_github(
        ...
        # Filter secrets
        gcp_secrets = [
            secret for secret in gcp_secrets
            if self._filter_secrets(secret.name)
        ]
        ...
        for gcp_secret in gcp_secrets:
            ...
            logger.info(f"Processing GCP secret: {gcp_secret.name} -> GitHub: {github_secret_name}")
            ...
            # If it's a dry run, just log what would happen
            if self.config.dry_run:
                logger.info(f"[DRY RUN] Would create/update GitHub secret: {github_secret_name}")
                results.append(SyncResult(
                    success=True,
            ...
            try:
                ...
                if not secret_value:
                    logger.error(f"Failed to get value for GCP secret: {gcp_secret.name}")
                    results.append(SyncResult(
                        success=False,
                        source=f"GCP:{gcp_secret.name}",
                        target=f"GitHub:{github_secret_name}",
                ...
                if "PLACEHOLDER" in secret_value:
                    logger.warning(f"GCP secret {gcp_secret.name} contains a placeholder value, skipping sync to GitHub")
                    results.append(SyncResult(
                        success=True,
                        source=f"GCP:{gcp_secret.name}",
                        target=f"GitHub:{github_secret_name}",
                ...
                visibility = None
                if self.config.sync_level == SyncLevel.ORGANIZATION and github_secret_name in github_secret_map:
                ...
                logger.error(f"Error syncing GCP secret {gcp_secret.name} to GitHub: {e}")
                results.append(SyncResult(
                    success=False,
                    source=f"GCP:{gcp_secret.name}",
                    target=f"GitHub:{github_secret_name}",
    ...
    def sync(
        ...
        if self.config.sync_direction == SyncDirection.GITHUB_TO_GCP:
            return self.sync_github_to_gcp(
                repo_name=repo_name,
                environment=environment
            )
        elif self.config.sync_direction == SyncDirection.GCP_TO_GITHUB:
            return self.sync_gcp_to_github(
                repo_name=repo_name,
                environment=environment
            )
        else:  # BIDIRECTIONAL
...
def main():
    ...
    parser.add_argument("--gcp-service-account-key", help="GCP service account key JSON")
    ...
    # Sync options
    parser.add_argument("--direction", choices=[d.value for d in SyncDirection], 
                      default=SyncDirection.BIDIRECTIONAL.value, help="Sync direction")
    parser.add_argument("--level", choices=[l.value for l in SyncLevel], 
                      default=SyncLevel.ORGANIZATION.value, help="Sync level")
    parser.add_argument("--secret-prefix", default="github-", help="Prefix for GCP secrets from GitHub")
    parser.add_argument("--include", action="append", help="Secret name patterns to include")
    parser.add_argument("--exclude", action="append", help="Secret name patterns to exclude")
    parser.add_argument("--dry-run", action="store_true", help="Don't actually perform sync")
