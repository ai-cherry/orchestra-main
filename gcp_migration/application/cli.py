...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    if not path.exists():
        raise click.FileError(
            config_path,
            hint=f"The configuration file {config_path} does not exist."
        )
    ...
    try:
        if path.suffix == ".json":
            ...
            raise click.FileError(
                config_path,
                hint="Configuration file must be JSON or YAML."
            )
    except Exception as e:
        raise click.FileError(
            config_path,
            hint=f"Failed to load configuration: {e}"
        )
...
@click.option(
    "--skip-validation", 
    is_flag=True, 
    help="Skip validation before execution"
)
@click.option(
    "--parallel/--sequential", 
    default=True, 
...
def validate(config: str, output: str, verbose: bool):
    ...
    try:
        ...
        # Run validation
        result = asyncio.run(
            validate_migration(config_data)
        )
...
def init_config(output: str, github_repo: Optional[str], gcp_project: Optional[str], migration_type: str):
...
@click.option(
    "--name", "-n", 
    required=True, 
    help="Name for the workstation cluster"
)
@click.option(
    "--output", "-o", 
...
def generate_workstation_config(project: Optional[str], location: str, name: str, output: str):
    ...
    if not project:
        ...
        if not project:
            ...
            console.print("Either specify --project or set GCP_PROJECT environment variable")
            sys.exit(1)
    ...
    if path.suffix == ".json":
        ...
        with open(f"{path}.tf", "w") as f:
            ...
            f.write('  workstation_cluster_id  = google_workstations_workstation_cluster.main.workstation_cluster_id\n')
            f.write('  location               = var.region\n')
            f.write('  host {\n')
    ...
    console.print(f"[bold green]Workstation configuration generated:[/bold green] {path}")
    console.print("Use this configuration with Terraform to create a Cloud Workstation cluster.")
