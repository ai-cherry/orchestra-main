...
class FunctionalStep(WorkflowStep[T, R]):
    ...
    def __init__(self, func: Callable[[T], R], **kwargs):
        """
...
class Workflow(Generic[T, R]):
        ...
        while remaining_steps and ready_steps:
            ...
            while retry <= step.retry_count:
                try:
                    if retry > 0:
                        logger.info(
                            f"Retrying step {step.name} (attempt {retry}/{step.retry_count})"
                        )
    ...
    async def validate(self, context: T) -> ValidationResult:
            ...
            for step_id in graph:
                if step_id not in visited:
                    ...
                    if has_cycle:
                        ...
                        checks.append({"name": "circular_dependency", "result": True})
        ...
        return ValidationResult(
            valid=len(errors) == 0, checks=checks, errors=errors, warnings=warnings
        )
...
class MigrationWorkflow(Workflow[MigrationContext, MigrationResult]):
    ...
    async def execute_migration(self, context: MigrationContext) -> MigrationResult:
        """
    ...
    def _collect_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "completed_steps": sum(
                1 for r in self.results.values() if r.status == StepStatus.COMPLETED
            ),
            "failed_steps": sum(
                1 for r in self.results.values() if r.status == StepStatus.FAILED
            ),
            "skipped_steps": sum(
                1 for r in self.results.values() if r.status == StepStatus.SKIPPED
            ),
            "step_durations": {
            ...
            "total_duration": sum(
                result.duration_seconds or 0 for result in self.results.values()
            ),
        }
...
class ValidateMigrationPlan(WorkflowStep[MigrationContext, ValidationResult]):
    ...
    async def execute(self, context: MigrationContext) -> ValidationResult:
        ...
        if not context.source:
            errors.append(
                {"type": "ValidationError", "message": "Migration source is required"}
            )
        else:
            checks.append({"name": "source_validation", "result": True})
        ...
        if not context.destination:
            ...
            checks.append({"name": "destination_validation", "result": True})

        # Check resources
        if not context.resources:
            errors.append(
                {"type": "ValidationError", "message": "No resources to migrate"}
            )
        else:
            checks.append({"name": "resources_validation", "result": True})
...
class UpdateResourceStatus(
    WorkflowStep[Tuple[MigrationContext, str, MigrationStatus], None]
):
    """Step to update the status of a resource."""

    async def execute(
        self, context: Tuple[MigrationContext, str, MigrationStatus]
    ) -> None:
        """
...
class ResourceMigrationStep(WorkflowStep[MigrationContext, MigrationResource]):
    ...
    async def _migrate_resource(
        self, context: MigrationContext, resource: MigrationResource
    ) -> None:
        """
