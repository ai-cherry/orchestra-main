...
class PerformanceBenchmark:
    ...
    async def run_benchmarks(self) -> None:
        """Run all benchmarks."""
        logger.info(
            f"Starting performance benchmarks with {self.iterations} iterations"
        )
    ...
    async def _benchmark_memory_operations(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "simple_mcp module not available, skipping memory benchmarks"
                )
    ...
    async def _benchmark_vector_search(self) -> List[BenchmarkResult]:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "AlloyDB connection not available, using simulated vector search"
                )
                db_available = False
            ...
            if in_workstation:
                ...
                if Path(startup_log_path).exists():
                    ...
                    start_match = re.search(
                        r"Starting workstation at: (\d+)", log_content
                    )
                    ...
                    if start_match and end_match:
                        ...
                        return BenchmarkResult(
                            ...
                            details={
                                "baseline_ms": self.baseline_data[
                                    "workstation_startup"
                                ],
    ...
    def generate_report(self) -> None:
        ...
        if not successful_results:
            ...
            overall_score = sum(r.improvement_pct for r in successful_results) / len(
                successful_results
            )
        ...
        # Print summary
        logger.info(
            f"Benchmark complete. Overall score: {overall_score:.1f}% ({overall_grade})"
        )
        logger.info(
            f"Detailed report saved to benchmark_results.json and PERFORMANCE_REPORT.md"
        )

        # Print console summary
        print(f"\n{BOLD}========== PERFORMANCE BENCHMARK SUMMARY =========={NC}")
        print(
            f"Overall Performance: {BOLD}{self._get_color_for_score(overall_score)}{overall_score:.1f}%{NC} (Grade: {BOLD}{overall_grade}{NC})"
        )
        print("\nKey Metrics:")
    ...
    def _generate_markdown_report(
        self, overall_score: float, overall_grade: str
    ) -> str:
...
"""
        ...
        for result in self.results:
            if not result.success:
                ...
                baseline = (
                    result.details.get("baseline_ms", "-") if result.details else "-"
                )
                if baseline != "-":
