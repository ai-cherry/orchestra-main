...
class Settings(BaseModel):
    ...
    def terraform_var_map(self) -> Dict[str, Union[str, bool, int]]:
        ...
        if self.docker_registry:
            container_image = (
                f"{self.docker_registry}/{self.app_name}:{self.docker_image_tag}"
            )
            var_map["container_image"] = container_image
        # Add machine types if available
        if "standard" in self.workstation_machine_types:
            var_map["standard_machine_type"] = self.workstation_machine_types[
                "standard"
            ]
...
def load_settings() -> Settings:
    ...
    for field_name in Settings.model_fields:
        ...
        if env_key in env_vars:
            ...
            if field_info.annotation == bool:
                settings_fields[field_name] = value.lower() in (
                    "true",
                    "t",
                    "yes",
                    "y",
    ...
    if "docker_registry" not in settings_fields and "gcp_project_id" in settings_fields:
        ...
        settings_fields[
            "docker_registry"
        ] = f"{region}-docker.pkg.dev/{project_id}/workstations"
    ...
    try:
        ...
        if "docker_registry" in str(e):
            ...
            print(
                "2. Set environment variable: export DOCKER_REGISTRY=gcr.io/your-project-id"
            )
            print("\nExample Docker registry formats:")
            print("- Google Container Registry: gcr.io/your-project-id")
            print(
                "- Artifact Registry: region-docker.pkg.dev/your-project-id/repository-name"
            )
            print("===========================\n")
        raise
