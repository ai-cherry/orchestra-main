...
def setup_service_account_auth(service_account_json: str) -> Dict[str, str]:
    ...
    exit_code, output = run_command("gcloud auth activate-service-account --key-file=\"$GOOGLE_APPLICATION_CREDENTIALS\"", env=env)
    if exit_code != 0:
...
def check_org_policies(project_id: str, env: Dict[str, str]) -> List[str]:
    ...
    # List policies
    exit_code, output = run_command(f"gcloud org-policies list --project={project_id} --format=json", env=env)
    if exit_code != 0:
    ...
    for constraint in constraints_to_check:
        ...
        if any(constraint_name in policy_name for policy_name in policy_names):
            ...
            # Get policy details
            run_command(f"gcloud org-policies describe {constraint} --project={project_id}", env=env)
...
def fix_cloud_run_policy(project_id: str, env: Dict[str, str]) -> bool:
    ...
    with open(policy_path, "w") as f:
        f.write(f"""name: projects/{project_id}/policies/run.requireInvokerIam
spec:
  rules:
  - enforce: false
""")
    ...
    # Apply the policy
    exit_code, output = run_command(f"gcloud org-policies set-policy {policy_path}", env=env)
    if exit_code != 0:
...
def fix_vertex_ai_policy(project_id: str, env: Dict[str, str]) -> bool:
    ...
    with open(models_policy_path, "w") as f:
        f.write(f"""name: projects/{project_id}/policies/vertexai.allowedModels
spec:
  rules:
  - values:
      allowedValues:
...
""")
    ...
    with open(genai_policy_path, "w") as f:
        f.write(f"""name: projects/{project_id}/policies/vertexai.allowedGenAIModels
spec:
  rules:
  - values:
      allowedValues:
...
""")
    ...
    exit_code, output = run_command(f"gcloud org-policies set-policy {models_policy_path}", env=env)
    if exit_code != 0:
    ...
    exit_code, output = run_command(f"gcloud org-policies set-policy {genai_policy_path}", env=env)
    if exit_code != 0:
...
def fix_allowed_member_domains_policy(project_id: str, env: Dict[str, str]) -> bool:
    ...
    with open(policy_path, "w") as f:
        f.write(f"""name: projects/{project_id}/policies/iam.allowedPolicyMemberDomains
spec:
  rules:
  - values:
      allowedValues:
...
def update_cloud_run_service(project_id: str, service_name: str, region: str, env: Dict[str, str]) -> bool:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Organization Policy Manager for AI Orchestra GCP Migration")
    parser.add_argument(
        "--json-secret", 
    ...
    parser.add_argument("--service-name", default="ai-orchestra-minimal", help="Cloud Run service name")
    ...
    if not args.json_secret:
        print("Error: Service account JSON is required. Please provide it via --json-secret or GCP_ORGANIZATION_POLICY_JSON env var.")
        return 1
    ...
    if "vertexai.allowedModels" in active_policies or "vertexai.allowedGenAIModels" in active_policies:
    ...
    if not update_cloud_run_service(args.project_id, args.service_name, args.region, env):
        success = False
