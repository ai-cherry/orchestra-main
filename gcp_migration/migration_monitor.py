...
class MigrationResource:
    ...
    def update_status(
        self, new_status: ResourceStatus, details: Optional[Dict[str, Any]] = None
    ):
    ...
    def _record_status_change(
        self, status: ResourceStatus, details: Optional[Dict[str, Any]] = None
    ):
...
class MigrationMonitor:
        ...
        if state_path.exists():
            try:
                ...
                for resource_data in state.get("resources", []):
                    ...
                    if last_updated_str:
                        try:
                            resource.last_updated = datetime.datetime.fromisoformat(
                                last_updated_str
                            )
                        except ValueError:
    ...
    def display_status(self):
        ...
        console.print()
        console.print(
            f"[bold blue]AI Orchestra GCP Migration Status[/bold blue]",
            justify="center",
        )
        console.print(
            f"Project: [green]{self.project_id}[/green]  Region: [green]{self.region}[/green]"
        )
        ...
        status_color = (
            "green" if completed == 100 else "yellow" if completed > 50 else "red"
        )
        console.print(
            f"Progress: [bold {status_color}]{completed:.1f}%[/bold {status_color}]"
        )

        bar_width = 50
        completed_chars = int(bar_width * (completed / 100))
        console.print(
            f"[{status_color}]{'■' * completed_chars}{'□' * (bar_width - completed_chars)}[/{status_color}]"
        )

        console.print()
        console.print(
            f"Status: [bold {'green' if status['status'] == 'COMPLETED' else 'yellow'}]{status['status']}[/bold]"
        )
        ...
        if status["recent_notifications"]:
            ...
            for notification in status["recent_notifications"]:
                ...
                console.print(
                    f"[{level_style}]{notification['level']}[/{level_style}] "
    ...
    def generate_report(self, output_file: Optional[str] = None) -> str:
        ...
        with open(report_file, "w") as f:
            ...
            if self.resources:
                ...
                for resource in sorted(self.resources.values(), key=lambda r: r.name):
                    f.write(
                        f"| {resource.name} | {resource.type} | {resource.status.value.upper()} | "
            ...
            if self.notifications:
                ...
                for notification in self.notifications[
                    ...
                    f.write(
                        f"| {notification['timestamp']} | {notification['level']} | {notification['message']} |\n"
                    )
            ...
            if status["failed_resources"] > 0:
                f.write(
                    "- **Critical:** Address failed resources before continuing migration\n"
                )
...
async def main():
    ...
    parser.add_argument(
        "--add-resource", action="store_true", help="Add a resource to monitor"
    )
