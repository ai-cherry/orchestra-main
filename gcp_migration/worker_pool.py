...
class WorkerPool:
    ...
    def __init__(
        ...
        self.task_queue: queue.PriorityQueue[PrioritizedTask] = queue.PriorityQueue(
            max_queue_size
        )
    ...
    def wait_all(
        ...
        if remaining_ids and timeout is not None:
            raise TimeoutError(
                f"Tasks {remaining_ids} did not complete within {timeout} seconds"
            )
...
if __name__ == "__main__":
    ...
    with worker_pool_context(num_workers=args.num_workers) as pool:
        ...
        for i in range(args.num_tasks):
            ...
            logger.info(
                f"Submitted task {i} with ID {task_id} and priority {priority.name}"
            )
