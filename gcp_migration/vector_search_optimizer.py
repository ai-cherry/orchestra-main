...
class VectorSearchConfig:
    ...
    nprobe: int = 50  # Number of clusters to visit during search (higher = better recall, slower)
...
class VectorSearchOptimizer:
    ...
    def __init__(self, config: Optional[VectorSearchConfig] = None):
        ...
        logger.info(f"Initialized vector search optimizer with IVF lists={self.config.ivf_lists}")
    ...
    def generate_optimized_sql(
        ...
        if self.config.prefetch_distance > 0:
            query = f"/*+ SeqScan(memories) Prefetch({self.config.prefetch_distance}) */ \n" + query
    ...
    def generate_index_creation_sql(self, table: str = "memories", column: str = "vector") -> str:
        """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Vector Search Optimizer for AI Orchestra")
    parser.add_argument(
        "--optimize-alloydb",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="AlloyDB cluster name"
    )
    parser.add_argument(
        "--instance",
        default="main-instance",
        help="AlloyDB instance name"
    )
    parser.add_argument(
