        ...
        for dir_path in self._temp_dirs:
            if dir_path.exists():
                try:
                    ...
                    logger.error(
                        f"Error cleaning up temporary directory {dir_path}: {e}"
                    )
...
class GCPAuth:
    ...
    def _authenticate_with_key(self) -> bool:
        ...
        if result.returncode != 0:
            logger.error(
                f"Failed to authenticate with service account key: {result.stderr}"
            )
            return False
    ...
    def _authenticate_with_impersonation(self) -> bool:
        ...
        if result.returncode != 0:
            logger.error(
                f"Failed to configure service account impersonation: {result.stderr}"
            )
            return False

        logger.info(
            f"Successfully configured impersonation of {self.service_account_email}"
        )
        return True
    ...
    def _authenticate_with_workload_identity(self) -> bool:
        ...
        if not self.workload_identity_provider:
            raise ValueError(
                "Workload identity provider is required for WIF authentication"
            )
        ...
        if self.service_account_email:
            ...
            if result.returncode != 0:
                logger.error(
                    f"Failed to configure service account impersonation for WIF: {result.stderr}"
                )
                return False
    ...
    def _authenticate_with_application_default(self) -> bool:
        ...
        if result.returncode == 0:
            ...
            if self.service_account_key:
                ...
                if result.returncode != 0:
                    logger.error(
                        f"Failed to authenticate with application default credentials: {result.stderr}"
                    )
                    return False
            else:
                logger.error(
                    "No service account key provided for application default authentication"
                )
                return False
...
def main():
    ...
    parser.add_argument(
        ...
        default=AuthMethod.SERVICE_ACCOUNT_KEY.value,
        help="Authentication method",
    )
    parser.add_argument("--service-account-key", help="Service account key JSON")
    parser.add_argument(
        "--service-account-email", help="Service account email for impersonation"
    )
    parser.add_argument(
        "--workload-identity-provider", help="Workload identity provider ID"
    )
    ...
    gcp_config = GCPConfig(
        project_id=args.project_id, region=args.region, zone=args.zone
    )
