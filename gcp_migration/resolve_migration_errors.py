...
class MigrationErrorResolver:
    ...
    def _classify_error(self, log_line: str) -> str:
        ...
        if any(pattern in log_lower for pattern in ["connection refused", "cannot connect", "timeout"]):
            return "connectivity"
        elif any(pattern in log_lower for pattern in ["permission denied", "access denied"]):
            return "permission"
        elif any(pattern in log_lower for pattern in ["out of memory", "memory allocation"]):
            return "memory"
            ...
            return "database"
        elif any(pattern in log_lower for pattern in ["configuration", "config", "invalid setting"]):
            return "config"
        elif any(pattern in log_lower for pattern in ["authentication", "unauthorized", "auth"]):
            return "auth"
        else:
            return "unknown"
    ...
    def repair_mcp_server(self) -> Dict[str, Any]:
        ...
        if status.get("port_in_use", False) and not status.get("running", False):
            try:
                ...
                if pid:
                    logger.info(f"Attempting to terminate process using port 8080 (PID {pid})")
                    ...
                    if term_result.returncode == 0:
                        ...
                        if restart_result.returncode == 0:
                            logger.info(f"Successfully started service after clearing port")
                            results["success"] = True
                            return results
            except Exception as e:
        ...
        try:
            service_logs = subprocess.run(
                ["sudo", "journalctl", "-u", self.mcp_service, "--no-pager", "-n", "50"],
                capture_output=True,
                text=True,
                check=False,
            )
            ...
            elif "configuration file" in log_output.lower() and "error" in log_output.lower():
    ...
    def generate_incident_report(self, issues: List[Dict[str, Any]], repairs: Dict[str, Any]) -> str:
        ...
        lines.append(f"- **Status**: {'Resolved' if repairs.get('success', False) else 'Unresolved'}")
        ...
        if actions:
            for i, action in enumerate(actions, 1):
                lines.append(f"{i}. **{action.get('action', 'Unknown').replace('_', ' ').title()}**")
                lines.append(f"   * Result: {'✅ Success' if action.get('success', False) else '❌ Failed'}")
        ...
        lines.append(f"Overall Status: {'✅ All checks passed' if integrity.get('all_passed', False) else '❌ Some checks failed'}")
        lines.append("")
        
        for check in integrity.get('checks', []):
            status = "✅ Passed" if check.get('passed', False) else "❌ Failed"
            lines.append(f"- **{check.get('check', 'Unknown').replace('_', ' ').title()}**: {status}")
        ...
        if repairs.get('success', False) and integrity.get('all_passed', False):
            ...
            lines.append("")
            lines.append("Recommended actions:")
            lines.append("1. Check service logs: `sudo journalctl -u mcp-server --no-pager`")
...
def main() -> int:
    ...
    parser.add_argument("--report-dir", default=INCIDENT_REPORT_DIR, help="Incident report directory")
    parser.add_argument("--service", default=DEFAULT_MCP_SERVICE, help="MCP service name")
    parser.add_argument("--check", action="store_true", help="Check for critical errors")
    ...
    parser.add_argument("--report", action="store_true", help="Generate incident report")
        ...
        if not args.json:
            ...
            if critical_issues:
                ...
                for i, issue in enumerate(critical_issues[:5], 1):  # Show up to 5
                    ...
                    print(f"   File: {issue.get('file', 'Unknown')}:{issue.get('line', 'Unknown')}")
    ...
    if (args.repair or args.all):
        ...
        if not args.json:
            ...
            print(f"Port Available: {'Yes' if server_status.get('port_available', False) else 'No'}")
            print()
    
    # Attempt repairs if needed
    if (args.repair or args.all) and (critical_issues or not server_status.get("running", False)):
        print("\n=== Repairing MCP Server ===\n")
        repair_results = resolver.repair_mcp_server()
        results["repair_results"] = repair_results
        
        if not args.json:
            print(f"Repair {'successful' if repair_results.get('success', False) else 'failed'}")
            ...
            if actions:
                ...
                for i, action in enumerate(actions, 1):
                    ...
                    print(f"{i}. {action.get('action', 'Unknown').replace('_', ' ').title()}: {status}")
    ...
    if args.verify or args.all:
        ...
        if not args.json:
            status = "All checks passed" if integrity_results.get("all_passed", False) else "Some checks failed"
            print(f"Overall Status: {status}")
            print()
            
            for check in integrity_results.get("checks", []):
                status = "✅ Passed" if check.get("passed", False) else "❌ Failed"
                print(f"- {check.get('check', 'Unknown').replace('_', ' ').title()}: {status}")
    ...
    if critical_issues and not repair_results.get("success", False):
        return 1  # Error
    elif repair_results.get("success", False) and not integrity_results.get("all_passed", False):
        return 2  # Warning
    else:
        return 0  # Success
