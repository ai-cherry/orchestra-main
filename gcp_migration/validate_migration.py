...
class ValidationReport:
        ...
        with open(path, "w") as f:
            ...
            for component in components:
                ...
                for result in component_results:
                    status = "✅ Pass" if result.success else "❌ Fail" if result.success is False else "⏳ Pending"
                    duration = f"{result.duration_seconds:.2f}s" if result.duration_seconds else "N/A"
                    error = result.error or "N/A"
                    f.write(f"| {result.test_name} | {status} | {duration} | {error} |\n")
            ...
            for i, result in enumerate(self.results, 1):
                ...
                f.write(f"### {i}. {result.component} - {result.test_name} ({status})\n\n")
    ...
    def print_summary(self) -> None:
        ...
        for component in components:
            component_results = [r for r in self.results if r.component == component]
            component_success = all(r.success for r in component_results if r.success is not None)
            component_status = "✅ PASSED" if component_success else "❌ FAILED"
            
            print(f"{component}: {component_status}")
            
            for result in component_results:
                status = "✓ PASSED" if result.success else "✗ FAILED" if result.success is False else "⋯ PENDING"
...
def main() -> int:
    ...
    Returns:
        Exit code
    """
    parser = argparse.ArgumentParser(description="AI Orchestra GCP Migration Validation")
