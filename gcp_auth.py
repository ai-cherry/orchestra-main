...
def check_auth_configured() -> bool:
    ...
    if ENV_CREDENTIALS_PATH in os.environ and os.path.isfile(
        os.environ[ENV_CREDENTIALS_PATH]
    ):
        logger.info(
            f"GOOGLE_APPLICATION_CREDENTIALS points to existing file: {os.environ[ENV_CREDENTIALS_PATH]}"
        )
        return True
...
def authenticate() -> bool:
    ...
    if ENV_CREDENTIALS_PATH in os.environ and os.path.isfile(
        ...
        logger.info(
            f"Already authenticated with credentials at {os.environ[ENV_CREDENTIALS_PATH]}"
        )
        return True
...
def get_storage_client():
    ...
    try:
        ...
        logger.error(
            "google-cloud-storage package not installed. Run: pip install google-cloud-storage"
        )
        return None
...
def get_bigquery_client():
    ...
    try:
        ...
        logger.error(
            "google-cloud-bigquery package not installed. Run: pip install google-cloud-bigquery"
        )
        return None
