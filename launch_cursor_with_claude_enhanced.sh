...
source ~/.Lambda_env_setup.sh 2>/dev/null || true
[ -f .env ] && source .env
...
if [ "${MCP_AUTO_START:-true}" = "true" ]; then
    ...
    python ~/cherry_ai-main/mcp_server/servers/Lambda_cloud_run_server.py \
        > logs/mcp/cloud_run.log 2>&1 &
    MCP_PIDS+=($!)

    # Add other servers as they're implemented
    # echo "Starting Secret Manager MCP server..."
    # python ~/cherry_ai-main/mcp_server/servers/Lambda_secret_manager_server.py \
