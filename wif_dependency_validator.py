...
class ValidationResult:
    ...
    def get_failed_script_validations(self) -> List[ScriptValidation]:
        ...
        return [
            validation
            for validation in self.script_validations
    ...
    def get_failed_workflow_validations(self) -> List[WorkflowValidation]:
        ...
        return [
            validation
            for validation in self.workflow_validations
            if validation.status == ValidationStatus.FAILED
        ]
...
class WIFDependencyValidator:
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized WIF dependency validator with base path: {self.base_path}"
        )
    ...
    def check_dependencies(self) -> List[DependencyCheck]:
        ...
        for dep_name, required_version in self.REQUIRED_DEPENDENCIES.items():
            logger.debug(
                f"Checking dependency: {dep_name} (required: {required_version})"
            )
            ...
            try:
                if dep_name == "gh":
                    ...
                    version_match = re.search(
                        r"gh version (\d+\.\d+\.\d+)", version_output
                    )
                elif dep_name == "gcloud":
                    version_output = subprocess.check_output(
                        ["gcloud", "--version"], text=True
                    )
                    version_match = re.search(
                        r"Google Cloud SDK (\d+\.\d+\.\d+)", version_output
                    )
                elif dep_name == "jq":
                    version_output = subprocess.check_output(
                        ["jq", "--version"], text=True
                    )
        ...
        logger.info(
            f"Dependency check complete. {len([r for r in results if r.status == ValidationStatus.PASSED])} passed, {len([r for r in results if r.status == ValidationStatus.FAILED])} failed."
        )
    ...
    def validate_scripts(self) -> List[ScriptValidation]:
        ...
        for script_path in self.WIF_SCRIPTS:
            ...
            try:
                ...
                if not content.startswith("#!/bin/bash") and not content.startswith(
                    ...
                    validation.issues.append(
                        f"Script {script_path} does not have a proper shebang"
                    )

                # Check for set -e (exit on error)
                if "set -e" not in content:
                    validation.issues.append(
                        f"Script {script_path} does not have 'set -e' to exit on error"
                    )

                # Check for hardcoded paths
                if re.search(r"\/(?:home|workspaces)\/[^\/]+\/", content):
                    validation.issues.append(
                        f"Script {script_path} contains hardcoded paths"
                    )

                # Check for proper error handling
                if "trap" not in content and script_path != "verify_wif_setup.sh":
                    validation.issues.append(
                        f"Script {script_path} does not have trap handlers for cleanup"
                    )
                ...
                if shutil.which("shellcheck"):
                    try:
                        subprocess.check_output(
                            ["shellcheck", "-x", str(full_path)],
                            stderr=subprocess.STDOUT,
                            text=True,
                        )
                    except subprocess.CalledProcessError as e:
                        validation.issues.append(
                            f"Shellcheck found issues in {script_path}:\n{e.output}"
                        )
            except Exception as e:
            ...
            if validation.issues:
                validation.status = ValidationStatus.FAILED
                validation.message = (
                    f"Script {script_path} has {len(validation.issues)} issues"
                )
            else:
        ...
        logger.info(
            f"Script validation complete. {len([r for r in results if r.status == ValidationStatus.PASSED])} passed, {len([r for r in results if r.status == ValidationStatus.FAILED])} failed."
        )
    ...
    def validate_workflows(self) -> List[WorkflowValidation]:
        ...
        for workflow_path in self.WIF_WORKFLOWS:
            ...
            try:
                ...
                if "permissions:" not in content or "id-token: write" not in content:
                    validation.issues.append(
                        f"Workflow {workflow_path} does not have required permissions for WIF"
                    )

                # Check for WIF authentication
                if "google-github-actions/auth" not in content:
                    validation.issues.append(
                        f"Workflow {workflow_path} does not use google-github-actions/auth for WIF"
                    )
                ...
                for secret in required_secrets:
                    if f"secrets.{secret}" not in content:
                        validation.issues.append(
                            f"Workflow {workflow_path} does not use required secret: {secret}"
                        )
                ...
                for action, version in self.REQUIRED_GH_ACTIONS.items():
                    if action not in content:
                        validation.issues.append(
                            f"Workflow {workflow_path} does not use required action: {action}"
                        )
                    elif version not in content:
                        validation.issues.append(
                            f"Workflow {workflow_path} does not use required version {version} of action: {action}"
                        )

                # Check for environment support
                if "environment:" not in content:
                    validation.issues.append(
                        f"Workflow {workflow_path} does not support different environments"
                    )

                # Check for proper error handling
                if "continue-on-error:" not in content:
                    validation.issues.append(
                        f"Workflow {workflow_path} does not have proper error handling"
                    )
                ...
                if shutil.which("actionlint"):
                    try:
                        subprocess.check_output(
                            ["actionlint", str(full_path)],
                            stderr=subprocess.STDOUT,
                            text=True,
                        )
                    except subprocess.CalledProcessError as e:
                        validation.issues.append(
                            f"Actionlint found issues in {workflow_path}:\n{e.output}"
                        )
            except Exception as e:
                validation.issues.append(
                    f"Error validating workflow {workflow_path}: {e}"
                )
            ...
            if validation.issues:
                validation.status = ValidationStatus.FAILED
                validation.message = (
                    f"Workflow {workflow_path} has {len(validation.issues)} issues"
                )
            else:
                validation.status = ValidationStatus.PASSED
        ...
        logger.info(
            f"Workflow validation complete. {len([r for r in results if r.status == ValidationStatus.PASSED])} passed, {len([r for r in results if r.status == ValidationStatus.FAILED])} failed."
        )
    ...
    def generate_report(self, output_path: Optional[str] = None) -> str:
        ...
        report = {
            ...
            if not self.validation_result.has_failures()
            else "failed",
            "dependency_checks": [
                {
                    "name": check.name,
    ...
    def generate_markdown_report(self, output_path: Optional[str] = None) -> str:
        ...
        if self.validation_result.test_results:
            report.append(
                f"- Status: {self.validation_result.test_results.get('status', 'unknown')}"
            )
            report.append(
                f"- Message: {self.validation_result.test_results.get('message', 'No message')}"
            )
        ...
        if self.validation_result.has_failures():
            ...
            if self.validation_result.get_failed_script_validations():
                ...
                for (
                    validation
                ) in self.validation_result.get_failed_script_validations():
                    for issue in validation.issues:
                        report.append(f"- {issue}")
                report.append("")
            ...
            if self.validation_result.get_failed_workflow_validations():
                ...
                for (
                    validation
                ) in self.validation_result.get_failed_workflow_validations():
                    for issue in validation.issues:
                        report.append(f"- {issue}")
                report.append("")
        else:
