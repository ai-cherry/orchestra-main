...
SERVICES = {
    "ai-orchestra-minimal": {"type": "orchestrator", "health_endpoint": "/health", "required": True},
    "web-scraping-agents": {"type": "webscraping", "health_endpoint": "/health", "required": True},
...
class OrchestraValidator:
    ...
    async def validate_infrastructure(self):
        ...
        try:
            ...
            self.results.append(ValidationResult("Infrastructure", "VPC Network", True, "VPC 'orchestra-vpc' exists"))
        except subprocess.CalledProcessError:
            self.results.append(
                ValidationResult("Infrastructure", "VPC Network", False, "VPC 'orchestra-vpc' not found")
            )
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Infrastructure", "Redis Instance", status == "READY", f"Redis status: {status}")
            )
        except subprocess.CalledProcessError:
            self.results.append(ValidationResult("Infrastructure", "Redis Instance", False, "Redis instance not found"))
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Infrastructure", "Artifact Registry", True, "Repository 'orchestra-repo' exists")
            )
        except subprocess.CalledProcessError:
            self.results.append(ValidationResult("Infrastructure", "Artifact Registry", False, "Repository not found"))

        # Check Service Directory namespace
        try:
            namespace_path = f"projects/{PROJECT_ID}/locations/{REGION}/namespaces/orchestra-ai"
            _ = self.sd_client.get_namespace(name=namespace_path)  # Just checking existence
            self.results.append(
                ValidationResult("Infrastructure", "Service Directory", True, "Namespace 'orchestra-ai' exists")
            )
        except gcp_exceptions.NotFound:
            self.results.append(
                ValidationResult("Infrastructure", "Service Directory", False, "Namespace 'orchestra-ai' not found")
            )
        ...
        for topic_id in PUBSUB_TOPICS:
            try:
                ...
                self.results.append(
                    ValidationResult("Infrastructure", f"Pub/Sub Topic '{topic_id}'", True, "Topic exists")
                )
            except subprocess.CalledProcessError:
                self.results.append(
                    ValidationResult("Infrastructure", f"Pub/Sub Topic '{topic_id}'", False, "Topic not found")
                )
    ...
    async def validate_services(self):
        ...
        for service_name, config in SERVICES.items():
            ...
            try:
                ...
                if not service_url:
                    self.results.append(
                        ValidationResult("Services", f"{service_name} deployment", False, "No URL found")
                    )
                    continue

                self.results.append(
                    ValidationResult("Services", f"{service_name} deployment", True, f"Deployed at {service_url}")
                )
                ...
                try:
                    ...
                    response = requests.get(health_url, headers={"Authorization": f"Bearer {token}"}, timeout=30)

                    if response.status_code == 200:
                        self.results.append(
                            ValidationResult("Services", f"{service_name} health", True, "Service is healthy")
                        )
                    else:
                        self.results.append(
                            ValidationResult(
                    ...
                    self.results.append(
                        ValidationResult("Services", f"{service_name} health", False, f"Health check failed: {str(e)}")
                    )

                # Check service account
                sa_email = service_data.get("spec", {}).get("template", {}).get("spec", {}).get("serviceAccountName")
                if sa_email:
                    expected_sa = f"orchestra-{config['type']}-sa@{PROJECT_ID}.iam.gserviceaccount.com"
                    self.results.append(
                        ValidationResult(
                            "Services", f"{service_name} service account", sa_email == expected_sa, f"Using: {sa_email}"
                        )
                    )
                ...
                if config["required"]:
                    self.results.append(
                        ValidationResult(
                            "Services", f"{service_name} deployment", False, "Service not found (required)"
                        )
                    )
                else:
                    self.results.append(
                        ValidationResult(
                            "Services", f"{service_name} deployment", True, "Service not deployed (optional)"
                        )
                    )
    ...
    async def validate_secrets(self):
        ...
        for secret_id in all_secrets:
            try:
                ...
                self.results.append(
                    ValidationResult("Secrets", f"Secret '{secret_id}'", True, "Secret exists and accessible")
                )
            except gcp_exceptions.NotFound:
                self.results.append(ValidationResult("Secrets", f"Secret '{secret_id}'", False, "Secret not found"))
            except Exception as e:
                self.results.append(
                    ValidationResult("Secrets", f"Secret '{secret_id}'", False, f"Error accessing secret: {str(e)}")
                )
    ...
    async def validate_integrations(self):
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Integrations", "Service Directory registrations", False, f"Failed to check: {str(e)}")
            )
        ...
        try:
            ...
            orchestra_dashboard = any(d.get("displayName") == "Orchestra AI Dashboard" for d in dashboards)
            self.results.append(
                ValidationResult(
                    "Integrations",
                    "Monitoring Dashboard",
                    orchestra_dashboard,
                    "Dashboard exists" if orchestra_dashboard else "Dashboard not found",
                )
            )
        except Exception:
            self.results.append(
                ValidationResult("Integrations", "Monitoring Dashboard", False, "Failed to check dashboards")
            )
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Integrations", "Alert Policies", False, "Failed to check alert policies")
            )

    async def validate_security(self):
        """Validate security configurations."""
        # Check service accounts exist
        service_accounts = ["orchestra-orchestrator-sa", "orchestra-webscraping-sa", "orchestra-admin-sa", "cicd-sa"]
        ...
        for sa_name in service_accounts:
            ...
            try:
                ...
                self.results.append(
                    ValidationResult("Security", f"Service Account '{sa_name}'", True, "Service account exists")
                )
            except subprocess.CalledProcessError:
                self.results.append(
                    ValidationResult("Security", f"Service Account '{sa_name}'", False, "Service account not found")
                )
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Security", "Workload Identity Pool", True, "GitHub WIF pool configured")
            )
        except subprocess.CalledProcessError:
            self.results.append(
                ValidationResult("Security", "Workload Identity Pool", False, "GitHub WIF pool not found")
            )
        ...
        try:
            ...
            self.results.append(
                ValidationResult("Security", "Redis Encryption", False, "Failed to check Redis encryption")
            )
...
if __name__ == "__main__":
    ...
    if not os.getenv("GOOGLE_APPLICATION_CREDENTIALS") and not os.getenv("GOOGLE_CLOUD_PROJECT"):
        console.print("[yellow]Warning: No GCP credentials detected. Ensure you're authenticated with gcloud.[/yellow]")
