...
class OrchestraValidator:
    ...
    async def validate_infrastructure(self):
        ...
        try:
            result = subprocess.run(
                ["gcloud", "compute", "networks", "describe", "orchestra-vpc", 
            ...
            self.results.append(ValidationResult(
                ...
                True, "VPC 'orchestra-vpc' exists"
            ))
        except subprocess.CalledProcessError:
            self.results.append(ValidationResult(
                "Infrastructure", "VPC Network",
                False, "VPC 'orchestra-vpc' not found"
            ))
        ...
        try:
            ...
            self.results.append(ValidationResult(
                "Infrastructure", "Redis Instance",
                False, "Redis instance not found"
            ))
        
        # Check Artifact Registry
        try:
            result = subprocess.run(
                ["gcloud", "artifacts", "repositories", "describe", "orchestra-repo",
                 "--location", REGION, "--project", PROJECT_ID, "--format", "json"],
                capture_output=True, text=True, check=True
            )
            self.results.append(ValidationResult(
                "Infrastructure", "Artifact Registry",
                True, "Repository 'orchestra-repo' exists"
            ))
        except subprocess.CalledProcessError:
            self.results.append(ValidationResult(
                "Infrastructure", "Artifact Registry",
        ...
        try:
            ...
            self.results.append(ValidationResult(
                ...
                True, "Namespace 'orchestra-ai' exists"
            ))
        except gcp_exceptions.NotFound:
            self.results.append(ValidationResult(
                "Infrastructure", "Service Directory",
                False, "Namespace 'orchestra-ai' not found"
            ))
        ...
        for topic_id in PUBSUB_TOPICS:
            try:
                ...
                self.results.append(ValidationResult(
                    "Infrastructure", f"Pub/Sub Topic '{topic_id}'",
                    True, "Topic exists"
                ))
            except subprocess.CalledProcessError:
                self.results.append(ValidationResult(
                    "Infrastructure", f"Pub/Sub Topic '{topic_id}'",
                    False, "Topic not found"
                ))
    ...
    async def validate_services(self):
        ...
        for service_name, config in SERVICES.items():
            ...
            try:
                ...
                if not service_url:
                    self.results.append(ValidationResult(
                        "Services", f"{service_name} deployment",
                        False, "No URL found"
                    ))
                    continue
                ...
                self.results.append(ValidationResult(
                    "Services", f"{service_name} deployment",
                    True, f"Deployed at {service_url}"
                ))
                ...
                try:
                    ...
                    if response.status_code == 200:
                        self.results.append(ValidationResult(
                            "Services", f"{service_name} health",
                            True, "Service is healthy"
                        ))
                    else:
                        self.results.append(ValidationResult(
                            "Services", f"{service_name} health",
                            False, f"Health check returned {response.status_code}"
                        ))
                except Exception as e:
                    self.results.append(ValidationResult(
                        "Services", f"{service_name} health",
                        False, f"Health check failed: {str(e)}"
                    ))
                
                # Check service account
                sa_email = service_data.get("spec", {}).get("template", {}).get(
                    "spec", {}).get("serviceAccountName")
                if sa_email:
                    expected_sa = f"orchestra-{config['type']}-sa@{PROJECT_ID}.iam.gserviceaccount.com"
                    self.results.append(ValidationResult(
                        "Services", f"{service_name} service account",
                ...
                if config["required"]:
                    self.results.append(ValidationResult(
                        "Services", f"{service_name} deployment",
                        False, "Service not found (required)"
                    ))
                else:
                    self.results.append(ValidationResult(
                        "Services", f"{service_name} deployment",
                        True, "Service not deployed (optional)"
                    ))
    ...
    async def validate_secrets(self):
        ...
        for secret_id in all_secrets:
            try:
                ...
                self.results.append(ValidationResult(
                    "Secrets", f"Secret '{secret_id}'",
                    True, "Secret exists and accessible"
                ))
            except gcp_exceptions.NotFound:
                self.results.append(ValidationResult(
                    "Secrets", f"Secret '{secret_id}'",
                ...
                self.results.append(ValidationResult(
                    "Secrets", f"Secret '{secret_id}'",
                    False, f"Error accessing secret: {str(e)}"
                ))
    ...
    async def validate_integrations(self):
        ...
        try:
            ...
            self.results.append(ValidationResult(
                "Integrations", "Service Directory registrations",
        ...
        try:
            ...
            orchestra_dashboard = any(
                d.get("displayName") == "Orchestra AI Dashboard"
                for d in dashboards
            )
            self.results.append(ValidationResult(
            ...
            self.results.append(ValidationResult(
                "Integrations", "Monitoring Dashboard",
                False, "Failed to check dashboards"
            ))
        ...
        try:
            ...
            self.results.append(ValidationResult(
                "Integrations", "Alert Policies",
                False, "Failed to check alert policies"
            ))
    ...
    async def validate_security(self):
        ...
        for sa_name in service_accounts:
            ...
            try:
                ...
                self.results.append(ValidationResult(
                    "Security", f"Service Account '{sa_name}'",
                    True, "Service account exists"
                ))
            except subprocess.CalledProcessError:
                self.results.append(ValidationResult(
                    "Security", f"Service Account '{sa_name}'",
                    False, "Service account not found"
                ))
        
        # Check Workload Identity Federation
        try:
            result = subprocess.run(
                ["gcloud", "iam", "workload-identity-pools", "describe", "github-wif-pool",
                 "--location", "global", "--project", PROJECT_ID],
                capture_output=True, text=True, check=True
            )
            self.results.append(ValidationResult(
                "Security", "Workload Identity Pool",
                True, "GitHub WIF pool configured"
            ))
        except subprocess.CalledProcessError:
            self.results.append(ValidationResult(
                "Security", "Workload Identity Pool",
                False, "GitHub WIF pool not found"
            ))
        ...
        try:
            ...
            self.results.append(ValidationResult(
                "Security", "Redis Encryption",
                False, "Failed to check Redis encryption"
            ))
