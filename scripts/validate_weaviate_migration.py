...
def validate_collection_schema(weaviate_client: weaviate.Client, collection_name: str) -> bool:
    ...
    try:
        ...
        if missing_properties:
            logger.error(
                f"Collection {collection_name} is missing required properties: {', '.join(missing_properties)}"
            )
            return False
...
def test_insert_retrieve(
    ...
    try:
        ...
        # Retrieve the object
        retrieved = weaviate_client.data_object.get_by_id(uuid=test_id, class_name=collection_name, with_vector=True)
...
def test_vector_search(
    ...
    try:
        ...
        result = (
            weaviate_client.query.get(collection_name, ["content", "_additional { id distance }"])
...
def run_performance_tests(weaviate_client: weaviate.Client, embedding_model: SentenceTransformer) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, embedding in enumerate(test_embeddings):
            ...
            weaviate_client.query.get("Session", ["content", "_additional { id distance }"]).with_near_vector(
                {"vector": embedding}
            ).with_limit(5).do()
        ...
        for text in test_texts:
            ...
            weaviate_client.query.get("Session", ["content", "_additional { id distance }"]).with_hybrid(
        ...
        for test_id, _, _ in test_objects:
            ...
            weaviate_client.data_object.get_by_id(uuid=test_id, class_name="Session")
            ...
            # Clean up
            weaviate_client.data_object.delete(uuid=test_id, class_name="Session")
...
def main():
    ...
    try:
        ...
        for collection, object_id in test_objects.items():
            results["hybrid_search"][collection] = test_hybrid_search(weaviate_client, collection, object_id)
        ...
        overall_success = (
            results["collections_exist"]
            and all_schema_valid
            and all_insert_retrieve
