...
class OrchestraSetupWizard:
    ...
    def save_env(self):
        ...
        with open(self.env_file, "w") as f:
            ...
            f.write(
                f"# Generated by Setup Wizard on {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            )
    ...
    def run_tests(self):
        ...
        if not all(
            [self.services_status.get("mongodb"), self.services_status.get("llm")]
        ):
    ...
    def print_summary(self):
        ...
        print(
            f"  MongoDB Atlas: {'✅ Configured' if self.services_status.get('mongodb') else '❌ Not configured'}"
        )
        print(
            f"  DragonflyDB:   {'✅ Configured' if self.services_status.get('dragonfly') else '❌ Not configured'}"
        )
        print(
            f"  Weaviate:      {'✅ Configured' if self.services_status.get('weaviate') else '❌ Not configured'}"
        )
        print(
        ...
        if not self.services_status.get("weaviate"):
            print(
                "  3. (Optional) Sign up for Weaviate: https://console.weaviate.cloud"
            )
