#!/usr/bin/env python3
"""
Review AI context files and validate project principles.

This script ensures all AI context files are consistent and
validates that the project follows its stated principles.
"""
...
def check_python_version() -> bool:
    ...
    if sys.version_info < (3, 11):
        logger.error("❌ Python 3.11+ required, but running {}.{}".format(
...
def main():
    ...
    logger.info("🔍 Orchestra AI Context Review")
    logger.info("="*50)
    
    # Check Python version
    if not check_python_version():
        logger.error("\n⚠️  CRITICAL: Python version requirement not met!")
        logger.error("Please upgrade to Python 3.11 or later")
    ...
    tools = check_existing_tools()
    
    # Summary
    logger.info("\n📊 Summary of Principles:")
    logger.info("✅ NO Docker/Poetry/Pipenv - use pip/venv only")
    logger.info("✅ Python 3.11+ required")
