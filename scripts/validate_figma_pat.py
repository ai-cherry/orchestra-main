...
def validate_figma_pat(figma_pat=None, file_id="368236963"):
    ...
    try:
        ...
        response = requests.get(f'https://api.figma.com/v1/files/{file_id}/variables/local', headers=headers)
        if response.status_code == 200:
...
def main():
    ...
    parser.add_argument('--token', help='Figma Personal Access Token (if not provided, will use FIGMA_PAT env var)')
    parser.add_argument('--file-id', default="368236963", help='Figma file ID to use for testing variables access')
