#!/bin/bash
# update_codespaces_secrets.sh - Update GitHub Codespaces secrets with GCP service account keys
...
# Update GitHub Codespaces secret
update_codespaces_secret() {
  local secret_name=$1
  local secret_value=$2
  local visibility=$3
  
  log "INFO" "Updating GitHub Codespaces secret: ${secret_name}..."
  
  # Update the secret
  if [[ "${visibility}" == "org" ]]; then
    echo "${secret_value}" | gh codespace secret set "${secret_name}" --org "${GITHUB_ORG}"
  else
    echo "${secret_value}" | gh codespace secret set "${secret_name}" --repo "${GITHUB_ORG}/${GITHUB_REPO}"
  fi
  
  log "SUCCESS" "Successfully updated GitHub Codespaces secret: ${secret_name}"
}

# Main function
main() {
  log "INFO" "Starting GitHub Codespaces secrets update..."
  ...
  update_codespaces_secret "GCP_PROJECT_ID" "${GCP_PROJECT_ID}" "org"
  
  # 2. GCP_REGION
  log "INFO" "Updating GCP_REGION secret..."
  update_codespaces_secret "GCP_REGION" "${REGION}" "org"
  ...
  update_codespaces_secret "GCP_MASTER_SERVICE_JSON" "${master_key}" "org"
  ...
  update_codespaces_secret "GCP_VERTEX_POWER_KEY" "${vertex_key}" "org"
  ...
  update_codespaces_secret "GCP_GEMINI_POWER_KEY" "${gemini_key}" "org"
  ...
  update_codespaces_secret "GCP_PROJECT_NUMBER" "${project_number}" "org"
  
  # 7. GOOGLE_APPLICATION_CREDENTIALS
  log "INFO" "Updating GOOGLE_APPLICATION_CREDENTIALS secret..."
  update_codespaces_secret "GOOGLE_APPLICATION_CREDENTIALS" "/workspaces/orchestra-main/service-account-key.json" "org"
