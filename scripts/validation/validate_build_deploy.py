...
def validate_cloudbuild_service(project_id, location):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True, f"Cloud Build service accessible, found {len(builds)} recent builds"
        else:
...
def validate_artifact_registry(project_id, location):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True, f"Artifact Registry accessible, found {len(repos)} repositories"
        else:
            return False, f"Error listing Artifact Registry repositories: {result.stderr}"
    except Exception as e:
...
def validate_cloud_deploy(project_id, location):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return True, f"Cloud Deploy accessible, found {len(pipelines)} delivery pipelines"
        else:
