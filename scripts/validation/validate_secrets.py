...
def validate_secrets(project_id):
    ...
    required_secrets = ["OPENAI_API_KEY", "PORTKEY_API_KEY"]
    ...
    try:
        ...
        for secret_id in required_secrets:
            ...
            results[secret_id] = {"exists": has_secret, "accessible": secret_accessible}
...
def main():
    ...
    if success and isinstance(results, dict):
        for secret_id, status in results.items():
            ...
            print(
                f"* {secret_id}: Exists: {exists_status}, Accessible: {access_status}"
            )
