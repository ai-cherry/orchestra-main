...
def validate_file_structure():
    ...
    critical_dirs = [
        "agent", "core", "packages", "scripts", "templates", 
        "infra", "docs", "apps"
    ]
    
    critical_files = [
        "cloudbuild.yaml", "pyproject.toml", ".flake8",
        "Dockerfile"
    ]
...
def validate_python_env():
    ...
    try:
        ...
        return True, f"Poetry: {poetry_result.stdout.strip()}\nCheck: {install_result.stdout.strip()}"
    except Exception as e:
        return False, str(e)
...
def main():
    ...
    # Overall status
    all_ok = all(structure["dirs"].values()) and all(structure["files"].values()) and git_ok and py_ok
