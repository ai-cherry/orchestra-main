...
def check_iam_practices(project_id):
    ...
    try:
        ...
        if result.returncode == 0:
            service_accounts = json.loads(result.stdout) if result.stdout.strip() else []
            ...
            for sa in service_accounts:
                ...
                if 'compute@developer' in email or 'editor' in email:
                    recommendations.append(f"Service account {email} may have overly broad permissions")
...
def check_docker_practices():
    ...
    try:
        ...
        if "apt-get update && apt-get install" in content and "apt-get clean" not in content:
            recommendations.append("Add 'apt-get clean' after installations to reduce image size")
...
def check_monitoring_practices(project_id):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if not metrics:
                recommendations.append("No custom metrics found. Consider implementing custom metrics for application-specific monitoring")
            ...
            if alert_result.returncode == 0:
                policies = json.loads(alert_result.stdout) if alert_result.stdout.strip() else []
                
                if not policies:
                    recommendations.append("No alerting policies found. Consider setting up alerts for critical metrics")
            
            return True, {
                "custom_metrics": len(metrics),
                "alert_policies": len(policies) if 'policies' in locals() else "unknown",
                "recommendations": recommendations
            }
        else:
...
def check_security_practices():
    ...
    if env_files:
        recommendations.append(f"Found {len(env_files)} .env files. Ensure these are not committed to source control and use Secret Manager instead")
    ...
    if potential_secret_files:
        recommendations.append(f"Found {len(set(potential_secret_files))} Python files with potential hardcoded secrets. Use Secret Manager instead")
    ...
    if not has_secret_manager:
        recommendations.append("No Secret Manager usage detected. Consider using GCP Secret Manager for sensitive information")
...
def main():
    ...
    print("* Recommendations:")
    print("  * Use Workload Identity Federation instead of service account keys when possible")
