...
def test_postgresql_operations(db: UnifiedDatabase):
    ...
    try:
        ...
        session = db.create_session(session_id=session_id, user_id="test_user", agent_id=str(agent["id"]), ttl_hours=1)
...
def test_weaviate_operations(db: UnifiedDatabase):
    ...
    try:
        ...
        memory_id = db.weaviate.store_memory(
            agent_id=agent_id, content="This is a test memory", memory_type="test", importance=0.5
        )
        logger.info(f"✓ Stored memory: {memory_id}")

        # Test memory search
        memories = db.weaviate.search_memories(agent_id=agent_id, query="test memory", limit=5)
        ...
        knowledge = db.search_knowledge(query="test knowledge", category="test", limit=5)
        logger.info(f"✓ Found {len(knowledge)} knowledge items")

        # Test conversation storage
        session_id = f"test_session_{datetime.now().strftime('%Y%m%d%H%M%S')}"
        conv_id = db.weaviate.store_conversation(
            session_id=session_id, agent_id=agent_id, user_id="test_user", message="Test message", role="user"
        )
...
def test_unified_operations(db: UnifiedDatabase):
    ...
    try:
        ...
        context = db.search_agent_context(
            agent_id=agent_id, query="test", include_conversations=True, include_memories=True
        )
        logger.info(
            f"✓ Found context: {len(context.get('memories', []))} memories, {len(context.get('conversations', []))} conversations"
        )
