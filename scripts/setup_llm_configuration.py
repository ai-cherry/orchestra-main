...
async def migrate_existing_config(db_url: str):
    ...
    async with async_session() as session:
        ...
        for use_case, display_name, description, temp, tokens in use_cases_data:
            ...
            """, use_case, display_name, description, temp, tokens)
        
        # Insert some default models
        models_data = [
            # Portkey models
            ('portkey', 'anthropic/claude-3-opus', 'Claude 3 Opus', '{"supports_tools": true, "max_tokens": 4096}', 0.015),
            ('portkey', 'anthropic/claude-3-sonnet', 'Claude 3 Sonnet', '{"supports_tools": true, "max_tokens": 4096}', 0.003),
            ('portkey', 'anthropic/claude-3-haiku', 'Claude 3 Haiku', '{"supports_tools": false, "max_tokens": 4096}', 0.00025),
            ...
            ('portkey', 'openai/gpt-3.5-turbo', 'GPT-3.5 Turbo', '{"supports_tools": true, "max_tokens": 4096}', 0.0005),
            ('portkey', 'google/gemini-1.5-pro', 'Gemini 1.5 Pro', '{"supports_tools": true, "max_tokens": 8192}', 0.007),
            ('portkey', 'google/gemini-1.5-flash', 'Gemini 1.5 Flash', '{"supports_tools": false, "max_tokens": 8192}', 0.00035),
            
            # OpenRouter models (as fallback)
            ('openrouter', 'anthropic/claude-3-opus:beta', 'Claude 3 Opus', '{"supports_tools": true, "max_tokens": 4096}', 0.015),
            ('openrouter', 'anthropic/claude-3-sonnet:beta', 'Claude 3 Sonnet', '{"supports_tools": true, "max_tokens": 4096}', 0.003),
            ('openrouter', 'google/gemini-pro-1.5', 'Gemini 1.5 Pro', '{"supports_tools": true, "max_tokens": 8192}', 0.007),
            ('openrouter', 'mistralai/mixtral-8x7b', 'Mixtral 8x7B', '{"supports_tools": false, "max_tokens": 32768}', 0.00024),
        ]
        ...
        for provider_name, model_id, display_name, capabilities, cost in models_data:
            if provider_name in providers:
                ...
                """, providers[provider_name], model_id, display_name, capabilities, cost)
        ...
        for use_case_enum, tier_mappings in router.MODEL_MAPPINGS.items():
            ...
            for tier_enum, mapping in tier_mappings.items():
                ...
                result = await session.execute("""
                    ...
                    RETURNING id
                """, use_case_id, tier_enum.value, primary_model_id, 
                    mapping.temperature, mapping.max_tokens, mapping.system_prompt)
                ...
                for priority, fallback_model in enumerate(mapping.fallback_models):
                    ...
                    if fallback_model_id:
                        ...
                        """, assignment_id, fallback_model_id, priority)
