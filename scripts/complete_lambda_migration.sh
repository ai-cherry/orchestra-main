#!/bin/bash
# Complete Migration from DragonflyDB + DigitalOcean to Lambda
# This replaces ALL external services with Lambda-hosted ones
...
echo "🚀 Complete Lambda Migration Script"
echo "================================="
echo "This will:"
...
Lambda_exec() {
    ssh -i ~/.ssh/Lambda_cherry_ai root@$Lambda_IP "$@"
}

echo "📦 Step 1: Installing Redis on lambda..."
Lambda_exec << 'EOF'
...
Lambda_exec "mkdir -p /opt/cherry_ai/scripts"
scp -i ~/.ssh/Lambda_cherry_ai /tmp/migrate_dragonfly_data.py root@$Lambda_IP:/opt/cherry_ai/scripts/
...
echo ""
echo "Next steps:"
echo "1. Test everything works: http://$Lambda_IP"
