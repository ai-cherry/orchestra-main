...
DELETE_FILES = [
    ...
    'core/orchestrator/src/memory/backends/redis_memory.py',
    'core/orchestrator/src/api/dependencies/cache.py',
...
class InfrastructurePurger:
    ...
    def _clean_python_content(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if any(keyword in line.lower() for keyword in ['redis', 'mongodb', 'firestore', 'dragonfly', 'gcp', 'cloud run']):
    ...
    def _clean_env_content(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if any(line.startswith(prefix) for prefix in ['REDIS_', 'MONGODB_', 'GCP_', 'FIRESTORE_', 'DRAGONFLY_', 'GOOGLE_CLOUD_']):
                continue
