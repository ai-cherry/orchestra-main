...
AVAILABLE_PROVIDERS = [
    "openai", "anthropic", "mistral", "huggingface", "cohere", 
    "openrouter", "perplexity", "deepseek", "codestral", "cody",
    "continue", "grok", "google", "azure", "aws", "pinecone",
    "weaviate", "elevenlabs"
]
...
def format_output(data: Any, output_format: str = "pretty") -> str:
    ...
    if output_format == "json":
        ...
        if isinstance(data, list):
            ...
            item_dict = {k: v for k, v in data.__dict__.items() if not k.startswith("_")}
            return "\n".join([f"{k}: {v}" for k, v in item_dict.items()])
        else:
            return str(data)
...
def create_key(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.provider not in AVAILABLE_PROVIDERS:
            ...
            logger.warning(f"Provider '{args.provider}' is not in the list of known providers: {providers_str}")
            if not args.force:
...
def list_keys(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.format == "json":
            print(json.dumps([{k: v for k, v in key.__dict__.items() if not k.startswith("_")} 
                             for key in keys], indent=2))
        else:
            ...
            for key in keys:
                ...
                if key.usage_stats:
                    print(f"Usage: ${key.usage_stats.get('total_cost', 0):.4f} "
...
def update_key(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not update_params:
            logger.error("No update parameters provided. Use --name, --description, --budget-limit, or --rate-limit.")
            return
...
def delete_key(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not args.force:
            confirm = input(f"Are you sure you want to delete virtual key '{args.id}'? This cannot be undone. (y/N): ")
            if confirm.lower() != 'y':
...
def create_config(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.strategy not in ROUTING_STRATEGIES:
            ...
            logger.error(f"Strategy '{args.strategy}' is not valid. Choose from: {strategies_str}")
            return
        ...
        if args.providers:
            try:
                ...
                logger.error("Invalid JSON format for providers. Example: '[{\"virtual_key\": \"vk_...\", \"weight\": 1}]'")
                return
...
def list_configs(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.format == "json":
            print(json.dumps([{k: v for k, v in config.__dict__.items() if not k.startswith("_")} 
...
def main() -> None:
    ...
    output_args.add_argument("--output", "-o", action="store_true", help="Show detailed output")
    output_args.add_argument("--format", "-f", choices=["pretty", "json"], default="pretty", 
                          help="Output format (default: pretty)")
    
    # Create key command
    create_key_parser = subparsers.add_parser("create-key", help="Create a new virtual key", parents=[output_args])
    create_key_parser.add_argument("--name", "-n", required=True, help="Name for the virtual key")
    create_key_parser.add_argument("--provider", "-p", required=True, help="Provider name (e.g., openai, anthropic)")
    create_key_parser.add_argument("--key", "-k", required=True, help="Provider API key")
    create_key_parser.add_argument("--description", "-d", help="Description for the key")
    create_key_parser.add_argument("--budget-limit", "-b", type=float, help="Monthly budget limit in USD")
    create_key_parser.add_argument("--rate-limit", "-r", type=int, help="Rate limit (requests per minute)")
    create_key_parser.add_argument("--force", action="store_true", help="Force creation even if provider is unknown")
    create_key_parser.set_defaults(func=create_key)
    
    # List keys command
    list_keys_parser = subparsers.add_parser("list-keys", help="List virtual keys", parents=[output_args])
    list_keys_parser.add_argument("--provider", "-p", help="Filter by provider")
    list_keys_parser.set_defaults(func=list_keys)
    
    # Get key command
    get_key_parser = subparsers.add_parser("get-key", help="Get details for a specific virtual key", parents=[output_args])
    get_key_parser.add_argument("--id", "-i", required=True, help="Virtual key ID")
    get_key_parser.set_defaults(func=get_key)
    
    # Update key command
    update_key_parser = subparsers.add_parser("update-key", help="Update a virtual key", parents=[output_args])
    ...
    update_key_parser.add_argument("--description", "-d", help="New description for the key")
    update_key_parser.add_argument("--budget-limit", "-b", type=float, help="New monthly budget limit in USD")
    update_key_parser.add_argument("--rate-limit", "-r", type=int, help="New rate limit (requests per minute)")
    ...
    delete_key_parser.add_argument("--force", "-f", action="store_true", help="Skip confirmation prompt")
    delete_key_parser.set_defaults(func=delete_key)
    
    # Rotate key command
    rotate_key_parser = subparsers.add_parser("rotate-key", help="Rotate a virtual key's provider API key", parents=[output_args])
    rotate_key_parser.add_argument("--id", "-i", required=True, help="Virtual key ID")
    rotate_key_parser.add_argument("--new-key", "-k", required=True, help="New provider API key")
    rotate_key_parser.set_defaults(func=rotate_key)
    
    # Create config command
    create_config_parser = subparsers.add_parser("create-config", help="Create a new gateway configuration", parents=[output_args])
    create_config_parser.add_argument("--name", "-n", required=True, help="Name for the gateway configuration")
    create_config_parser.add_argument("--strategy", "-s", required=True, choices=ROUTING_STRATEGIES,
                                   help="Routing strategy")
    create_config_parser.add_argument("--providers", "-p", help="Provider configurations as JSON array")
    create_config_parser.add_argument("--cache-config", "-c", help="Cache configuration as JSON object")
    create_config_parser.set_defaults(func=create_config)
    
    # List configs command
    list_configs_parser = subparsers.add_parser("list-configs", help="List gateway configurations", parents=[output_args])
    list_configs_parser.set_defaults(func=list_configs)
    
    # Get usage command
    get_usage_parser = subparsers.add_parser("get-usage", help="Get usage statistics", parents=[output_args])
