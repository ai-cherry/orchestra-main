...
def get_normalized_names(component_name: str) -> List[str]:
    ...
    if variant:
        ...
        variant_camel = (
            variant[0].lower() + variant[1:] if len(variant) > 1 else variant.lower()
        )
...
def analyze_component_mapping(
    ...
    for component_name in mapping_data.keys():
        ...
        if (
            not component_result["found_in_js"]
            and not component_result["found_in_android"]
        ):
            results["not_found"] += 1
            ...
            for comp in results["component_results"]:
                writer.writerow(
                    [
                        ...
                        ", ".join(comp["android_matches"])
                        if comp["android_matches"]
                        else "None",
                    ]
                )
...
def generate_mapping_recommendations(results: Dict) -> Dict:
    ...
    recommendations = {"rename_suggestions": [], "implementation_suggestions": []}
    ...
    for comp in results["component_results"]:
        if not comp["found_in_js"] and not comp["found_in_android"]:
            ...
            similar_js = [
                p for p in successful_js_patterns if base_name.lower() in p.lower()
            ]
            similar_android = [
                p for p in successful_android_patterns if base_name.lower() in p.lower()
            ]
...
def print_analysis_results(results: Dict, recommendations: Dict) -> None:
    ...
    print(
        f"Found in JS: {results['found_in_js']} ({results['found_in_js']/results['total_components']*100:.1f}%)"
    )
    print(
        f"Found in Android: {results['found_in_android']} ({results['found_in_android']/results['total_components']*100:.1f}%)"
    )
    print(
        f"Found in both: {results['found_in_both']} ({results['found_in_both']/results['total_components']*100:.1f}%)"
    )
    print(
        f"Not found: {results['not_found']} ({results['not_found']/results['total_components']*100:.1f}%)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze component mappings between definitions and implementations"
    )
    parser.add_argument(
        "--mapping",
        default="component-adaptation-mapping.json",
        help="Path to component-adaptation-mapping.json",
    )
    parser.add_argument(
        "--js",
        default="packages/ui/src/tokens/variables.js",
        help="Path to JavaScript implementation file",
    )
    parser.add_argument(
        "--android",
        default="packages/ui/android/src/main/res/values/styles.xml",
        help="Path to Android implementation file",
    )
    parser.add_argument(
        "--output",
        default="component_mapping_analysis.csv",
    ...
    # Run analysis
    results = analyze_component_mapping(
        args.mapping, args.js, args.android, args.output
    )
