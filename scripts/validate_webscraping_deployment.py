        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if project == self.project_id:
                    ...
                    self.log_result("GCP Setup", "WARN", f"gcloud configured for project {project}, expected {self.project_id}")
            else:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
                async with session.get(f"{service_url}/mcp/tools") as response:
                    if response.status == 200:
                        ...
                        if len(tools) >= 5:  # Expected tools: web_search, scrape_website, analyze_content, bulk_scrape, get_task_status
