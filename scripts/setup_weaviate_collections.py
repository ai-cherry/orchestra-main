...
def main():
    ...
    try:
        ...
        if success_count == total_collections:
            logger.info(f"All {total_collections} collections successfully {'verified' if args.verify else 'created and verified'}")
            return 0
        else:
            logger.error(f"Only {success_count}/{total_collections} collections were successfully {'verified' if args.verify else 'created and verified'}")
            return 1
