...
TEMP_FILE_COUNT=$(find "$PROJECT_T" -type f \( -name "tmp_*" -o -name "temp_*" -o -name "*.tmp" -o -name "*.bak" \) -not -path "*/venv/*" -not -path "*/.git/*" -not -path "*/node_modules/*" 2>/dev/null | wc -l)
...
LARGE_FILES=$(find "$PROJECT_T" -name "*.py" -size +700k -not -path "*/venv/*" -not -path "*/.git/*" 2>/dev/null | wc -l)
...
if [ -f "$PROJECT_T/.cleanup_registry.json" ]; then
    REGISTRY_SIZE=$(wc -l < "$PROJECT_T/.cleanup_registry.json")
...
if [ -d "$PROJECT_T/status/automation_health" ]; then
    HEALTH_FILES=$(find "$PROJECT_T/status/automation_health" -name "*.health" -type f 2>/dev/null | wc -l)
    FAILED_SCRIPTS=$(find "$PROJECT_T/status/automation_health" -name "*.health" -type f -exec grep -l "^failed" {} \; 2>/dev/null | wc -l)
...
if command -v git >/dev/null 2>&1 && git -C "$PROJECT_T" rev-parse --git-dir >/dev/null 2>&1; then
    UNTRACKED_PY=$(git -C "$PROJECT_T" ls-files --others --exclude-standard | grep "\.py$" | wc -l)
