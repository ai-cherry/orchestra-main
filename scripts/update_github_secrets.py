...
class SecretAnalyzer:
        ...
        for secret in self.secrets:
            ...
            if issues:
                self.inconsistencies.append({
                    "secret": secret,
                    "issues": issues,
                    "recommended": self.recommended_mappings.get(secret, secret)
                })
    ...
    def generate_report(self, output_file: Optional[str] = None) -> str:
        ...
        report += "1. **Use Organization Secrets**: Store shared secrets at the organization level with the `ORG_` prefix\n"
