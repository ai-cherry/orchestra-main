...
def verify_model_assignments():
    ...
    if not MODE_DEFINITIONS_PATH.exists():
        print(f"{Fore.RED}Mode definitions file not found. Cannot verify model assignments.{Style.RESET_ALL}")
        return
        ...
        for slug, expected_model in expected_assignments.items():
            if slug in modes:
                ...
                if expected_model.lower() in actual_model:
                    ...
                    print(f"{Fore.RED}✗ {slug}: Expected {expected_model}, got {actual_model}{Style.RESET_ALL}")
                    all_correct = False
            else:
                print(f"{Fore.YELLOW}! {slug}: Mode not found in configuration{Style.RESET_ALL}")
                all_correct = False
...
def verify_enhanced_access_permissions():
    ...
    if not MODE_DEFINITIONS_PATH.exists():
        print(f"{Fore.RED}Mode definitions file not found. Cannot verify access permissions.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        write_access_modes = ["code", "debug", "architect", "orchestrator", "reviewer", "strategy", "creative"]
...
def print_workflow_summary():
    ...
    if not MODE_DEFINITIONS_PATH.exists():
        print(f"{Fore.RED}Mode definitions file not found. Cannot show workflows.{Style.RESET_ALL}")
        return
    ...
    try:
        ...
        if not workflows:
            print(f"{Fore.YELLOW}No workflows defined in configuration.{Style.RESET_ALL}")
            return
...
def print_usage_instructions():
    ...
    print(f"\n2. Start a workflow:")
    print(f"   {Fore.CYAN}python tools/mode_switcher.py --workflow feature_development{Style.RESET_ALL}")
