...
class CascadeTestRequest(BaseModel):
    ...
    prompt: str
    cascade_order: List[str] = ["openrouter", "portkey", "openai", "anthropic", "deepseek"]
...
async def call_provider(
    ...
    try:
        ...
        REQUEST_LATENCY.labels(provider=provider_name, model=provider_model, status="success").observe(elapsed_ms / 1000)
        
        if token_counts["input"] > 0:
            TOKEN_COUNT.labels(provider=provider_name, model=provider_model, direction="input").inc(token_counts["input"])
        if token_counts["output"] > 0:
            TOKEN_COUNT.labels(provider=provider_name, model=provider_model, direction="output").inc(token_counts["output"])
        ...
        REQUEST_LATENCY.labels(provider=provider_name, model=model or "unknown", status="error").observe(elapsed_ms / 1000)
