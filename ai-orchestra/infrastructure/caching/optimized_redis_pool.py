            ...
            for pool_type, pools in cls._metrics.items():
                ...
                for key, metrics in pools.items():
                    result["pools"][pool_type][key] = {
                        ...
                        "error_rate": (metrics["errors"] / metrics["operations"] * 100) 
                                     if metrics["operations"] > 0 else 0,
...
class OptimizedRedisClient:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        log_event(logger, "redis", "client_created", {
            "host": self.host,
            "port": self.port,
            "db": self.db,
    ...
    async def close(self) -> None:
        ...
        if self._client is not None:
            ...
            log_event(logger, "redis", "client_closed", {
                "host": self.host,
                "port": self.port,
                "db": self.db,
