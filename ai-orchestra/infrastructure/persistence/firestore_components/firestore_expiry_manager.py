...
class FirestoreExpiryManager:
    ...
    def __init__(
        ...
        collection_name: str,
        event_bus: Optional[PlaceholderEventBus] = None,
    ):  # TODO: Replace PlaceholderEventBus with actual type
        """
    ...
    async def is_item_expired(
        ...
        if expiry_timestamp:
            ...
            if not isinstance(expiry_timestamp, datetime):
                logger.warning(
                    f"Expiry field for item {doc_snapshot.id} is not a datetime object: {type(expiry_timestamp)}"
                )
                return False
    ...
    async def prune_expired_items(
        ...
        async for doc_snapshot in doc_stream:
            ...
            if current_batch_operation_count >= delete_batch_size:
                try:
                    await batch.commit()
                    logger.info(
                        f"Committed batch of {current_batch_operation_count} deletions. Total deleted so far: {deleted_count}"
                    )
                    await self.event_bus.publish(
                        PlaceholderMemoryEvent(
                            PlaceholderEventType.MEMORY_ITEM_EXPIRED,
                            key="batch_prune",
                            count=current_batch_operation_count,
                        )
                    )
        ...
        if current_batch_operation_count > 0:
            try:
                await batch.commit()
                logger.info(
                    f"Committed final batch of {current_batch_operation_count} deletions. Total deleted: {deleted_count}"
                )
                await self.event_bus.publish(
                    PlaceholderMemoryEvent(
                        PlaceholderEventType.MEMORY_ITEM_EXPIRED,
                        key="final_batch_prune",
                        count=current_batch_operation_count,
                    )
                )
            except Exception as e:
