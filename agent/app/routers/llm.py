...
from core.llm_router_dynamic import get_dynamic_llm_router, UseCase, ModelTier, RouterConfig
...
async def generate_code(request: CodeGenerationRequest) -> Dict[str, str]:
    ...
    try:
        ...
        response = await llm_router.complete(request.prompt, use_case=UseCase.CODE_GENERATION, tier=request.tier)
...
async def design_architecture(request: ArchitectureRequest) -> Dict[str, str]:
    ...
    try:
        ...
        response = await llm_router.complete(
            request.requirements, use_case=UseCase.ARCHITECTURE_DESIGN, tier=request.tier
        )
