"""
Natural Language Interface for Orchestra AI
Voice and text interaction with orchestrator and agents
"""
...
from core.auth.simple_auth import verify_api_key
from agent.app.services.real_agents import run_agent_task, get_all_agents
...
async def process_voice_command(
    ...
    transcribed_text = await voice_transcriber.transcribe(
    ...
    result = await process_text_command(text_command, api_key)
    
    # Generate speech response
    speech_url = await response_generator.text_to_speech(result.text)
    result.speech_url = speech_url
    
    return result

@router.websocket("/stream")
async def websocket_nl_interface(
    websocket: WebSocket,
    api_key: str = Depends(verify_api_key)
):
...
async def handle_agent_command(intent, context):
    ...
    if intent.action == "start":
        ...
        return {
            "actions": [{"type": "agent_started", "agent_id": agent_id}],
            "data": result
        }
    elif intent.action == "status":
        return {
            "actions": [{"type": "status_check"}],
            "data": {"agents": agents}
        }
...
async def handle_general_command(intent, context):
    ...
    return {
        "actions": [],
        "data": {"message": "Command understood but not yet implemented"}
    } 