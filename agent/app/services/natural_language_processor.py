            ...
            for pattern, intent_cat, action in patterns:
                ...
                if match:
                    ...
                    return Intent(
                        ...
                        workflow_name=self._resolve_workflow_name(text) if intent_cat == IntentCategory.WORKFLOW else None
                    )
...
class ResponseGenerator:
    ...
    def __init__(self):
        self.resemble_project_uuid = None
        self.resemble_voice_uuid = None
        self._init_resemble()
    
    def _init_resemble(self):
        """Initialize Resemble AI voice settings"""
        if RESEMBLE_API_KEY:
            try:
                # Get default project and voice
                projects = Resemble.v2.projects.all(1, 10)
                if projects and projects.get('items'):
                    self.resemble_project_uuid = projects['items'][0]['uuid']
                
                voices = Resemble.v2.voices.all(1, 10)
                if voices and voices.get('items'):
                    self.resemble_voice_uuid = voices['items'][0]['uuid']
            except Exception as e:
                print(f"Failed to initialize Resemble: {e}")
...
Just tell me what you need in natural language!"""
    ...
    async def text_to_speech(self, text: str) -> Optional[str]:
        """Convert text to speech using Resemble AI"""
        if not all([RESEMBLE_API_KEY, self.resemble_project_uuid, self.resemble_voice_uuid]):
            return None
        
        try:
            # Create speech clip with Resemble
            response = Resemble.v2.clips.create_sync(
                self.resemble_project_uuid,
                self.resemble_voice_uuid,
                text,
                title=f"Response_{os.getpid()}",
                sample_rate=22050,
                output_format="mp3",
                precision="PCM_16"
            )
            
            if response and response.get('audio_src'):
