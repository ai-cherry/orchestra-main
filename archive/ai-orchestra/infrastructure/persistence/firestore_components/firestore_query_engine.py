...
from datetime import (
    datetime,
    timezone,
)  # Ensure datetime is imported if used for expiry checks here
...
from .memory_item_serializer import (
...
class FirestoreQueryEngine:
    ...
    def __init__(
        ...
        collection_name: str,
        serializer: MemoryItemSerializer,
    ):
        """
        ...
        logger.info(
            f"FirestoreQueryEngine initialized for collection: {self.collection_name}"
        )
    ...
    async def execute_query(
        ...
        if (
            page_size < 1 or page_size > 1000
        ):  # Firestore limit for array_contains_any, good general limit
        ...
        try:
            query = self._apply_filters_and_ordering(
                self._collection_ref, filters, tags, order_by, order_direction
            )
            ...
            if page > 1:
                ...
                async for (
                    doc_snap
                ) in offset_docs_query.stream():  # Iterate to get to the last one
            ...
            async for doc_snapshot in docs_stream:
                if doc_snapshot.exists:
                    ...
                    try:
                        item = self.serializer.to_memory_item(
                            doc_snapshot.id, firestore_data
                        )
                        if (
                            getattr(item, "expiry", None)
                            and item.expiry < current_time_utc
                        ):
                            logger.debug(
                        ...
                        continue
            logger.info(
                f"Query returned {len(items)} items for page {page} (total potential: {total_count})."
            )
            ...
            logger.error(
                f"Error executing query on '{self.collection_name}': {e}", exc_info=True
            )
    ...
    async def stream_query_results(
        ...
        try:
            ...
            logger.error(
                f"Error streaming query results from '{self.collection_name}': {e}",
                exc_info=True,
            )
            raise
