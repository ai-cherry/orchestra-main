...
class ScalingPolicy:
    ...
    async def create_instance(self, config: Dict[str, Any]) -> VultrInstance:
        """Create a new Vultr instance"""
            ...
            "label": config.get("label", f"orchestra-{datetime.now().timestamp()}"),
            "enable_ipv6": True,
            "backups": "enabled",
    ...
    async def execute_scaling(self, decision: Dict[str, Any]):
        ...
        try:
            ...
            if action == "scale_up":
                # Create new instance
                config = {
                    "label": f"orchestra-worker-{datetime.now().timestamp()}",
                    "role": "worker",
                    "region": "ewr",
...
class VultrOrchestrator:
    ...
    async def start(self):
        """Start the Vultr orchestrator"""
        logger.info("Starting Vultr orchestrator")
