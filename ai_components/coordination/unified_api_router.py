"""Unified API Router for intelligent routing between OpenRouter and conductor services."""
...
class RetryStrategy:
    ...
    """Intelligent router for API requests."""
        ...
        """
        """
                "User-Agent": "cherry_ai-AI/1.0"
            }
        )
        self.conductor_client = httpx.AsyncClient(
            timeout=30.0,
            limits=httpx.Limits(
    ...
    def _initialize_routing_rules(self) -> Dict[str, ServiceType]:
        """Initialize routing rules based on request patterns."""
            "multi_agent": ServiceType.CONDUCTOR,
            "workflow": ServiceType.CONDUCTOR,
            "coordinate": ServiceType.CONDUCTOR,
    ...
    def _requires_coordination(self, payload: Dict[str, Any]) -> bool:
        """
        """
    ...
    @handle_errors
    async def _call_conductor(
        self, request_type: str, payload: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        """
        endpoint = f"{self.conductor_url}/api/{request_type}"

        response = await self.conductor_client.post(
            endpoint,
