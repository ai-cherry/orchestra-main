...
def find_files(base_dir: str, patterns: List[str]) -> Set[Path]:
    ...
    for root, _, files in os.walk(base_dir):
        ...
        if any(
            skip in root
            for skip in [".git", "google-cloud-sdk", ".venv", "__pycache__"]
        ):
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check legacy files for deprecation notices"
    )
    parser.add_argument(
        "--base-dir", type=str, default=".", help="Base directory to search"
    )
    parser.add_argument(
        "--fix",
        action="store_true",
        help="Add deprecation notices to files missing them",
    )
    parser.add_argument(
