...
from vertexai.preview.generative_models import GenerativeModel, ChatSession, Part, Tool, FunctionDeclaration
...
) if hasattr(settings, 'REDIS_URL') and settings.REDIS_URL else None
...
class GeminiService:
    ...
    async def analyze_content(
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.error(
                f"Content analysis failed after {elapsed:.2f}s: {str(e)}",
                exc_info=True
            )
