...
async def get_agent_status(agent_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        async def get_agent_with_logs(transaction: AsyncTransaction) -> Tuple[Dict, List[Dict]]:
        ...
        if agent_data is None:
            return {"error": f"Agent {agent_id} not found", "status": "unknown", "success": False}
        ...
        return {"error": "Request timed out. Please try again later.", "status": "error", "success": False}
    except Exception as e:
...
async def promote_memory(memory_id: str, tier: str) -> Dict[str, Any]:
    ...
    if tier not in valid_tiers:
        return {"error": f"Invalid tier: {tier}. Must be one of {valid_tiers}", "success": False}
