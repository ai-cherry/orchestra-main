#!/bin/bash
# deploy_now.sh - Immediate deployment script using service account key from environment variable
# This script deploys the MCP server to Cloud Run using the GCP_MASTER_SERVICE_JSON environment variable
...
echo -e "${GREEN}Starting immediate deployment of ${SERVICE_NAME} to ${ENVIRONMENT} environment...${NC}"

# Check if GCP_MASTER_SERVICE_JSON environment variable exists
if [ -z "${GCP_MASTER_SERVICE_JSON}" ]; then
    echo -e "${RED}GCP_MASTER_SERVICE_JSON environment variable not found.${NC}"
    echo -e "${YELLOW}Please set the GCP_MASTER_SERVICE_JSON environment variable with the service account key.${NC}"
    exit 1
fi

# Create a temporary service account key file
echo -e "${GREEN}Creating temporary service account key file...${NC}"
echo "${GCP_MASTER_SERVICE_JSON}" > temp_service_account_key.json

# Authenticate with service account key
echo -e "${GREEN}Authenticating with service account key...${NC}"
gcloud auth activate-service-account --key-file=temp_service_account_key.json
export GOOGLE_APPLICATION_CREDENTIALS="$(pwd)/temp_service_account_key.json"
...
echo -e "${GREEN}Cleaning up temporary service account key file...${NC}"
rm temp_service_account_key.json
