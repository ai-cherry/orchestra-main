...
class WIFConfig(BaseModel):
    ...
    workload_identity_pool: Optional[str] = Field(
        None, description="Workload Identity Pool ID"
    )
    workload_identity_provider: Optional[str] = Field(
        None, description="Workload Identity Provider ID"
    )
    ...
    def validate_service_account(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v is not None:
            if not re.match(
                r"^[a-zA-Z0-9-]+@[a-zA-Z0-9-]+\.iam\.gserviceaccount\.com$", v
            ):
    ...
    def validate_wif_configuration(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if values.get("workload_identity_provider") and not values.get("project_id"):
            raise ValueError(
                "Project ID must be set when workload_identity_provider is set"
            )
    ...
    def get_workload_identity_provider_resource_name(self) -> Optional[str]:
        ...
        if (
            not self.project_id
            or not self.workload_identity_pool
            or not self.workload_identity_provider
        ):
            return None
