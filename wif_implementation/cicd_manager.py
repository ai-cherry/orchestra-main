            ...
            if pipeline["type"] == "github_actions":
                ...
                if re.search(r"google-github-actions/auth@v\d+", content):
                    ...
                    if re.search(r"workload_identity_provider", content):
                        ...
                        logger.info(f"Pipeline {pipeline['name']} uses Workload Identity Federation")
                    else:
                    ...
                    logger.info(f"Pipeline {pipeline['name']} uses unknown authentication method")
    ...
    def _create_templates(self, plan: ImplementationPlan) -> bool:
        ...
        templates = {
            ...
            "cloudbuild-template.yaml": {
                "steps": [
                    {"name": "gcr.io/cloud-builders/docker", "args": ["build", "-t", "gcr.io/$PROJECT_ID/IMAGE_NAME", "."]},
                    {"name": "gcr.io/cloud-builders/docker", "args": ["push", "gcr.io/$PROJECT_ID/IMAGE_NAME"]},
                    {"name": "gcr.io/cloud-builders/gcloud", "args": ["run", "deploy", "SERVICE_NAME", "--image", "gcr.io/$PROJECT_ID/IMAGE_NAME", "--region", "REGION", "--platform", "managed", "--allow-unauthenticated"]}
                ],
        ...
        if self.dry_run:
            ...
            for template_name, template_data in templates.items():
                ...
                with open(template_path, "w") as f:
                    if template_name.endswith('.yml') or template_name.endswith('.yaml'):
    ...
    def _update_pipelines(self, plan: ImplementationPlan) -> bool:
        ...
        for pipeline in self.pipelines:
            ...
            if pipeline.get("auth_method") == "wif":
                logger.info(f"Pipeline {pipeline['name']} already uses Workload Identity Federation")
                continue
            
            # Skip pipelines with unknown authentication method
            if pipeline.get("auth_method") == "unknown":
                logger.warning(f"Pipeline {pipeline['name']} has unknown authentication method, skipping")
                continue
            ...
            # Update Cloud Build files
                ...
                logger.info(f"Cloud Build pipeline {pipeline['name']} uses default service account, no update needed")
            ...
            # Update Jenkins files
                ...
                logger.warning(f"Jenkins pipeline {pipeline['name']} update not implemented")
            
            # Update Azure Pipelines files
            elif pipeline["type"] == "azure_pipelines":
                logger.warning(f"Azure Pipelines pipeline {pipeline['name']} update not implemented")
                    ...
                    try:
                        with open(workflow_file, "r") as f:
                            ...
                            if "workflow_dispatch" in content:
                                logger.info(f"Testing GitHub Actions workflow: {workflow_name}")
