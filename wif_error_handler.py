...
class WIFErrorHandler:
    ...
    ERROR_PATTERNS = [
        ...
        (
            r"Permission 'iam.serviceAccounts.getAccessToken' denied",
            ErrorCategory.AUTHENTICATION,
        ),
        # Permission errors
        (r"Error: Permission denied", ErrorCategory.PERMISSION),
        (
            r"Error: You do not have permission to perform this action",
    ...
    def get_recovery_steps(self, category: ErrorCategory) -> List[str]:
        ...
        """
        return self.RECOVERY_PROCEDURES.get(
            category, self.RECOVERY_PROCEDURES[ErrorCategory.UNKNOWN]
        )
    ...
    def create_error(
        ...
        logger.error(
            f"Error: {message} (Category: {category.value}, Severity: {severity.value})"
        )
    ...
    def wrap_script(self, script_path: str, args: List[str] = None) -> int:
        ...
        if not os.access(script_path, os.X_OK):
            logger.warning(
                f"Script {script_path} is not executable. Making it executable..."
            )
            try:
        ...
        if self.backup:
            backup_path = (
                f"{script_path}.wif-backup.{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )
            try:
        ...
        try:
            ...
            while True:
                ...
                if stderr_line:
                    ...
                    if (
                        "error" in stderr_line.lower()
                        or "warning" in stderr_line.lower()
                    ):
            ...
            if "command not found" in error.message:
                ...
                try:
                    if dependency == "gh":
                        ...
                        logger.warning(
                            f"Don't know how to install dependency: {dependency}"
                        )
                        return RecoveryResult(
                            success=False,
...
def parse_args() -> argparse.Namespace:
    ...
    wrap_parser = subparsers.add_parser(
        "wrap", help="Wrap a WIF script with enhanced error handling"
    )
    ...
    recover_parser = subparsers.add_parser(
        "recover", help="Attempt to recover from a failed WIF setup"
    )

    # Diagnose command
    diagnose_parser = subparsers.add_parser(
        "diagnose", help="Diagnose issues with WIF setup"
    )

    # Monitor command
    monitor_parser = subparsers.add_parser(
        "monitor", help="Monitor WIF operations in real-time"
    )
