    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        return {
            "tests": {
                ...
                "network_latency": {"enabled": True, "min_ms": 10, "max_ms": 200},
            },
            "firestore": {
...
class FirestoreTester(ConnectionTester):
    ...
    async def test_async_connection(self) -> None:
        ...
        try:
            ...
            if result.get("status") == "healthy":
                self.record_result(
                    "async_connection", True, duration_ms, details=result
                )
            else:
                self.record_result(
                    "async_connection",
    ...
    async def test_crud_operations(self) -> None:
        ...
        start_time = time.time()
        try:
            await self.async_manager.create_document(
                self.collection_name, doc_id, test_data
            )
            duration_ms = (time.time() - start_time) * 1000
        ...
        try:
            ...
            if doc and doc.get("test_value") == test_data["test_value"]:
                ...
                self.record_result(
                    "read_document", False, duration_ms, error="Document data mismatch"
                )
        except Exception as e:
            duration_ms = (time.time() - start_time) * 1000
        ...
        try:
            await self.async_manager.update_document(
                self.collection_name, doc_id, update_data
            )
            duration_ms = (time.time() - start_time) * 1000
            ...
            if (
                doc
                and doc.get("test_value") == update_data["test_value"]
                and doc.get("updated")
            ):
    ...
    async def test_connection_pool(self) -> None:
        ...
        logger.info(
            f"Testing connection pool with {num_operations} parallel operations"
        )
        ...
        async def single_operation(i: int) -> Dict[str, Any]:
            ...
            try:
                ...
                doc = await self.async_manager.get_document(
                    self.collection_name, doc_id
                )
                await self.async_manager.delete_document(self.collection_name, doc_id)
                duration_ms = (time.time() - start_time) * 1000
                return {"index": i, "success": True, "duration_ms": duration_ms}
            except Exception as e:
                duration_ms = (time.time() - start_time) * 1000
                return {
                    "index": i,
    ...
    async def test_error_handling(self) -> None:
        ...
        try:
            ...
            doc = await self.async_manager.get_document(
                self.collection_name, "non-existent-doc"
            )
            duration_ms = (time.time() - start_time) * 1000
            if doc is None:
                self.record_result(
        ...
        try:
            ...
            duration_ms = (time.time() - start_time) * 1000
            self.record_result(
                "error_handling_invalid_collection",
                False,
                duration_ms,
...
async def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run connection tests for Firestore and Redis"
    )
    parser.add_argument("--config", "-c", help="Path to configuration file")
    parser.add_argument(
        "--firestore-only", action="store_true", help="Only test Firestore"
    )
    parser.add_argument("--redis-only", action="store_true", help="Only test Redis")
    parser.add_argument(
        "--output",
        "-o",
        default="connection_test_report.json",
    ...
    if "firestore" in combined_results and combined_results["firestore"]:
        ...
        print(
            f"Firestore: {success_rate:.1f}% successful "
    ...
    if "redis" in combined_results and combined_results["redis"]:
        ...
        print(
            f"Redis: {success_rate:.1f}% successful "
            + ("✅" if success_rate > 90 else "⚠️" if success_rate > 70 else "❌")
        )
