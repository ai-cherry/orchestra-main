...
class EnhancedMemoryServer(EnhancedMCPServerBase):
    ...
    async def _handle_tool_internal(self, name: str, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        if name == "store_memory_batch":
            ...
            results = await self.vector_processor.process_vectors_in_batches(
                memories,
                self._store_memory_batch
            )
            
            success_count = sum(1 for r in results if r["success"])
            return [TextContent(
                type="text",
                text=f"✅ Stored {success_count}/{len(memories)} memories successfully"
            )]
            ...
            return [TextContent(
                type="text",
                text=f"✅ Cleared {cache_type} cache(s)"
            )]
    ...
    async def _store_memory_with_cache(self, **kwargs) -> str:
        ...
        keys_to_remove = [
            key for key in self.search_cache.cache.keys()
            if f'"agent_id": "{agent_id}"' in key
        ]
    ...
    async def _optimize_agent_memories(
        self, 
        agent_id: str, 
    ...
    async def _handle_original_tools(self, name: str, arguments: Dict[str, Any]) -> List[TextContent]:
        ...
        return [TextContent(
            type="text",
            text=f"Tool {name} executed with arguments: {arguments}"
        )]
