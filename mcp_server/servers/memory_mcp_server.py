...
FIRESTORE_PROJECT = os.getenv("FIRESTORE_PROJECT", os.getenv("GCP_PROJECT_ID"))
                ...
                if data:
                    ...
                    if query.query.lower() in memory['content'].lower():
                        results.append({
                            **memory,
                            'layer': 'short-term',
                            'score': 0.8  # Fixed score for text match
                        })
        except Exception as e:
...
async def check_consolidation_needed(agent_id: str):
    ...
    try:
        if redis_client:
            ...
            keys = redis_client.keys(f"memory:short:*")
            agent_memories = 0
            ...
            if agent_memories > 100:
                ...
                await consolidate_memories(
                    ConsolidationRequest(agent_id=agent_id)
                )
    except Exception as e:
