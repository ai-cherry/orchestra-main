...
class HybridSearchRequest(BaseModel):
    ...
    filters: Optional[Dict[str, Any]] = Field(None, description="Simple key-value filters for properties (e.g., {'property': 'value'}). More complex filters need GraphQL.")
    properties_to_return: Optional[List[str]] = Field(None, description="Specific properties to return for each object.")
...
async def startup_event():
    ...
    try:
        ...
        logger.critical(f"Critical error during Weaviate client initialization: {e}. Server might not function correctly.")
...
async def get_mcp_tools():
    tools = [
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    "class_names": MCPToolProperty(type="array", items={"type": "string"}, description="Optional list of class names.")
                }
            )
        ),
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    ...
                    "vector": MCPToolProperty(type="array", items={"type": "number"}, description="Optional pre-computed vector."),
        ...
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    ...
                    "include_vector": MCPToolProperty(type="boolean", description="Include vector in response (default: false).")
                },
        ...
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    ...
                    "alpha": MCPToolProperty(type="number", description="Weighting for hybrid (0=keyword, 1=vector, default 0.5)."),
                    "vector": MCPToolProperty(type="array", items={"type": "number"}, description="Optional query vector."),
                    "query_properties": MCPToolProperty(type="array", items={"type": "string"}, description="Properties for keyword search."),
                    ...
                    "properties_to_return": MCPToolProperty(type="array", items={"type": "string"}, description="Specific properties to return."),
                    "include_vector": MCPToolProperty(type="boolean", description="Include vector in results (default false).")
                },
        ...
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    "graphql_query": MCPToolProperty(type="string", description="The GraphQL query string.")
                },
...
async def add_object(request: AddObjectRequest, client: weaviate.WeaviateClient = Depends(get_client)):
    try:
        ...
        uuid_returned = collection.data.insert(
            ...
            uuid=request.uuid  # Let Weaviate generate if None
        )
...
async def get_object(request: GetObjectRequest, client: weaviate.WeaviateClient = Depends(get_client)):
    try:
        ...
        return GetObjectResponse(properties=None, error=f"Object with UUID {request.uuid} not found in {request.collection_name}.")
    except Exception as e:
