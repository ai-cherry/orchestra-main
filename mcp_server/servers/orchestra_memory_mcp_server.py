...
class MCPServiceRegistry(ServiceRegistry):
    ...
    async def initialize_services(self):
        ...
        try:
            ...
            if self._settings.dragonfly.enabled:
                ...
                self.dragonfly_client = redis_async.from_url(df_url, max_connections=self._settings.dragonfly.max_connections or 50)
                await self.dragonfly_client.ping()
                logger.info(f"Connected to DragonflyDB at {self._settings.dragonfly.host}:{self._settings.dragonfly.port}")
            ...
            if self._settings.mongodb.enabled:
                ...
                logger.info(f"Connected to MongoDB at {self._settings.mongodb.uri}, using database '{self._db_name_mongo}'")
            ...
            if not (self.dragonfly_client and await self.dragonfly_client.ping()): self.dragonfly_client = None
            if not self.mongodb_client: self.mongodb_client = None # Ping is implicit
            if not (self.weaviate_client_instance and self.weaviate_client_instance.is_ready()): self.weaviate_client_instance = None
    ...
    def get_service(self, service_name: str) -> Any:
        if service_name == "dragonfly" and self._settings.dragonfly.enabled:
            ...
            return None
        elif service_name == "weaviate_client" and self._settings.weaviate.enabled: # Match service name used in UnifiedMemoryService
...
async def get_mcp_tools_list():
    return [
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    "content": MCPToolProperty(type="object", description="The content to store (can be any JSON-serializable type)."),
                    "metadata": MCPToolProperty(type="object", description="Optional dictionary of metadata."),
                    "importance": MCPToolProperty(type="number", description="Optional importance score (e.g., 0.0 to 1.0)."),
                    "ttl_seconds": MCPToolProperty(type="integer", description="Optional time-to-live in seconds for short-term memory.")
                },
                required=["content"]
            )
        ),
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={"item_id": MCPToolProperty(type="string", description="The ID of the memory item to retrieve.")},
                required=["item_id"]
            )
        ),
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={
                    ...
                    "search_type": MCPToolProperty(type="string", description="Type of search: 'semantic', 'keyword', or 'hybrid' (default: 'hybrid')."),
                    "limit": MCPToolProperty(type="integer", description="Maximum number of results (default: 10)."),
                    "layers": MCPToolProperty(type="array", items={"type": "string"}, description="Optional list of layers to search: SHORT_TERM, MID_TERM, LONG_TERM."),
        ...
        MCPToolDefinition(
            ...
            parameters=MCPToolParameters(
                properties={"item_id": MCPToolProperty(type="string", description="The ID of the memory item to forget.")},
                required=["item_id"]
            )
        ),
        MCPToolDefinition(
...
async def health_check_endpoint(ums: UnifiedMemoryService = Depends(get_ums)):
    try:
        ...
        if overall_healthy:
            ...
            raise HTTPException(status_code=503, detail={"message": "One or more memory layers are unhealthy.", "details": layer_health_details})
    except Exception as e:
