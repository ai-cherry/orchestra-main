...
app = FastAPI(
    title="cherry_ai Memory MCP Server",
...
async def startup_event():
    ...
    logger.info("cherry_ai Memory MCP Server starting up...")
    try:
...
async def shutdown_event():
    logger.info("cherry_ai Memory MCP Server shutting down...")
...
@app.get("/mcp/cherry_ai_memory/tools", response_model=List[MCPToolDefinition])
async def get_mcp_tools_list():
    return [
        MCPToolDefinition(
            name="remember",
            description="Stores a piece of information in the cherry_aited memory system. The system will decide the appropriate layer based on policy.",
            parameters=MCPToolParameters(
                properties={
...
# --- Tool Endpoints ---
@app.post("/mcp/cherry_ai_memory/remember", response_model=RememberResponse)
...
@app.post("/mcp/cherry_ai_memory/recall", response_model=Optional[MemoryItemResponse])
...
@app.post("/mcp/cherry_ai_memory/search_memories", response_model=SearchMemoriesResponse)
...
@app.post("/mcp/cherry_ai_memory/forget", response_model=ForgetResponse)
...
@app.get("/mcp/cherry_ai_memory/get_memory_stats", response_model=GetMemoryStatsResponse)
...
async def health_check_endpoint(ums: UnifiedMemoryService = Depends(get_ums)):
    try:
        ...
        if overall_healthy:
            return {"status": "healthy", "service": "cherry_ai-memory-mcp", "details": layer_health_details}
        else:
...
if __name__ == "__main__":
    port = int(os.getenv("CHERRY_AI_CONDUCTOR_PORT", "8002"))
    host = os.getenv("MCP_CHERRY_AI_MEMORY_HOST", "0.0.0.0")

    logger.info(f"Starting cherry_ai Memory MCP Server on {host}:{port}")
