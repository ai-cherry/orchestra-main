...
try:
    ...
    logger.warning("Google Cloud libraries not installed. Install with 'pip install google-cloud-monitoring google-cloud-monitoring-dashboards'")
    HAS_GCP = False
    ...
    for policy_def in policies:
        ...
        for condition_def in policy_def["conditions"]:
            ...
            if "aggregations" in condition_def["condition"]:
                ...
                threshold.comparison = monitoring_v3.AlertPolicy.Condition.ComparisonType[condition_def["condition"]["comparison"]]
                ...
                if "denominatorFilter" in condition_def["condition"]:
                    threshold.denominator_filter = replace_variables(condition_def["condition"]["denominatorFilter"], variables)
    ...
    for check_def in uptime_checks:
        ...
        if "contentMatchers" in check_def:
            for matcher_def in check_def["contentMatchers"]:
                ...
                matcher.matcher = monitoring_v3.UptimeCheckConfig.ContentMatcher.ContentMatcherOption[matcher_def["matcher"]]
