...
class MemoryCompressionEngine:
    ...
    def compress(entry: MemoryEntry) -> MemoryEntry:
        ...
        if entry.content != compressed_entry.content:
            logger.debug(f"Compressed memory entry from {len(str(entry.content))} to {len(str(compressed_entry.content))} chars")
...
class TokenBudgetManager:
    ...
    def optimize_for_tool(self, entries: List[MemoryEntry], tool: ToolType) -> List[MemoryEntry]:
        ...
        sorted_entries = sorted(entries, key=lambda e: (e.priority, e.metadata.context_relevance), reverse=True)
