...
class StorageConfig(BaseModel):
    ...
    connection_string: Optional[str] = Field(
        default=None, description="Connection string for database-backed storage"
    )
    max_entries: int = Field(
        default=10000, description="Maximum number of entries to store"
    )
    ttl_seconds: int = Field(
        default=86400, description="Default TTL for entries (24 hours)"
    )
...
class CopilotConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Whether to enable the Copilot adapter"
    )
    ...
    vscode_extension_path: Optional[str] = Field(
        default=None, description="Path to VS Code Copilot extension"
    )
    use_openai_fallback: bool = Field(
        default=True, description="Whether to use OpenAI as a fallback"
    )
    openai_model: str = Field(
        default="gpt-3.5-turbo", description="OpenAI model to use as fallback"
    )
...
class GeminiConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Whether to enable the Gemini adapter"
    )
    ...
    embeddings_model: str = Field(
        default="embedding-001", description="Embeddings model to use"
    )
    token_limit: int = Field(
        default=200000, description="Token limit for context window"
    )
...
class MCPConfig(BaseModel):
    ...
    copilot: CopilotConfig = Field(
        default_factory=CopilotConfig, description="GitHub Copilot configuration"
    )
    gemini: GeminiConfig = Field(
        default_factory=GeminiConfig, description="Google Gemini configuration"
    )
