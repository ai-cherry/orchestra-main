#!/usr/bin/env python3
"""
Test script for the optimized memory components.

This script tests the basic functionality of the optimized memory storage
and the performance memory manager working together.
"""
...
from mcp_server.storage.sync_storage_adapter import SyncStorageAdapter
...
async def test_memory_components():
    ...
    logger.info("Starting memory components test...")
    ...
    storage = OptimizedMemoryStorage(storage_config)

    # Create synchronous adapter
    logger.info("Creating sync storage adapter...")
    sync_adapter = SyncStorageAdapter(storage)
    ...
    for item in search_result:
        logger.info(f"  - {item['key']}: {json.dumps(item['content'], indent=2)}")
    ...
    logger.info("Memory components test complete.")
    return True
