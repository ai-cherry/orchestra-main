...
class OperationContext(BaseModel):
    ...
    created_at: float = Field(default_factory=time.time, description="Timestamp when this operation was created")
    updated_at: float = Field(default_factory=time.time, description="Timestamp when this operation was last updated")
    memory_keys: List[str] = Field(default_factory=list, description="Keys of memory entries related to this operation")
        ...
        try:
            ...
            if target_modes:
                ...
                await self.transition_manager.prepare_transition(
                    initial_mode, first_target, operation_id, 
                    {
        ...
        if next_index >= len(context.target_modes):
            ...
            try:
                ...
                transition_context = await self.transition_manager.prepare_transition(
                    current_mode, context.return_mode, operation_id,
                    {
                        "memory_key": memory_key, 
                        "step": "final", 
                        "complete": True,
