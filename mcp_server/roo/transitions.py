...
class TransitionContext(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this transition")
    ...
    timestamp: float = Field(default_factory=time.time, description="Timestamp when this transition was created")
    memory_keys: List[str] = Field(default_factory=list, description="Keys of memory entries related to this transition")
