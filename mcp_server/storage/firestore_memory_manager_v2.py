...
class FirestoreMemoryManagerV2(IMemoryManager):
    ...
    async def create_memory(self, key: str, entry: MemoryEntry, source_tool: str) -> bool:
        ...
        async with self._batch_lock:
            self._batch_operations.append(
                BatchOperation("set", key, entry.to_dict())
            )
    ...
    async def get_memory_status(self) -> Dict[str, Any]:
        ...
        cache_stats = {
            ...
            "usage_percentage": round((len(self._cache) / self.max_cache_size) * 100, 2) if self.max_cache_size > 0 else 0
        }
    ...
    async def _process_batch_operations(self) -> None:
        ...
        while True:
            try:
                ...
                if (current_time - self._last_batch_flush >= self.batch_flush_interval and 
                    len(self._batch_operations) > 0):
                    await self._flush_batch_operations()
