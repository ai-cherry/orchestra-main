...
async def demo_file_ingestion(pipeline: DataIngestionPipeline) -> None:
    ...
    csv_file.write_text("""date,product,quantity,price
...
2024-01-05,Widget B,90,39.99""")
    ...
    json_file.write_text(json.dumps({
        "customers": [
            {"id": 1, "name": "Alice", "email": "alice@example.com", "orders": 5},
            {"id": 2, "name": "Bob", "email": "bob@example.com", "orders": 3},
            {"id": 3, "name": "Charlie", "email": "charlie@example.com", "orders": 7}
        ],
        "metadata": {
            "exported": "2024-01-15",
            "version": "1.0"
...
<inventory>
    ...
    csv_results = await pipeline.ingest_file(
        csv_file,
        processor_kwargs={"chunk_size": 2}  # Small chunk for demo
    )
...
async def main():
    ...
    pipeline = DataIngestionPipeline(
        enable_embedding_generation=False,  # Disabled for demo
        batch_size=10
    )
