...
# Script to run the Phidata UI dashboard
...
# Run the dependency check script if available
if [ -f "test_phi_import.py" ]; then
    echo "Running dependency check script..."
    python test_phi_import.py
fi
...
# Run the dashboard
echo "Starting Phidata UI dashboard..."
