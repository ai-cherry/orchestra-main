...
class SyncResult:
    ...
    def get_references_by_type(self, reference_type: ReferenceType) -> List[DocReference]:
...
class WIFDocsSynchronizer:
    ...
    REFERENCE_PATTERNS = [
        ...
        (r'\[(.*?)\]\(((?:\.\/)?(?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?)\)', ReferenceType.INLINE_LINK),
        (r'\[(.*?)\]\(((?:\.\/)?github-workflow-wif-template\.yml(?:\.updated)?)\)', ReferenceType.INLINE_LINK),
        (r'\[(.*?)\]\(((?:\.\/)?docs\/workload_identity_federation\.md)\)', ReferenceType.INLINE_LINK),
        
        # Reference links: [text][ref] or [ref]
        (r'\[(.*?)\]\[((?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?)\]', ReferenceType.REFERENCE_LINK),
        (r'\[(.*?)\]\[(github-workflow-wif-template\.yml(?:\.updated)?)\]', ReferenceType.REFERENCE_LINK),
        (r'\[(.*?)\]\[(workload_identity_federation\.md)\]', ReferenceType.REFERENCE_LINK),
        
        # Code blocks with commands
        (r'```(?:bash|sh).*?((?:\.\/)?(?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?).*?```', ReferenceType.CODE_BLOCK),
        (r'```(?:bash|sh).*?((?:\.\/)?github-workflow-wif-template\.yml(?:\.updated)?).*?```', ReferenceType.CODE_BLOCK),
        
        # Commands in text
        (r'`((?:\.\/)?(?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?)`', ReferenceType.COMMAND),
        (r'`((?:\.\/)?github-workflow-wif-template\.yml(?:\.updated)?)`', ReferenceType.COMMAND),
        
        # Headings
        (r'^#+\s+.*?((?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?).*?$', ReferenceType.HEADING),
        (r'^#+\s+.*?(github-workflow-wif-template\.yml(?:\.updated)?).*?$', ReferenceType.HEADING),
        (r'^#+\s+.*?(workload_identity_federation\.md).*?$', ReferenceType.HEADING),
        
        # Plain text references
        (r'(?<![`\[])((?:setup_github_secrets|update_wif_secrets|verify_github_secrets|github_auth)\.sh(?:\.updated)?)(?![`\]])', ReferenceType.TEXT),
        (r'(?<![`\[])(github-workflow-wif-template\.yml(?:\.updated)?)(?![`\]])', ReferenceType.TEXT),
    ...
    def __init__(
        ...
        logger.debug(f"Initialized WIF documentation synchronizer with base path: {self.base_path}")
    ...
    def scan_file(self, file_path: Path) -> List[DocReference]:
            ...
            if reference_type == ReferenceType.CODE_BLOCK:
                ...
                for match in re.finditer(pattern, content, re.DOTALL):
                    ...
                    if old_reference != new_reference:
                        ...
                        line_content = lines[line_number] if line_number < len(lines) else ""
        ...
        for line_number, line in enumerate(lines):
            for pattern, reference_type in self.REFERENCE_PATTERNS:
                if reference_type != ReferenceType.CODE_BLOCK:  # Skip code blocks, already handled
    ...
    def scan_directory(self, directory_path: Optional[Path] = None) -> SyncResult:
        ...
        logger.info(f"Scan complete. Found {len(self.sync_result.references)} references in {self.sync_result.scanned_files} files.")
    ...
    def update_references(self) -> SyncResult:
        ...
        logger.info(f"Updated {len(self.sync_result.get_updated_references())} references in {len(self.sync_result.modified_files)} files.")
        
        return self.sync_result
    
    def _update_file_references(self, file_path: str, references: List[DocReference]) -> None:
        """
        ...
        if self.backup:
            backup_path = f"{file_path}.wif-docs-backup.{datetime.now().strftime('%Y%m%d%H%M%S')}"
            try:
        ...
        code_block_references = [r for r in sorted_references if r.reference_type == ReferenceType.CODE_BLOCK]
        non_code_block_references = [r for r in sorted_references if r.reference_type != ReferenceType.CODE_BLOCK]
        ...
        for reference in code_block_references:
            ...
            pattern = re.compile(f"```(?:bash|sh).*?{re.escape(reference.old_reference)}.*?```", re.DOTALL)
            updated_content = pattern.sub(lambda m: m.group(0).replace(reference.old_reference, reference.new_reference), updated_content)
            reference.updated = True
            logger.debug(f"Updated code block reference in {file_path}: {reference.old_reference} -> {reference.new_reference}")
        ...
        for reference in non_code_block_references:
            if reference.reference_type == ReferenceType.INLINE_LINK:
                ...
                updated_content = pattern.sub(f"[\\1]({reference.new_reference})", updated_content)
                reference.updated = True
                ...
                pattern = re.compile(f"`{re.escape(reference.old_reference)}`")
                updated_content = pattern.sub(f"`{reference.new_reference}`", updated_content)
                reference.updated = True
            elif reference.reference_type == ReferenceType.HEADING:
                # For headings, we need to replace in the heading
                pattern = re.compile(f"^(#+\\s+.*?){re.escape(reference.old_reference)}(.*?)$", re.MULTILINE)
                updated_content = pattern.sub(f"\\1{reference.new_reference}\\2", updated_content)
                reference.updated = True
            ...
            logger.debug(f"Updated {reference.reference_type.value} reference in {file_path}: {reference.old_reference} -> {reference.new_reference}")
