...
FAILED_BUILD=$(gcloud builds list --filter="status=FAILURE" --limit=1 --format="value(id)")

if [ -n "$FAILED_BUILD" ]; then
    echo "Found failed build: $FAILED_BUILD"
    echo "Getting error details..."
    gcloud builds log $FAILED_BUILD 2>&1 | tail -50 | grep -E "(ERROR|FAILED|error:|Error:|Failed)"
else
...
for SERVICE in "ai-orchestra-minimal" "web-scraping-agents"; do
    STATUS=$(gcloud run services describe $SERVICE --region=$REGION --format="value(status.conditions[0].message)" 2>/dev/null)
    if [ -n "$STATUS" ]; then
        echo "$SERVICE: $STATUS"
    else
