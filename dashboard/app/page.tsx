...
export default function HomePage() {
    ...
    const apiUrl = process.env.NEXT_PUBLIC_DASHBOARD_API_URL || "http://localhost:8080"; // Placeholder
    ...
    return (
        <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-gray-800">
            <div className="bg-gray-700 p-8 rounded-lg shadow-xl w-full max-w-md">
                <h1 className="text-3xl font-bold mb-6 text-center text-sky-400">AI Agent Dashboard</h1>

                <div className="mb-6">
                    <h2 className="text-xl font-semibold mb-2 text-sky-300">Agent API URL</h2>
                    <a
                        href={apiUrl}
                        target="_blank"
                ...
                <div>
                    <h2 className="text-xl font-semibold mb-3 text-sky-300">Agent Health Status</h2>
                    {isLoading ? (
                        ...
                        <div className="flex items-center">
                            ...
                            <span className="text-lg">
                                {health?.status ? `Status: ${health.status}` : (health?.error ? `Error: ${health.error}` : "Unknown")}
                            </span>
                        </div>
                    )}
                </div>
