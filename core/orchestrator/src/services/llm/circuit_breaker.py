...
class CircuitBreaker:
    ...
    def _get_circuit_state(self, provider: str) -> CircuitState:
        ...
        with self._lock:
            ...
            if (self.circuit_state[provider] == CircuitState.OPEN and
                    provider in self.last_failure_time and
                    time.time() - self.last_failure_time[provider] > self.reset_timeout):
    ...
    async def call_provider_async(self, provider: str, func: Callable[..., Awaitable[T]], *args: Any, **kwargs: Any) -> T:
        """
