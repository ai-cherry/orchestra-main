...
async def get_memory_manager(
    ...
    if _memory_manager is None:
        ...
        try:
            ...
            logger.info(
                f"Memory manager initialized: {type(_memory_manager).__name__}")
        except Exception as e:
...
async def initialize_memory_manager(settings: Settings = None) -> None:
    ...
    try:
        logger.info(
            f"Initializing memory manager: {type(_memory_manager).__name__}")
        await _memory_manager.initialize()
...
async def close_memory_manager() -> None:
    ...
    if _memory_manager is not None:
        try:
            logger.info(
                f"Closing memory manager: {type(_memory_manager).__name__}")
...
async def get_memory_service(
    ...
    if _memory_service is None:
        try:
            ...
            if settings.ENVIRONMENT in ["prod", "production", "stage", "staging"] and settings.get_gcp_project_id() is not None:
                ...
                storage_type = "memory"
                storage_config = {
                    "namespace": f"orchestra-{settings.ENVIRONMENT}"
                }

            logger.info(
                f"Creating memory service with {storage_type} storage adapter")
