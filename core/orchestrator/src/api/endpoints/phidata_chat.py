...
from packages.ingestion.src.api.chat_integration import get_ingestion_middleware, process_phidata_message, IngestionChatMiddleware
...
async def phidata_chat(
    ...
    try:
        ...
        history_items = await memory.get_conversation_history(
        ...
        if not persona_config:
            raise HTTPException(
                status_code=500, 
                detail="No persona configuration available"
            )
        ...
        if isinstance(agent_response.text, dict) or (
            agent_response.metadata and 
            agent_response.metadata.get("content_type") in ["application/json", "structured"]
        ):
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to process Phidata chat: {str(e)}"
        )
