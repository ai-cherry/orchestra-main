...
class WorkflowEngine:
    ...
    async def _transition_state(
        ...
        if to_state in [WorkflowState.COMPLETED, WorkflowState.FAILED, WorkflowState.CANCELLED]:
        ...
        for transition in valid_transitions:
            ...
            if transition.condition_name:
                if transition.condition_name not in self._conditions:
                    logger.warning(
                        f"Unknown condition: {transition.condition_name}"
                    )
                    continue
            ...
            if transition.action_name:
                if transition.action_name not in self._actions:
                    logger.warning(
                        f"Unknown action: {transition.action_name}"
                    )
                    continue
