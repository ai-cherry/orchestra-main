...
class Workflow:
    ...
    def completed(self) -> bool:
        ...
        return (
            self.current_step >= len(self.steps) or
            all(step.completed for step in self.steps)
        )
...
class ModeManager:
    ...
    def advance_workflow(self) -> Tuple[bool, str]:
        ...
        return True, f"Advanced to step {self.current_workflow.current_step + 1}: {next_step.task}"
    ...
    def suggest_next_modes(self, limit: int = 3) -> List[Dict[str, str]]:
        ...
        if not self.current_mode:
            ...
            for slug in reversed(self.history[-5:]):
                if (
                    slug in self.modes and 
                    slug != self.current_mode.slug and
                    not any(s["slug"] == slug for s in suggestions)
                ):
                    mode = self.modes[slug]
                    suggestions.append({
                        "slug": slug,
        ...
        try:
            ...
            state = {
                ...
                "start_time": self.current_workflow.start_time.isoformat() 
                    if self.current_workflow.start_time else None,
                "end_time": self.current_workflow.end_time.isoformat() 
                    if self.current_workflow.end_time else None,
