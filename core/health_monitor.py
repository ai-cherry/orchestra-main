...
class HealthMonitor:
    ...
    def __init__(self, check_interval: int = 60, check_timeout: int = 10, failure_threshold: int = 3):
        """
    ...
    async def _check_and_update_component(self, name: str, check_func: Callable):
        ...
        async with self._lock:
            self._components[name] = ComponentHealth(
                name=name, status=status, last_check=time.time(), error_message=error_msg
            )
