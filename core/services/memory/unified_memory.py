...
class LongTermStore(MemoryStore):
    ...
    async def store(self, item: MemoryItem) -> bool:
        ...
        try:
            ...
            uuid_generated = collection.data.insert(
    ...
    async def delete(self, item_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Attempted deletion of item {item_id} (Weaviate UUID {weaviate_uuid}) from {self.class_name}.")
            return True
        except Exception as e:
    ...
    async def search(
        ...
        try:
            ...
            if search_type == "semantic":
                ...
                response = collection.query.hybrid(
                    query=query_text, alpha=0.0, query_properties=query_properties or ["content", "metadata_json"], **common_params
                )
            else:
            ...
            logger.error(f"Error searching in Weaviate ({self.class_name}) with type '{search_type}': {e}", exc_info=True)
            return []
...
class UnifiedMemoryService(MemoryService):
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Unified memory service initialized with {len(self.stores)} stores: {list(self.stores.keys())}"
        )
    ...
    async def store(
        ...
        if vector: metadata["vector"] = vector # Pass vector via metadata to LongTermStore

        effective_layer = layer or self.policy.select_layer(content, metadata)
        
        item = MemoryItem(
            id=item_id, content=content, metadata=metadata,
            timestamp=datetime.utcnow(), layer=effective_layer, ttl=ttl
        )

        store = self.stores.get(effective_layer)
        if not store: raise ValueError(f"No store available for layer {effective_layer}")

        success = await store.store(item)
        if not success: raise RuntimeError(f"Failed to store item {item_id} in {effective_layer}")
    ...
    def _translate_simple_filters_to_weaviate(self, simple_filters: Optional[Dict[str, Any]]) -> Optional[wvcq.Filter]:
        ...
        for key_op, value in simple_filters.items():
            ...
            try:
                prop_filter_starter = wvcq.Filter.by_property(path_list[0]) if len(path_list) == 1 else wvcq.Filter.by_property(path_list)
                ...
                if operator == "eq":
                    ...
                    if not isinstance(value, str):
                        logger.warning(f"'like' operator for '{path_str}' requires a string value. Got {type(value)}. Skipping.")
                        continue
                    conditions.append(prop_filter_starter.like(value))
                elif operator == "is_none":
                     if not isinstance(value, bool):
                        logger.warning(f"'is_none' operator for '{path_str}' requires a boolean value. Got {type(value)}. Skipping.")
                        continue
                     conditions.append(wvcq.Filter.by_property(path_list).is_none(value)) # is_none is different
                elif operator == "contains_any":
                    if not isinstance(value, list):
                        logger.warning(f"'contains_any' operator for '{path_str}' requires a list value. Got {type(value)}. Skipping.")
                        continue
                    conditions.append(prop_filter_starter.contains_any(value))
                elif operator == "contains_all":
                    if not isinstance(value, list):
                        logger.warning(f"'contains_all' operator for '{path_str}' requires a list value. Got {type(value)}. Skipping.")
                        continue
                    conditions.append(prop_filter_starter.contains_all(value))
                else:
                    logger.warning(f"Unsupported Weaviate filter operator '{operator}' for path '{path_str}'. Skipping.")
            except Exception as e:
    ...
    async def search(
        ...
        if not target_layers:
            if search_type == "semantic" or search_type == "hybrid":
                if MemoryLayer.LONG_TERM in self.stores: target_layers.append(MemoryLayer.LONG_TERM)
                if search_type == "hybrid" and MemoryLayer.MID_TERM in self.stores: # Optionally include mid-term for hybrid's keyword part
                    target_layers.append(MemoryLayer.MID_TERM)
            elif search_type == "keyword":
                if MemoryLayer.MID_TERM in self.stores: target_layers.append(MemoryLayer.MID_TERM)
                if MemoryLayer.LONG_TERM in self.stores: target_layers.append(MemoryLayer.LONG_TERM) # Weaviate can also do keyword
            if not target_layers: target_layers = list(self.stores.keys())
        ...
        for layer_enum in target_layers:
            ...
            if isinstance(store, LongTermStore):
                ...
                task = store.search(
                    query_text=query, limit=limit, filters=weaviate_native_filters,
                    search_type=search_type, alpha=alpha, vector=query_vector,
        ...
        for res in all_results:
            if res.item.id not in unique_results_dict or \
               (res.score is not None and unique_results_dict[res.item.id].score is not None and res.score > unique_results_dict[res.item.id].score) or \
               (res.score is not None and unique_results_dict[res.item.id].score is None): # Prefer scored results
    ...
    async def promote(self, item_id: str, target_layer: MemoryLayer, original_item: Optional[MemoryItem] = None) -> bool:
    ...
    async def evict(self, item_id: str) -> bool:
        ...
        if item_id in self._access_counts: del self._access_counts[item_id]
        if item_id in self._last_access: del self._last_access[item_id]
        
        if deleted_any: logger.info(f"Evicted item {item_id} from relevant stores.")
        return deleted_any
