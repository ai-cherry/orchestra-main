...
"""
"""
    """Generates a comprehensive summary of Cherry AI deployment"""
            "conductor_api": {
                "port": 8000,
                "process_name": "gunicorn",
                "description": "Main conductor API"
            },
            "weaviate": {
                "port": 8080,
    ...
    def generate_summary(self) -> str:
        ...
        summary = f"""
"""
        conductor_running, conductor_info = self.check_process("gunicorn")
        conductor_port = self.check_port(8000)
        summary += f"\n✅ conductor API (Port 8000):\n"
        summary += f"   Process: {'Running' if conductor_running else 'Not Running'}\n"
        if conductor_info:
            summary += f"   {conductor_info}\n"
        summary += f"   Port: {'Open' if conductor_port else 'Closed'}\n"
...
"""
    """Main function"""
    logger.info("Generating Cherry AI deployment summary...")
    
    summary_generator = cherry_aiDeploymentSummary()
