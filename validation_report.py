...
class EnvironmentValidator:
    ...
    def validate_figma_gcp_sync(self) -> None:
        ...
        if figma_script_path.exists():
            ...
            if 'generate_terraform' in script_content:
                self.results.success("Terraform Variables", "Can convert Figma variables to Terraform variables.tf")
            else:
                self.results.failure("Terraform Variables", "Cannot find Terraform conversion functionality")
                
            # Check for Secret Manager update via vertex_agent SA
            if 'update_secret' in script_content and 'GCPManager' in script_content:
                self.results.success("Secret Manager Update", "Can update Secret Manager via vertex_agent SA")
            else:
                self.results.failure("Secret Manager Update", "Cannot find Secret Manager update functionality")
                
            # Check for platform-specific style file generation
            style_generators = ['generate_css', 'generate_js', 'generate_android_xml', 'generate_ios_swift']
            ...
            if len(found_generators) >= 3:  # At least 3 platforms supported
                self.results.success("Style Generation", f"Can generate platform-specific style files ({len(found_generators)} platforms)")
            else:
                self.results.failure("Style Generation", f"Limited platform support for style generation (only {len(found_generators)} platforms)")
        else:
        ...
        if not missing_scopes:
            self.results.pending("FIGMA_PAT Permissions", "Scope validation requires manual verification")
        else:
            self.results.warning("FIGMA_PAT Permissions", f"Script doesn't explicitly check for these scopes: {', '.join(missing_scopes)}")
    ...
    def validate_component_library(self) -> None:
        ...
        variables_js_path = self.root_path / 'packages' / 'ui' / 'src' / 'tokens' / 'variables.js'
        android_styles_path = self.root_path / 'packages' / 'ui' / 'android' / 'src' / 'main' / 'res' / 'values' / 'styles.xml'
        
        if not mapping_path.exists():
            self.results.failure("Mapping File", "component-adaptation-mapping.json does not exist")
            return
            
        if not variables_js_path.exists():
            self.results.failure("Variables JS", "packages/ui/src/tokens/variables.js does not exist")
            return
            
        if not android_styles_path.exists():
            self.results.failure("Android Styles", "packages/ui/android/src/main/res/values/styles.xml does not exist")
            return
        ...
        button_variants_in_mapping = [k for k in mapping_data.keys() if k.startswith("Button")]
        ...
        if len(button_styles_in_android) >= 4:
            ...
            self.results.success("Button Variants", f"Found {len(button_styles_in_android)} button styles in Android: {button_styles_str}")
        else:
            self.results.failure("Button Variants", f"Expected at least 4 button variants, found {len(button_styles_in_android)}")
            
        # Check for Card elevation states (should have 2)
        card_variants_in_mapping = [k for k in mapping_data.keys() if k.startswith("Card")]
        ...
        if len(card_styles_in_android) >= 2:
            ...
            self.results.success("Card Elevation States", f"Found {len(card_styles_in_android)} card styles in Android: {card_styles_str}")
        else:
            self.results.failure("Card Elevation States", f"Expected at least 2 card elevation states, found {len(card_styles_in_android)}")
            
        # Check for Input validation styles (should have 3)
        input_variants_in_mapping = [k for k in mapping_data.keys() if k.startswith("Input")]
        input_fields_in_android = re.findall(r'TextInputLayout|TextInputEditText|boxStrokeColor|errorTextColor', android_styles)
        
        input_validation_count = len(set(input_fields_in_android))
        if input_validation_count >= 3:
            self.results.success("Input Validation Styles", f"Found sufficient input validation styles: {input_validation_count} relevant properties")
        else:
            self.results.failure("Input Validation Styles", f"Expected at least 3 input validation styles, found {input_validation_count} relevant properties")
        ...
        if match_percentage >= 80:
            self.results.success("Match component-adaptation-mapping.json", f"Found {match_percentage:.0f}% of components in implementation files")
        else:
            self.results.warning("Match component-adaptation-mapping.json", f"Only {match_percentage:.0f}% of components found in implementation files")
    ...
    def validate_infrastructure(self) -> None:
        ...
        if not tf_config_path.exists():
            self.results.failure("Terraform Config", "infra/vertex_workbench_config.tf does not exist")
            return
            
        if not tf_vars_path.exists():
            self.results.warning("Terraform Variables", "infra/vertex_workbench.tfvars does not exist")
        ...
        if "machine_type" in tf_config and "n1-standard-4" in tf_config:
            self.results.success("Vertex AI Workbench", "Configured with n1-standard-4 instance type")
        else:
            self.results.failure("Vertex AI Workbench", "Not configured with n1-standard-4 instance type")
            
        # Check for Firestore NATIVE with backup policies
        if "google_firestore_database" in tf_config and "type                        = \"FIRESTORE_NATIVE\"" in tf_config:
            if "google_storage_bucket" in tf_config and "firestore_backups" in tf_config:
                self.results.success("Firestore NATIVE", "Configured with backup policies")
            else:
                self.results.warning("Firestore NATIVE", "Configured but without clear backup policies")
        else:
        ...
        if "google_redis_instance" in tf_config and "memory_size_gb = 3" in tf_config:
            ...
            self.results.failure("Memorystore Redis", "Not configured with 3GB capacity")
        ...
        if not missing_roles:
            self.results.success("Service Account Roles", "All required roles are assigned")
        else:
            self.results.failure("Service Account Roles", f"Missing roles: {', '.join(missing_roles)}")
    ...
    def validate_cicd_pipeline(self) -> None:
        ...
        if not github_actions_dir.exists():
            self.results.warning("GitHub Actions Directory", ".github/workflows directory does not exist")
            
        # Since we don't have clear access to the GitHub workflows, we'll mark these as pending
        self.results.pending("GitHub Actions Trigger", "Trigger on Figma file changes (webhook ID:368236963)")
        self.results.pending("GitHub Actions Validation", "Run vertex-ai-validate on style changes")
        self.results.pending("GitHub Actions Deployment", "Deploy to Cloud Run with canary staging")
        
        # Check for secrets mapping
        secrets_script_path = self.root_path / 'scripts' / 'update_github_org_secrets.sh'
        
        if not secrets_script_path.exists():
            self.results.failure("Secrets Mapping Script", "scripts/update_github_org_secrets.sh does not exist")
            return
        ...
        if not missing_mappings:
            self.results.success("Secrets Mapping", "All required secrets are properly mapped")
        else:
            self.results.failure("Secrets Mapping", f"Missing mappings: {', '.join(missing_mappings)}")
    ...
    def validate_ai_requirements(self) -> None:
        ...
        for tool_name, min_version in cline_tools:
            self.results.pending(f"Cline MCP: {tool_name}", f"Version {min_version}+ required")
        ...
        for py_file in self.root_path.glob('**/*.py'):
            ...
            if 'from google.cloud import aiplatform' in py_content and 'validate' in py_content.lower() and 'design' in py_content.lower():
                vertex_ai_check_1 = True
                
            if 'from google.cloud import aiplatform' in py_content and 'component' in py_content.lower() and 'test' in py_content.lower():
                vertex_ai_check_2 = True
                
        if vertex_ai_check_1:
            self.results.success("Vertex AI Design Token Validation", "Found code for validating design tokens via AutoML")
        else:
            self.results.pending("Vertex AI Design Token Validation", "Could not verify AutoML design token validation")
            
        if vertex_ai_check_2:
            self.results.success("Vertex AI Component Test Cases", "Found code for generating component test cases")
        else:
            self.results.pending("Vertex AI Component Test Cases", "Could not verify component test case generation")
