...
class UnifiedMemoryManager:
    ...
    def __init__(self, default_tool: AITool = None):
        self.default_tool = default_tool or (
            AITool.ROO if roo_available else AITool.CLINE if cline_available else None
        )
...
class UnifiedModeManager:
    ...
    def map_mode(mode: str, source_tool: AITool, target_tool: AITool) -> str:
        ...
        mode_mappings = {
            ...
            (AITool.CLINE, AITool.ROO): {"plan": "architect", "act": "code"},
        }
    ...
    @staticmethod
    def execute_in_mode(
        tool: AITool, mode: str, prompt: str, context: Optional[str] = None
    ) -> Optional[str]:
        ...
        if not mode_manager:
            logger.error(
                f"Cannot execute in mode: no mode manager available for {tool}"
            )
            return None
        ...
        for step in self.steps:
            ...
            if "mode" in step:
                if tool == AITool.ROO and step["mode"] not in ROO_MODE_MAP:
                    adapted_step["mode"] = UnifiedModeManager.map_mode(
                        step["mode"], AITool.CLINE, AITool.ROO
                    )
                elif tool == AITool.CLINE and step["mode"] not in CLINE_MODE_MAP:
                    adapted_step["mode"] = UnifiedModeManager.map_mode(
                        step["mode"], AITool.ROO, AITool.CLINE
                    )
...
class UnifiedWorkflowOrchestrator:
    ...
    def execute_cross_tool_workflow(
        self, workflow_name: str, tools: List[AITool] = None
    ) -> str:
...
class MCPServerManager:
    ...
    @staticmethod
    def execute_server_operation(
        server_name: str, operation: str, parameters: Dict[str, Any] = None
    ) -> Optional[str]:
...
def main():
    ...
    workflow_parser = subparsers.add_parser(
        "workflow", help="Execute a unified workflow"
    )
    workflow_parser.add_argument(
        "--name",
        required=True,
        choices=UNIFIED_WORKFLOWS.keys(),
        help="Name of the workflow to execute",
    )
    workflow_parser.add_argument(
        "--tool", choices=[t.value for t in AITool], help="Tool to use for execution"
    )

    # Cross-tool workflow command
    cross_tool_parser = subparsers.add_parser(
        "cross-tool", help="Execute a workflow across multiple tools"
    )
    cross_tool_parser.add_argument(
        "--name",
        required=True,
        choices=UNIFIED_WORKFLOWS.keys(),
        help="Name of the workflow to execute",
    ...
    cross_tool_parser.add_argument(
        "--tools",
        nargs="+",
        choices=[t.value for t in AITool],
    ...
    memory_subparsers = memory_parser.add_subparsers(
        dest="memory_command", help="Memory operation to perform"
    )
    ...
    get_memory_parser.add_argument(
        "--scope",
    ...
    set_memory_parser.add_argument(
        "--scope",
        choices=[s.value for s in MemoryScope],
        default="session",
        help="Memory scope",
    ...
    # Sync memory
    sync_memory_parser = memory_subparsers.add_parser(
        "sync", help="Sync memory between tools"
    )
    sync_memory_parser.add_argument("--key", required=True, help="Memory key")
    sync_memory_parser.add_argument(
        "--source", required=True, choices=[t.value for t in AITool], help="Source tool"
    )
    sync_memory_parser.add_argument(
        "--target", required=True, choices=[t.value for t in AITool], help="Target tool"
    )
    sync_memory_parser.add_argument(
        "--scope",
        choices=[s.value for s in MemoryScope],
    ...
    server_parser = subparsers.add_parser(
        "server", help="Perform MCP server operations"
    )
    server_subparsers = server_parser.add_subparsers(
        dest="server_command", help="Server operation to perform"
    )

    # List servers
    list_servers_parser = server_subparsers.add_parser(
        "list", help="List available MCP servers"
    )

    # Check server status
    status_server_parser = server_subparsers.add_parser(
    ...
    execute_server_parser = server_subparsers.add_parser(
        "execute", help="Execute operation on MCP server"
    )
    execute_server_parser.add_argument("--name", required=True, help="Server name")
    execute_server_parser.add_argument(
        "--operation", required=True, help="Operation name"
    )
    execute_server_parser.add_argument("--parameters", help="JSON string of parameters")

    # List workflows
    list_workflows_parser = subparsers.add_parser(
        "list-workflows", help="List available unified workflows"
    )
    ...
    if args.command == "workflow":
        ...
        if args.memory_command == "get":
            ...
            content = memory_manager.retrieve(
                args.key, scope=MemoryScope(args.scope), tool=tool
            )
            if content:
                print(content)
            else:
            ...
            result = memory_manager.save(
                args.key, args.content, scope=MemoryScope(args.scope), tool=tool
            )
        ...
        if args.server_command == "list":
            ...
            result = server_manager.execute_server_operation(
                args.name, args.operation, parameters
            )
            if result:
                print(result)
            else:
