...
def import_from_script(script_path: str, function_name: str):
    ...
    try:
        ...
        print(f"{Colors.RED}Error importing {function_name} from {script_path}: {str(e)}{Colors.ENDC}")
        return None
...
def validate_component_mapping() -> dict:
    ...
    if exit_code == 0:
        ...
        if "Input validation" in stdout:
            match = re.search(r"Found sufficient input validation styles: (\d+)", stdout)
...
def validate_infrastructure() -> dict:
    ...
    if "machine_type" in tf_config and "n1-standard-4" in tf_config:
        ...
        print(f"{Colors.GREEN}✓ Vertex AI Workbench properly configured with n1-standard-4{Colors.ENDC}")
    else:
        results["vertex_ai"] = {
            "status": "failure",
            "message": "Vertex AI Workbench not configured with n1-standard-4"
        }
        print(f"{Colors.RED}✗ Vertex AI Workbench not configured with n1-standard-4{Colors.ENDC}")
    ...
    if "google_firestore_database" in tf_config and "FIRESTORE_NATIVE" in tf_config:
        if "google_storage_bucket" in tf_config and "firestore_backups" in tf_config:
            ...
            print(f"{Colors.GREEN}✓ Firestore NATIVE configured with backup policies{Colors.ENDC}")
        else:
            results["firestore"] = {
                "status": "warning",
                "message": "Firestore NATIVE configured without clear backup policies"
            }
            print(f"{Colors.YELLOW}⚠ Firestore NATIVE configured without clear backup policies{Colors.ENDC}")
    else:
        results["firestore"] = {
            "status": "failure",
    ...
    if "google_redis_instance" in tf_config and "memory_size_gb = 3" in tf_config:
        ...
        print(f"{Colors.GREEN}✓ Memorystore Redis configured with 3GB capacity{Colors.ENDC}")
    else:
        results["redis"] = {
            "status": "failure",
            "message": "Memorystore Redis not configured with 3GB capacity"
        }
        print(f"{Colors.RED}✗ Memorystore Redis not configured with 3GB capacity{Colors.ENDC}")
    ...
    if not missing_roles:
        ...
        print(f"{Colors.RED}✗ Service account missing roles: {', '.join(missing_roles)}{Colors.ENDC}")
    
    # Overall result is successful if all checks passed
    results["passed"] = (
        results.get("vertex_ai", {}).get("status") == "success" and
        results.get("firestore", {}).get("status") in ["success", "warning"] and
        results.get("redis", {}).get("status") == "success" and
        results.get("roles", {}).get("status") == "success"
    )
...
def validate_cicd_pipeline() -> dict:
    ...
    if not os.path.exists(workflow_path):
        ...
        if "repository_dispatch" in workflow_content and "figma-update" in workflow_content:
            results["trigger"] = {
                "status": "success",
                "message": "GitHub Actions configured to trigger on Figma file changes"
            }
            print(f"{Colors.GREEN}✓ GitHub Actions configured to trigger on Figma file changes{Colors.ENDC}")
        else:
            results["trigger"] = {
                "status": "warning",
                "message": "GitHub Actions may not be configured to trigger on Figma file changes"
            }
            print(f"{Colors.YELLOW}⚠ GitHub Actions may not be configured to trigger on Figma file changes{Colors.ENDC}")
        ...
        if "validate" in workflow_content and "design" in workflow_content:
            ...
            print(f"{Colors.GREEN}✓ GitHub Actions includes validation step for design tokens{Colors.ENDC}")
        else:
            results["validation"] = {
                "status": "warning",
                "message": "GitHub Actions may not include validation step for design tokens"
            }
            print(f"{Colors.YELLOW}⚠ GitHub Actions may not include validation step for design tokens{Colors.ENDC}")
        ...
        if "Cloud Run" in workflow_content and "canary" in workflow_content:
            ...
            print(f"{Colors.GREEN}✓ GitHub Actions includes Cloud Run deployment with canary staging{Colors.ENDC}")
        else:
            results["deployment"] = {
                "status": "warning",
                "message": "GitHub Actions may not include Cloud Run deployment with canary staging"
            }
            print(f"{Colors.YELLOW}⚠ GitHub Actions may not include Cloud Run deployment with canary staging{Colors.ENDC}")
    ...
    if not os.path.exists(secrets_script_path):
        ...
        if not missing_mappings:
            ...
            print(f"{Colors.GREEN}✓ All required secrets are properly mapped{Colors.ENDC}")
        else:
            results["secrets"] = {
                "status": "failure",
                "message": f"Missing secret mappings: {', '.join(missing_mappings)}"
            }
            print(f"{Colors.RED}✗ Missing secret mappings: {', '.join(missing_mappings)}{Colors.ENDC}")
    ...
    if not os.path.exists(webhook_script_path):
        print(f"{Colors.YELLOW}Warning: {webhook_script_path} doesn't exist{Colors.ENDC}")
    ...
    results["passed"] = (
        results.get("secrets", {}).get("status") == "success" and
        (results.get("trigger", {}).get("status") == "success" or results.get("webhook_script", {}).get("status") == "success")
    )
...
def validate_ai_requirements() -> dict:
    ...
    if not os.path.exists(script_path):
        ...
        if exit_code == 0:
            ...
            print(f"{Colors.GREEN}✓ All Cline MCP tools meet version requirements{Colors.ENDC}")
        else:
            ...
            if missing_tools:
                ...
                print(f"{Colors.YELLOW}⚠ Missing Cline MCP tools: {', '.join(missing_tools)}{Colors.ENDC}")
            else:
                results["cline"] = {
                    "status": "warning",
                    "message": "Cline MCP tools verification failed"
                }
                print(f"{Colors.YELLOW}⚠ Cline MCP tools verification failed{Colors.ENDC}")
        ...
        try:
            with open(py_file, 'r') as f:
                ...
                if 'google.cloud' in content and 'aiplatform' in content:
                    if ('validate' in content.lower() and 'design' in content.lower()) or ('validate_design_tokens' in content):
    ...
    if found_vertex_validation:
        ...
        print(f"{Colors.GREEN}✓ Found code for validating design tokens via Vertex AI{Colors.ENDC}")
    else:
        results["vertex_validation"] = {
            "status": "warning",
            "message": "Could not find code for validating design tokens via Vertex AI"
        }
        print(f"{Colors.YELLOW}⚠ Could not find code for validating design tokens via Vertex AI{Colors.ENDC}")
    ...
    if found_component_test:
        ...
        print(f"{Colors.GREEN}✓ Found code for generating component test cases{Colors.ENDC}")
    else:
        results["component_test"] = {
            "status": "warning",
            "message": "Could not find code for generating component test cases"
        }
        print(f"{Colors.YELLOW}⚠ Could not find code for generating component test cases{Colors.ENDC}")
    
    # Overall result
    # Consider it a success if either Cline tools pass or we found Vertex AI validation code
    results["passed"] = (
        results.get("cline", {}).get("status") in ["success", "warning"] or
        results.get("vertex_validation", {}).get("status") == "success"
    )
...
def generate_report(validation_results: Dict[str, Dict], output_file: Optional[str] = None) -> None:
    ...
    # Overall status
    if successes == total:
        overall_status = f"{Colors.GREEN}All checks passed ({successes}/{total}){Colors.ENDC}"
    elif successes >= total * 0.8:
        overall_status = f"{Colors.YELLOW}Most checks passed ({successes}/{total}){Colors.ENDC}"
    else:
        overall_status = f"{Colors.RED}Several checks failed ({successes}/{total}){Colors.ENDC}"
    ...
    for section, results in validation_results.items():
        if not results.get("passed", True):
            ...
            if section == "Figma PAT Validation":
                print(f"  • Ensure FIGMA_PAT environment variable is set with a token that has the required scopes:")
                print(f"    - files:read")
                print(f"    - variables:write")
                print(f"    - code_connect:write")
                print(f"  • You can validate it manually: python scripts/validate_figma_pat.py")
                
            elif section == "Component Library Validation":
                print(f"  • Ensure the component-adaptation-mapping.json properly maps to the implementation files")
                print(f"  • Run the component mapping analysis: python scripts/improve_component_mapping.py")
                
            elif section == "Infrastructure Validation":
                if "vertex_ai" in results and results["vertex_ai"].get("status") != "success":
                    print(f"  • Update Vertex AI Workbench configuration to use n1-standard-4 instance type")
                    
                if "firestore" in results and results["firestore"].get("status") != "success":
                    print(f"  • Configure Firestore NATIVE with proper backup policies")
                    
                if "redis" in results and results["redis"].get("status") != "success":
                    print(f"  • Update Memorystore Redis configuration to use 3GB capacity")
                    
                if "roles" in results and results["roles"].get("status") != "success":
                    print(f"  • Ensure service account has all required roles:")
                    for role in ["roles/secretmanager.secretAccessor", "roles/aiplatform.user", "roles/redis.editor"]:
                        if role in results.get("roles", {}).get("message", ""):
                            print(f"    - Add {role}")
                    
            elif section == "CI/CD Pipeline Validation":
                if "trigger" in results and results["trigger"].get("status") != "success":
                    print(f"  • Configure GitHub Actions workflow to trigger on Figma file changes")
                    print(f"    - Add 'on.repository_dispatch.types: [figma-update]' to workflow file")
                    
                if "secrets" in results and results["secrets"].get("status") != "success":
                    print(f"  • Update secrets mapping in scripts/update_github_org_secrets.sh")
                    
                if "webhook_script" not in results or results["webhook_script"].get("status") != "success":
                    print(f"  • Create a script to set up Figma webhook: scripts/setup_figma_webhook.sh")
                ...
                if "cline" in results and results["cline"].get("status") != "success":
                    ...
                    for tool in results.get("cline", {}).get("missing_tools", ["figma-sync", "terraform-linter", "gcp-cost"]):
                        print(f"    - cline install {tool}")
                    
                if "vertex_validation" in results and results["vertex_validation"].get("status") != "success":
                    print(f"  • Implement Vertex AI validation for design tokens")
                    
                if "component_test" in results and results["component_test"].get("status") != "success":
                    print(f"  • Implement Vertex AI component test case generation")
    
    if not any_recommendations:
        print(f"{Colors.GREEN}No recommendations needed - all checks passed!{Colors.ENDC}")
    ...
    if output_file:
        ...
        clean_results = {}
        for section, results in validation_results.items():
            clean_results[section] = {k: v for k, v in results.items() if k != "stdout" and k != "stderr"}
        ...
        try:
            with open(output_file, 'w') as f:
                ...
                f.write("## Summary\n\n")
                f.write(f"Overall Status: {'Passed' if successes == total else 'Warning' if successes >= total * 0.8 else 'Failed'}\n")
                ...
                for section, results in clean_results.items():
                    ...
                    for key, value in results.items():
                        if key not in ["status", "passed", "details", "error"] and isinstance(value, dict):
                ...
                if any_recommendations:
                    for section, results in clean_results.items():
                        if not results.get("passed", True):
                            ...
                            if section == "Figma PAT Validation":
                                f.write("- Ensure FIGMA_PAT environment variable is set with a token that has the required scopes:\n")
                                f.write("  - files:read\n")
                                f.write("  - variables:write\n")
                                f.write("  - code_connect:write\n")
                                f.write("- You can validate it manually: python scripts/validate_figma_pat.py\n")
            ...
            print(f"{Colors.RED}Error saving report to {output_file}: {str(e)}{Colors.ENDC}")

def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description='Validate Figma-GCP synchronization environment')
    parser.add_argument('--figma-pat', help='Figma Personal Access Token (if not provided, will use FIGMA_PAT env var)')
