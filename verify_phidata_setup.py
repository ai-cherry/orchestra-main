...
def check_dependencies() -> bool:
    ...
    try:
        ...
        print(
            f"{GREEN}✅ phi.storage.postgres.pgvector: Installed (PgVector2 class){RESET}"
        )
    except ImportError:
        print(
            f"{RED}❌ phi.storage.postgres.pgvector: Not installed (needed for vector storage){RESET}"
        )
        print(
            f"{YELLOW}   Run: pip install 'phi-postgres>=0.2.0' 'phi-vectordb>=0.1.0'{RESET}"
        )
        all_deps_installed = False
    ...
    try:
        ...
        print(
            f"{GREEN}✅ phi.storage.assistant.pg: Installed (PgAssistantStorage class){RESET}"
        )
    except ImportError:
        print(
            f"{RED}❌ phi.storage.assistant.pg: Not installed (needed for agent storage){RESET}"
        )
    ...
    try:
        ...
        print(
            f"{GREEN}✅ google.cloud.sql.connector: Installed (Connector class){RESET}"
        )
    except ImportError:
        print(
            f"{RED}❌ google.cloud.sql.connector: Not installed (needed for CloudSQL connection){RESET}"
        )
    ...
    try:
        ...
        print(
            f"{YELLOW}⚠️ phi.embedder: Not found (optional, needed for VertexAI embeddings){RESET}"
        )
...
def print_next_steps(
    env_ok: bool, deps_ok: bool, script_ok: bool, terraform_ok: bool
) -> None:
    ...
    if not env_ok:
        ...
        print(
            format_command(
                "cp .env.postgres.example .env.postgres  # Create from example if needed"
            )
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Verify Phidata setup with PostgreSQL."
    )
    parser.add_argument(
        "--test-connection",
        action="store_true",
        help="Test actual PostgreSQL connection",
    )
    ...
    print(
        f"Environment Variables: {GREEN+'OK'+RESET if env_ok else RED+'Missing'+RESET}"
    )
    print(f"Dependencies: {GREEN+'OK'+RESET if deps_ok else RED+'Missing'+RESET}")
    print(
        f"PostgreSQL Setup Script: {GREEN+'Found'+RESET if script_ok else RED+'Missing'+RESET}"
    )
    print(
        f"Environment File: {GREEN+'OK'+RESET if env_file_ok else YELLOW+'Missing/Empty'+RESET}"
    )
    print(
        f"Terraform Files: {GREEN+'OK'+RESET if terraform_ok else RED+'Missing'+RESET}"
    )
    print(
        f"Phidata Agent Example: {GREEN+'Found'+RESET if example_ok else RED+'Missing'+RESET}"
    )
