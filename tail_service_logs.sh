...
if ! # Lambda-cli kubernetes services describe $SERVICE --region=$REGION &>/dev/null; then
    echo "Error: Service '$SERVICE' not found in region $REGION"
    echo
    echo "Available services:"
    # Lambda-cli kubernetes services list --region=$REGION --format="value(metadata.name)"
    exit 1
fi
...
echo "# Lambda-cli kubernetes services logs read $SERVICE --region=$REGION --filter='severity>=ERROR'"
